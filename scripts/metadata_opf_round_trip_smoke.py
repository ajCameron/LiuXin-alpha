...
def open_database(database_path: Path, db_type: str) -> Any:
...
def round_trip_item(
    ...
    *,
    ...
    try:
        ...
        errors = tuple(compare_snapshots(before, after, strict=strict))
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--limit", type=int, default=3, help="Number of item rows to test when --item-id is omitted.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    with quiet_call(open_database, database_path, str(args.db_type), quiet=quiet) as db:
