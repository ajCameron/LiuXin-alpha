...
- Add visible/reportable diagnostics for lossy file-format conversion,
  starting with PML unsupported-character replacement. The current PML output
  fallback may still emit `?`, but conversion should report the loss with
  enough context for users, logs, and future automated conversion reports.
- Design the conversion pipeline as a capability graph, including direct
  format-to-format edges and external-tool adapters where those preserve more
  structure than the legacy `input -> OEB -> output` path.
