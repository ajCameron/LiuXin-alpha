...
def _build_fake_database() -> FakeDatabase:
    ...
    db.add_row(
        ...
        {
            "asset_replica_id": 70,
            "asset_replica_digital_asset_id": 60,
            "asset_replica_mode": "primary",
        },
    )
    db.add_row(
        "entity_identifiers",
        {
            ...
            "entity_identifier_entity_type": "manifestation",
            "entity_identifier_entity_id": 10,
            "entity_identifier_scheme": "isbn_13",
            "entity_identifier_value": "9780000000001",
    ...
    db.interlinks[("items", 1, "digital_assets")] = [
        {
            "digital_asset_item_link_digital_asset_id": 60,
            "digital_asset_item_link_priority": 1,
            "digital_asset_item_link_type": "contains",
            "digital_asset_item_link_primary": 1,
        }
    ]
    return db


def test_manifestation_metadata_hydrator_from_manifestation_id_and_source_row() -> None:
    db = _build_fake_database()
    hydrator = ManifestationMetadataHydrator(db)

    container = hydrator.from_manifestation_id(10)
    assert container.manifestation is not None
    assert container.manifestation.manifestation_id == 10
    assert [row.row_id for row in container.get_related("expressions") if isinstance(row, Row)] == [20]
    ...
    assert [row.row_id for row in container.get_related("files") if isinstance(row, Row)] == [50]
    ...
    assert from_source.manifestation is not None
    assert from_source.manifestation.manifestation_id == 10


def test_manifestation_metadata_container_from_database() -> None:
    db = _build_fake_database()
    container = ManifestationMetadata.from_database(db, manifestation_id=10)
    assert container.manifestation is not None
    assert container.manifestation.manifestation_id == 10
