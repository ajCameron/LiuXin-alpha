...
- Added `--metadata-read-source`, `--cache-type`, and
  `--no-cache-db-fallback` CLI flags to both read-only web surfaces so they can
  serve through a loaded storage cache without custom Python wiring.
...
- Added web-surface parser/route coverage for cache read-source startup wiring.
