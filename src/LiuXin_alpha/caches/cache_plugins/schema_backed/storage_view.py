...
from LiuXin_alpha.caches.cache_plugins.schema_backed.common import _sort_key
...
class SchemaBackedCacheView(CacheViewAPI):
    ...
    def _visible_columns(self) -> tuple[CacheViewColumnSpec, ...]:
        ...
        base_table = cast(
            SchemaBackedMainTableCache,
            self._cache.get_main_table(self.spec.base_table),
        )
        return tuple(
            CacheViewColumnSpec(
                key=str(column_name),
                label=str(column_name),
                field_key=f"{base_table.table}.{column_name}",
            )
            for column_name in base_table.column_headings
        )
    ...
    def _refresh_field_map(self) -> None:
        ...
        for index, column in enumerate(self._visible_columns()):
            ...
            canonical_field_key = str(field.field_key)
            self._field_map[index] = canonical_field_key
            self._fields[canonical_field_key] = field
            self._fields.setdefault(column.key, field)
            self._fields.setdefault(str(field.column_name), field)
    ...
    def value_for(
        ...
        return self._cache.get_cached_value(
            int(row_id),
            field.field_key,
    ...
    def row_values_for_id(self, row_id: int) -> Sequence[Any]:
        return self._cache.get_cached_row_values(
            int(row_id),
            tuple(self._field_map.values()),
        )
