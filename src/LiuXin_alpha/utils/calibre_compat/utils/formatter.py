...
from LiuXin_alpha.utils.calibre_compat.utils.formatter_functions import formatter_functions, compile_user_function
...
class _Parser:
    ...
    def expr(self):
        if self.token_is_id():
            ...
            return cls.eval_(self.parent, self.parent_kwargs, self.parent_book, self.locals, *args)
...
class _CompileParser(_Parser):
    ...
    def expr(self, level):
        ...
        if self.token_is_id():
            ...
            if self.compile_text:
                self.compile_text += (
                    "\targs[%d].append(self.__funcs__['%s']" ".eval_(formatter, kwargs, book, locals, *args[%d]))\n"
                ) % (level, id, level + 1)
            return cls.eval_(self.parent, self.parent_kwargs, self.parent_book, self.locals, *args)
        elif self.token_is_constant():
            # String or number
            v = six_unicode(self.token())
            if self.compile_text:
                tv = v.replace("\\", "\\\\")
                tv = tv.replace("'", "\\'")
    ...
    def _eval_program(self, val, prog, column_name):
        ...
        if tweaks["compile_gpm_templates"]:
            if column_name is not None and self.template_cache is not None:
                ...
                compile_text = (
                    "__funcs__ = formatter_functions().get_functions()\n"
