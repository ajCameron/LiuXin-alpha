...
# ---------------------------------------------------------------------------
# Some callers (and our tests) expect imageops.resize(...) to exist, but the
# primary implementation focuses on higher-level operations. Delegate to the
# CLI-based fallback implementation for resize, which operates purely on bytes.
try:
    from .imageops_alt_fallback import resize as resize  # type: ignore[F401]
except Exception as _e:  # pragma: no cover
    def resize(data: bytes, width: int, height: int, fmt: str = "png") -> bytes:
        raise RuntimeError("ImageMagick resize unavailable") from _e
