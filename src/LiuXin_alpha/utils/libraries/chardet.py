...
def substitute_entites(raw):
    from LiuXin.utils.calibre import xml_entity_to_unicode
...
def detect_xml_encoding(raw, verbose=False, assume_utf8=False):
    ...
    if encoding.lower().replace("_", "-").strip() in (
        ...
        # Microsoft Word exports to HTML with encoding incorrectly set to gb2312 instead of gbk. gbk is a superset of
        # gb2312, anyway.
        encoding = "gbk"
    try:
...
def recode_to_utf8(raw, decode_errors="ignore", encode_errors="ignore"):
    """
    Recode a unicode string to utf-8.
    ...
    :return:
    """
    # Detect the encoding - decode then re-encode
    raw_encoding = force_encoding(raw=raw, verbose=False)
    raw = raw.decode(encoding=raw_encoding, errors=decode_errors)
    return raw.encode(encoding="utf-8", errors=encode_errors)
