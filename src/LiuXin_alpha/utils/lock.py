...
# Considered basic functionality, so all logging just using print
...
def test_exclusive_file(path=None):
    if path is None:
        ...
        with ExclusiveFile(f):
            ...
            err = fork_job("LiuXin_alpha.utils.lock", "test_exclusive_file", (f,))["result"]
            if err is not None:
