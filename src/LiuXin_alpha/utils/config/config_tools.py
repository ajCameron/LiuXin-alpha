...
# if False:
#     prefs, ConfigProxy, Config, StringConfig  # Needed to make pyflakes happy - comment out for pycharm


def to_json(obj):
    """
    Serialize an object into a json string - provides some additional functionality for objects which are not otherwise
    serializable.
    Allows you to serialize datetime objects as well as the standard JSON complement
    :param obj:
    :return:
    """
...
def from_json(obj):
    ...
    Can also handle datetime objects.
    :param obj:
    :return:
    """
    if "__class__" in obj:
...
class OptionParser(optparse.OptionParser):
    ...
    def merge_options(self, lower, upper):
        ...
        Default values in upper are overridden by non default values in lower.
        """
        ...
        if os.path.exists(self.file_path):
            with ExclusiveFile(self.file_path) as f:
                ...
                try:
                    ...
                    print("Unexpected exception type: {}".format(type(e).__name__))
                    import traceback
