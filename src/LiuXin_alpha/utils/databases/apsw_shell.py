    ...
    * https://sqlite.org/src/info/2466653295
    ...
    def __init__(self, stdin=None, stdout=None, stderr=None, encoding="utf8", args=None, db=None):
        ...
        if self.interactive is None:
            self.interactive = (
                getattr(self.stdin, "isatty", False)
                and self.stdin.isatty()
                and getattr(self.stdout, "isatty", False)
    ...
    def _ensure_db(self):
        ...
        if not self._db:
            ...
            self._db = apsw.Connection(
                self.dbfilename,
                flags=apsw.SQLITE_OPEN_URI | apsw.SQLITE_OPEN_READWRITE | apsw.SQLITE_OPEN_CREATE,
            )
        return self._db
...
""" % (
    ...
    def process_sql(self, sql, bindings=None, internal=False, summary=None):
        ...
        try:
            ...
            if not internal and self.echo:
                ...
                if (
                    last
                    and last.f_code.co_name == "sqlite3_prepare"
        ...
        *recipes_segdir* etc.  Consequently to dump this example
        ...
        try:
            ...
            def comment(s):
                ...
                self.write(
                    self.stdout,
                    textwrap.fill(s, 78, initial_indent="-- ", subsequent_indent="-- ") + "\n",
                )
            ...
            comment("DatabasePing: " + self.db.filename)
            try:
                import getpass
                import socket
            ...
            comment(
                ...
                + str(self.db.cursor().execute("pragma max_page_count").fetchall()[0][0])
                + ";\n"
            )
            blank()
            ...
            try:
                ...
                # Dump the table
                for table in tables:
                    for sql in self.db.cursor().execute(
                        ...
                        if sql[0].lower().split()[:3] == ["create", "virtual", "table"]:
                            ...
                            self.write(
                                self.stdout,
                                "DROP TABLE IF EXISTS " + self._fmt_sql_identifier(table) + ";\n",
                            )
                            self.write(self.stdout, sqldef(sql[0]))
                            self._output_table = self._fmt_sql_identifier(table)
                            self.process_sql(
                                "select * from " + self._fmt_sql_identifier(table),
                                internal=True,
                            )
    ...
    def command_find(self, cmd):
        ...
        for (table,) in self.db.cursor().execute(
    ...
    def command_import(self, cmd):
        ...
        try:
            ...
            # how many columns?
            ncols = len(
                self.db.cursor().execute("pragma table_info(" + self._fmt_sql_identifier(cmd[1]) + ")").fetchall()
            )
            if ncols < 1:
            ...
            sql = "insert into %s values(%s)" % (
                self._fmt_sql_identifier(cmd[1]),
                ",".join("?" * ncols),
            )
    ...
    def command_autoimport(self, cmd):
        ...
        try:
            ...
            self.write(
                self.stdout,
                "Detected Format %s  Columns %d  Rows %d\n" % (fmt, ncols, lines),
            )
            # Header row
                ...
                if len(datas[i]) > 1:
                    raise self.Error(
                        ...
                        % (
                            i + 1,
                            header[i],
                            ", ".join([dl.__name__ for dl in datas[i]]),
                        )
                    )
                if datas[i]:
                    datas[i] = datas[i][0]
        ...
        def write(self, dest, text):
            ...
            try:
                ...
                if (
                    ev.args[0] == "ascii"
                    and dest.encoding
                    and ev.args[0] != dest.encoding
                    and hasattr(dest, "fileno")
                    and isinstance(dest.fileno(), int)
                    and dest.fileno() >= 0
                ):
                    args = [
                        dest.encoding,
