...
class CombinedStyle(object):
    ...
    def serialize(self, styles, normal_style):
        ...
        p_pr = makeelement(block, "w:pPr")
        self.bs.serialize_properties(p_pr, normal_style.bs)
        if self.outline_level is not None:
            makeelement(p_pr, "w:outlineLvl", w_val=str(self.outline_level + 1))
        r_pr = makeelement(block, "w:rPr")
        self.rs.serialize_properties(r_pr, normal_style.rs)
...
class StylesManager(object):
    ...
    def finalize(self, all_blocks):
        ...
        self.log.debug(
            "%d Text Styles %d Combined styles" % tuple(map(len, (self.descendant_text_styles, self.combined_styles)))
        )
