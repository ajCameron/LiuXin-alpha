...
class Blocks(object):
    ...
    def __exit__(self, etype, value, traceback):
        ...
        if 0 < self.pos < len(self.all_blocks):
...
class Convert(object):
    ...
    def __call__(self):
        ...
        if (
            ...
            and six_unicode(self.oeb.metadata.cover[0]) in self.oeb.manifest.ids
        ):
            cover_id = six_unicode(self.oeb.metadata.cover[0])
    ...
    def process_tag(self, html_tag, stylizer, is_first_tag=False, float_spec=None):
        ...
        if display in {"inline", "inline-block"} or tagname == "br":  # <br> has display:block but we dont want to
