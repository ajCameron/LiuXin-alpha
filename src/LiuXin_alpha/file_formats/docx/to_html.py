...
class Convert(object):
        ...
        for p, wp in iteritems(self.object_map):
            if len(p) > 0 and not p.text and len(p[0]) > 0 and not p[0].text and p[0][0].get("class", None) == "tab":
                ...
                if style.text_indent is inherit or (
                    hasattr(style.text_indent, "endswith") and style.text_indent.endswith("pt")
                ):
                        ...
                        if "id" not in para.attrib:
                            para.set(
                                ...
                                generate_anchor(
                                    ...
                                    frozenset(itervalues(self.anchor_map)),
                                ),
                            )
            ...
            if x.tag.endswith("}r"):
                ...
                if anchor and anchor not in self.anchor_map and anchor != "_GoBack":
                    ...
                    self.anchor_map[anchor] = current_anchor = generate_anchor(
                        anchor, frozenset(itervalues(self.anchor_map))
                    )
                ...
                self.anchor_map[anchor] = current_anchor = generate_anchor(
                    "toc", frozenset(itervalues(self.anchor_map))
                )
    ...
    def wrap_elems(self, elems, wrapper):
        ...
        for elem in elems:
            try:
                ...
                # Probably a hyperlink that spans multiple paragraphs,theoretically we should break this up into
        ...
        for child in run:
            if self.namespace.is_tag(child, "w:t"):
                ...
                for img in self.images.to_html(child, self.current_page, self.docx, self.dest_dir):
                    text.add_elem(img)
                    ans.append(text.elem)
            elif self.namespace.is_tag(child, "w:footnoteReference") or self.namespace.is_tag(
    ...
    def mark_block_runs(self, paras):
        ...
        def process_run(run):
            ...
            if has_visible_border:
                border_style.margin_left, border_style.margin_right = (
                    max_left,
                    max_right,
                )
