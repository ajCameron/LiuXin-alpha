...
class Styles(object):
    ...
    def __call__(self, root, fonts, theme):
        ...
        def resolve(local_s, local_p):
            if local_p is not None:
                ...
                local_s.resolve_based_on(local_p)
            local_s.resolved = True
    ...
    def run_val(self, parent_styles, direct_formatting, attr):
        ...
        if attr in direct_formatting.toggle_properties:
            ...
            vals = [
                ...
                if rs is not self.default_character_style and getattr(rs, attr) is not inherit
            ]
            if vals:
                return sum(vals) % 2 == 1
    ...
    def resolve_paragraph(self, p):
        ...
        if ans is None:
            ...
            if (
                ...
                and getattr(linked_style.paragraph_style, "numbering", inherit) is not inherit
            ):
        ...
        for p, runs in iteritems(layers):
            ...
            for prop in ("font_family", "font_size", "color"):
                if has_links and prop == "color":
                    # We cannot promote color as browser rendering engines will override the link color setting it to
                    # blue, unless the color is specified on the link element itself
                    continue
    ...
    def generate_css(self, dest_dir, docx, notes_nopb, nosupsub):
        ...
        if not notes_nopb:
            s += "dl.notes dd { page-break-after: always }"
