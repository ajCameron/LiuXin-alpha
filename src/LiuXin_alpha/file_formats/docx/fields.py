...
scanner = re.Scanner(
    [
        ...
        (
            r'[^\s\\"]\S*',
            lambda s, t: (t, WORD),
        ),  # A non-quoted word, must not start with a backslash or a space or a
        #                                            quote
        (r"\s+", None),
    ],
    flags=re.DOTALL,
...
parse_index = parser(
    ...
    " l:page-number-separator p:letter-range s:sequence-name r:run-together "
    "y:yomi z:langcode",
)
...
class Fields(object):
    ...
    def __call__(self, doc, log):
        ...
        for elem in self.namespace.XPath(
            '//*[name()="w:p" or name()="w:r" or name()="w:instrText" or '
            '(name()="w:fldChar" and (@w:fldCharType="begin" or '
            '@w:fldCharType="end"))]'
        )(doc):
