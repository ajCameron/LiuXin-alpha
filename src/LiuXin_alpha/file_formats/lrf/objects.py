...
class TextCSS(object):
    ...
    def to_css(cls, obj, inline=False):
        ...
        if fs is not None:
            ans += item("font-size: %fpt;" % (int(fs) / 10.0))
        ...
        if lh is not None:
            ans += item("text-align: %fpt;" % (int(lh) / 10.0))

        pi = getattr(obj, "parindent", None)

        if pi is not None:
            ans += item("text-indent: %fpt;" % (int(pi) / 10.0))
...
class Text(LRFStream):
    ...
    def __unicode__(self):
        ...
        for c in self.content:
            if isinstance(c, six_string_types):
        ...
        if len(open_containers) > 0:
            if len(open_containers) == 1:
                ...
                raise LRFParseError(
                    "Malformed text stream %s" % ([i.name for i in open_containers if isinstance(i, Text.TextTag)],)
                )
        return s
