...
class LrsParser(object):
    ...
    def third_pass(self):
        ...
        for item_id, tag in self.objects.items():
            if tag.name in local_map.keys():
                settings = self.attrs_to_dict(tag, local_map[tag.name][1] + ["objid", "objlabel"])
    ...
    def second_pass(self):
        ...
        self._style_labels = {}
        for object_id, tag in self.objects.items():
            if tag.name in map.keys():
                ...
                if x:
                    self._style_labels[x] = self.parsed_objects[object_id]
    ...
    def first_pass(self):
        ...
        def me(base, tagname):
            ...
            if tag is None:
                return "", "", ""
            tag = (
                self.tag_to_string(tag),
                tag.get("reading") if "reading" in tag else "",
            )
            return tag

        title = me(book_info, "Title")
        author = me(book_info, "Author")
        publisher = me(book_info, "Publisher")
        category = me(book_info, "Category")[0]
        classification = me(book_info, "Classification")[0]
        freetext = me(book_info, "FreeText")[0]
        language = me(doc_info, "Language")[0]
        creator = me(doc_info, "Creator")[0]
        producer = me(doc_info, "Producer")[0]
        bookid = me(book_info, "BookID")[0]
