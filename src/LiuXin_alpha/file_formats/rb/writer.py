...
try:
    ...
    # Image # May be needed to keep pyflakes happy.
except ImportError:
    import Image
...
class RBWriter(object):
    ...
    def _text(self, oeb_book):
        ...
        for i in range(0, (len(text) + TEXT_RECORD_SIZE - 1) / TEXT_RECORD_SIZE):
            ...
            pages.append(
                zobj.compress(text[i * TEXT_RECORD_SIZE : (i * TEXT_RECORD_SIZE) + TEXT_RECORD_SIZE]) + zobj.flush()
            )
        ...
        for item in manifest:
            if item.media_type in OEB_RASTER_IMAGES:
                try:
                    ...
                    im = Image.open(six_cStringIO(item.data)).convert("L")
                    data = six_cStringIO()
                    im.save(data, "PNG")
                    data = data.getvalue()
