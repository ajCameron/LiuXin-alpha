...
def sony_metadata(oeb):
    ...
    try:
        pt = six_unicode(oeb.metadata.publication_type[0])
        short_title = ":".join(pt.split(":")[2:])
    except:
        pass

    try:
        date = parse_date(six_unicode(m.date[0]), as_utc=False).strftime("%Y-%m-%d")
    except:
        date = strftime("%Y-%m-%d")
    try:
        language = six_unicode(m.language[0]).replace("_", "-")
    except:
        language = "en"
    ...
    try:
        base_id = six_unicode(list(filter(cal_id, m.identifier))[0])
    except:
        base_id = str(uuid4())
    ...
    if False and toc.depth() < 3:
        ...
        toc = TOC(
            ...
            title=six_unicode(oeb.metadata.title[0]),
        )
