...
try:
    ...
    HAS_QT = False
    QImage = QFont = QPainter = QPointF = QTextLayout = QTextOption = QFontMetrics = QTextCharFormat = None
...
def layout_text(prefs, img, title, subtitle, footer, max_height, style):
    ...
    if subtitle:
        ...
        subtitle_block.position = (
            style.hmargin,
            title_block.position.y + title_block.height + gap,
        )
    ...
    footer_block.position = (
        style.hmargin,
        img.height() - style.vmargin - footer_block.height,
    )
...
def format_fields(mi, prefs):
    ...
    return tuple(safe_format(field) for field in ("title_template", "subtitle_template", "footer_template"))
...
def _basic_format_text(mi):
    ...
    footer = escape_formatting(" & ".join(authors[:2])) if authors else escape_formatting(_("Unknown"))

    subtitle_parts = []
    series = getattr(mi, "series", None) or ""
    if series:
        subtitle_parts.append(force_unicode(series))
        sidx = getattr(mi, "series_index", None)
        if sidx not in (None, "", 0):
            try:
                formatted_idx = fmt_sidx(sidx, use_roman=get_use_roman())
            except Exception:
                formatted_idx = force_unicode(sidx)
            subtitle_parts.append(formatted_idx)

    subtitle = escape_formatting(" - ".join(subtitle_parts))
    subtitle = "<i>" + subtitle if subtitle else ""
...
def _safe_format_text(mi, prefs):
    try:
        ...
        # Template formatting can fail on partially-ported formatter internals.
        # Fall back to plain metadata so cover generation still works without Qt.
...
def load_styles(prefs, respect_disabled=True):
    ...
    ans = tuple(
        ...
        if isinstance(x, type) and issubclass(x, Style) and x is not Style and x.NAME not in disabled
    )
    if not ans and disabled:
        # If all styles have been disabled, ignore the disabling and return all the styles
...
def _draw_fallback_cover_with_pillow(title, subtitle, footer, width, height):
    ...
    title_font = load_font(max(18, img.height // 18))
    ...
    def text_height(font):
        try:
            ...
            return max(1, draw.textsize("Ag", font=font)[1])

    title_h = text_height(title_font)
    subtitle_h = text_height(subtitle_font)
    footer_h = text_height(footer_font)
    ...
    def wrap_lines(text, font):
        ...
        for word in words[1:]:
            ...
            try:
                ...
                rendered_width = draw.textsize(test, font=font)[0]
            if rendered_width <= max_text_width:
                cur = test
            else:
                lines.append(cur)
                cur = word
...
def _fallback_cover_bytes(title, subtitle, footer, width, height):
    ...
    if data is not None:
        return data
    data = _fallback_resource_cover_bytes()
    if not data:
        return b""
    try:
        from LiuXin_alpha.utils.plugins.fallbacks.magick import Image as FallbackImage

        with FallbackImage(data) as img:
            img.resize(max(64, int(width)), max(64, int(height)))
            return img.to_bytes(format="jpeg")
    except Exception:
        return data
...
def generate_cover(mi, prefs=None, as_qimage=False):
    ...
    for block, local_color in zip((title_block, subtitle_block, footer_block), colors):
        p.setPen(local_color)
        block.draw(p)
    p.end()
...
def generate_masthead(title, output_path=None, width=600, height=60, as_qimage=False, font_family=None):
    if not HAS_QT:
        if as_qimage:
            raise RuntimeError("Cannot return a QImage when PyQt5 is unavailable")
        data = _fallback_cover_bytes(sanitize(title), "", "", width, height)
