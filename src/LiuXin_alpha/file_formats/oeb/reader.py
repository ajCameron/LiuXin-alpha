...
class OEBReader(object):
    ...
    def _manifest_add_missing(self, invalid):
        ...
        while unchecked:
            ...
            for item in unchecked:
                ...
                if item.media_type in OEB_DOCS or item.media_type[-4:] in (
                    "/xml",
                    "+xml",
                ):
            ...
            for href in new:
                ...
                if href not in warned:
                    ...
                    warned.add(href)
                item_id, _ = manifest.generate(id="added")
                guessed = guess_type(href)[0]
                media_type = guessed or BINARY_MIME
                added = manifest.add(item_id, href, media_type)
                unchecked.add(added)
    ...
    def _manifest_from_opf(self, opf):
        ...
        for elem in xpath(opf, "/o2:package/o2:manifest/o2:item"):
            ...
            if not self.oeb.container.exists(href):
                ...
                continue
            if man_id in manifest.ids:
                self.logger.warn("Duplicate manifest id %r" % man_id)
                man_id, href = manifest.generate(man_id, href)
            manifest.add(man_id, href, media_type, fallback)
    ...
    def _find_ncx(self, opf):
        ...
        if result:
            element_id = result[0]
            if element_id not in self.oeb.manifest.ids:
                return None
            item = self.oeb.manifest.ids[element_id]
            self.oeb.manifest.remove(item)
            return item
    ...
    def _toc_from_navpoint(self, item, toc, navpoint):
        ...
        for child in children:
            ...
            author_element = xpath(child, 'descendant::calibre:meta[@name = "author"]')
            if author_element:
                author = author_element[0].text
            else:
                author = None

            description_element = xpath(child, 'descendant::calibre:meta[@name = "description"]')
            if description_element:
                description = etree.tostring(description_element[0], method="text", encoding=six_unicode).strip()
                if not description:
                    description = None
            else:
                description = None
    ...
    def _toc_from_ncx(self, item):
        ...
        title = COLLAPSE_RE.sub(" ", title.strip())
        title = title or six_unicode(self.oeb.metadata.title[0])
        toc = self.oeb.toc
        toc.title = title
    ...
    def _toc_from_opf(self, opf, item):
        ...
        if self._toc_from_ncx(item):
            return
        # Prefer HTML to tour based TOC, since several LIT files have good HTML TOCs but bad tour based TOCs
    ...
    def _find_page_map(self, opf):
        ...
        if result:
            ...
            if manifest_id not in self.oeb.manifest.ids:
                return None
            item = self.oeb.manifest.ids[manifest_id]
            self.oeb.manifest.remove(item)
            return item
        for item in self.oeb.manifest.values():
    ...
    def _cover_from_html(self, hcover):
        ...
        manifest_id, href = self.oeb.manifest.generate("cover", "cover.jpg")
    ...
    def _locate_cover_image(self):
        ...
        if self.oeb.metadata.cover:
            ...
            item = self.oeb.manifest.ids.get(cand_id, None)
            if item is not None and item.media_type in OEB_IMAGES:
                return item
            else:
                self.logger.warn("Invalid cover image @id %r" % cand_id)
        ...
        if self.COVER_SVG_XP(html):
            ...
            cand_id, href = self.oeb.manifest.generate(hcover.id, href)
            item = self.oeb.manifest.add(cand_id, href, SVG_MIME, data=svg)
            return item
        if self.COVER_OBJECT_XP(html):
            local_object = self.COVER_OBJECT_XP(html)[0]
            href = hcover.abshref(local_object.get("data"))
            item = self.oeb.manifest.hrefs.get(href, None)
            if item is not None and item.media_type in OEB_IMAGES:
                return item
