...
class CSSFlattener(object):
    ...
    def __call__(self, oeb, context):
        ...
        for item in oeb.manifest.values():
            ...
            if item.media_type in OEB_STYLES:
                cssutils.replaceUrls(item.data, item.abshref, ignoreImportRules=True)
    ...
    def get_embed_font_info(self, family, failure_critical=True):
        ...
        if not faces:
            msg = "No embeddable fonts found for family: %r" % self.opts.embed_font_family
            if failure_critical:
        ...
        for i, font in enumerate(faces):
            ...
            cfont = {
                ...
                "panose-1": " ".join(map(six_unicode, font["panose"])),
                "src": "url(%s)" % item.href,
            }
    ...
    def clean_edges(self, cssdict, style, fsize):
        ...
        for kind in ("margin", "padding"):
            for edge in ("bottom", "top"):
                edge_property = "%s-%s" % (kind, edge)
                if edge_property not in cssdict:
                    continue
                if "%" in cssdict[edge_property]:
                    continue
                value = style[edge_property]
                if value == 0:
                    continue
                elif value <= slineh:
                    cssdict[edge_property] = "%0.5fem" % (dlineh / fsize)
                else:
                    try:
                        value = round(value / slineh) * dlineh
                    except Exception as e:
                        self.oeb.logger.warning(
                            "Exception message: {} - Invalid length:".format(e.message),
                            value,
                        )
                        value = 0.0
    ...
    def flatten_node(self, node, stylizer, names, styles, pseudo_styles, psize, item_id):
        ...
        try:
            ...
            self.oeb.logger.exception("Failed to set minimum line-height - exception message: {}".format(e.message))
    ...
    def replace_css(self, css):
        ...
        css_id, href = manifest.generate("css", "stylesheet.css")
        item = manifest.add(css_id, href, CSS_MIME, data=cssutils.parseString(css, validate=False))
    ...
    def collect_global_css(self):
        ...
        for css in global_css:
            ...
            if css.strip():
                ...
                manifest.add(id_, href, CSS_MIME, data=cssutils.parseString(css, validate=False))
            gc_map[css] = href
    ...
    def flatten_spine(self):
        ...
        items = sorted(
            ((key, val) for (val, key) in iteritems(styles)),
            key=lambda local_x: numeric_sort_key(local_x[0]),
        )

        # :hover must come after link and :active must come after :hover
        psels = sorted(
            iterkeys(pseudo_styles),
            key=lambda local_x: {"hover": 1, "active": 2}.get(local_x, 0),
        )
        for psel in psels:
