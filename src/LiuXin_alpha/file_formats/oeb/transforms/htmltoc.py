...
class HTMLTOCAdder(object):
    ...
    def __call__(self, oeb, context):
        ...
        if style not in STYLE_CSS:
            ...
            style = "nested"
        content_id, css_href = oeb.manifest.generate("tocstyle", "tocstyle.css")
        oeb.manifest.add(content_id, css_href, CSS_MIME, data=STYLE_CSS[style])
        ...
        content_id, href = oeb.manifest.generate("contents", "contents.xhtml")
        item = oeb.manifest.add(content_id, href, XHTML_MIME, data=contents)
        if self.position == "end":
            oeb.spine.add(item, linear=False)
        else:
            oeb.spine.insert(0, item, linear=True)
