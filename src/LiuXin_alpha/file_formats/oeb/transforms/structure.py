...
class DetectStructure(object):
    ...
    def __call__(self, oeb, opts):
        ...
        if self.oeb.auto_generated_toc or opts.use_auto_toc:
            ...
            if self.oeb.toc.count() < 2 < orig_toc.count():
                self.oeb.toc = orig_toc
            else:
                self.oeb.auto_generated_toc = True
                self.log("Auto generated TOC with %d entries." % self.oeb.toc.count())
    ...
    def detect_start_reading(self):
        ...
        for item in self.oeb.spine:
            ...
            if matches:
                ...
                if not eid:
                    eid = "start_reading_at_" + six_unicode(uuid.uuid4()).replace("-", "")
                    elem.set("id", eid)
                if "text" in self.oeb.guide:
                    self.oeb.guide.remove("text")
                self.oeb.guide.add("text", "Start", item.href + "#" + eid)
                self.log("Setting start reading at position to %s in %s" % (self.opts.start_reading_at, item.href))
                return
            ...
            for a in XPath("//h:a[@href]")(item.data):
                ...
                if not purl[0] or purl[0] == "file":
                    ...
                    if not self.oeb.toc.has_href(href):
                        ...
                        if num >= self.opts.max_toc_links > 0:
    ...
    def elem_to_link(self, item, elem, title_attribute, counter):
        ...
        elem_id = elem.get("id", "calibre_toc_%d" % counter)
        elem.set("id", elem_id)
