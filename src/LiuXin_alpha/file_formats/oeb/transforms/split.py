...
class SplitError(ValueError):
    def __init__(self, path, root):
        ...
        ValueError.__init__(
            self,
            _("Could not find reasonable point at which to split: " "%(path)s Sub-tree size: %(size)d KB")
...
class Split(object):
    ...
    def find_page_breaks(self, item):
        ...
        page_breaks.sort(key=lambda local_x: int(local_x.get("pb_order")))
...
class FlowSplitter(object):
    ...
    def split_on_page_breaks(self, orig_tree):
        ...
        while ordered_ids:
            ...
            for i in memory_range(len(self.trees) - 1, -1, -1):
                ...
                elem = pattern(tree)
                if elem:
                    self.log.debug("\t\tSplitting on page-break at id=%s" % elem[0].get("id"))
                ...
                if ids:
                    ...
                    if body is not None:
                        for x in ids:
                            body.insert(
                                0,
                                body.makeelement(XHTML("div"), id=x, style="height:0pt"),
                            )
                ids = set([])
                trees.append(tree)
        self.trees = trees
    ...
    def is_page_empty(self, root):
        ...
        txt_cleanup_pat = r"\s+|\xa0"
        try:
            # Python 2 - encode the raw string as unicode
            txt_cleanup_pat = txt_cleanup_pat.decode("raw_unicode_escape")
        ...
        txt = re.sub(
            txt_cleanup_pat,
            "",
            etree.tostring(body, method="text", encoding=six_unicode),
        )
        if len(txt) > 1:
            return False
    ...
    def split_to_size(self, tree):
        ...
        for t, size in zip(trees, sizes):
            ...
            if self.is_page_empty(r):
                ...
                self.log.debug("\t\t\tCommitted sub-tree #%d (%d KB)" % (len(self.split_trees), size / 1024.0))
            else:
    ...
    def find_split_point(self, root):
        ...
        def pick_elem(local_elems):
            if local_elems:
                ...
                if local_elems:
                    i = int(math.floor(len(local_elems) / 2.0))
                    local_elems[i].set(SPLIT_POINT_ATTR, "1")
                    return local_elems[i]
    ...
    def commit(self):
        ...
        for current, tree in zip(*map(reversed, (self.files, self.trees))):
            for a in tree.getroot().xpath("//h:a[@href]", namespaces=NAMESPACES):
                ...
                if href.startswith("#"):
                    anchor = href[1:]
                    current_file = self.anchor_map[anchor]
                    current_file = self.item.relhref(current_file)
                    if current_file != current:
                        a.set("href", current_file + href)
        ...
        def fix_toc_entry(toc):
            if toc.href:
                ...
                if local_href == self.item.href:
                    local_nhref = self.anchor_map[local_frag if local_frag else None]
                    if local_frag:
                        local_nhref = "#".join((local_nhref, local_frag))
                    toc.href = local_nhref
            for x in toc:
                fix_toc_entry(x)
