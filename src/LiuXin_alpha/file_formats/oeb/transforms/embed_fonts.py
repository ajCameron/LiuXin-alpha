...
def used_font(style, embedded_fonts):
    ff = [
        ...
        if six_unicode(f).lower() not in {"serif", "sansserif", "sans-serif", "fantasy", "cursive", "monospace"}
    ]
    if not ff:
        return False, None
...
class EmbedFonts(object):
            ...
            for i, rule in enumerate(item.data.cssRules):
                ...
                for sel in rule.selectorList:
                    ...
                    if sel and sel.startswith("."):
                        # We dont care about pseudo-selectors as the worst that can happen is some extra characters will
                        # remain in the font
                        sel = sel.partition(":")[0]
    ...
    def find_usage_in(self, elem, inherited_style, ff_rules):
        ...
        if existing is None:
            ...
            if in_book is None:
                ...
                # TODO: Create a page rule from the book rule (cannot use it directly as paths might be different)
                item = in_book["item"]
    ...
    def embed_font(self, style):
        ...
        for f in fonts:
            if (
                f["weight"] == weight
                and f["font-style"] == style.get("font-style", "normal")
                and f["font-stretch"] == style.get("font-stretch", "normal")
            ):
                ...
                css = """@font-face { font-family: "%s"; font-weight: %s; font-style: %s; font-stretch: %s; src:
                url(%s) }""" % (
                    f["font-family"],
                    f["font-weight"],
                    f["font-style"],
