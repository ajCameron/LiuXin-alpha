...
def namespaces(test, parse_function):
    ...
    """.format(
        xhtml=XHTML_NS, svg=SVG_NS, xlink=XLINK_NS
    )
    root = parse_function(markup)
    ...
    if parse_function is not html5_parse:
        markup = '<html:html xmlns:html="{html}" id="a"><html:body><html:p></html:p>' "</html:body></html>".format(
            html=XHTML_NS
        )
        root = parse_function(markup)
...
class ParsingTests(BaseTest):
    ...
    def test_polish_parser(self):
        ...
        for ds in (False, True):
            ...
            for tag, lnum in iteritems({"html": 2, "head": 3, "body": 3, "p": 3, "svg": 4, "image": 4, "b": 5}):
                ...
                self.assertEqual(
                    lnum,
                    elem.sourceline,
                    "Line number incorrect for %s, source: %s:" % (tag, src),
                )
        ...
        root = parse(
            ...
            'xmlns:xml="http://www.w3.org/XML/1998/namespace"><body/></html>'
        )
        self.assertNotIn("xmlnsU0003Axml", root.attrib, "xml namespace declaration not removed")

        root = parse(
            '<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en-US" xmlns:extra="extra">' "<body/></html>"
        )
        self.assertIn(
            "extra",
            root.nsmap,
            "Extra namespace declaration on <html> tag not preserved",
        )
...
def timing():
    ...
    for name, f in (
        ...
        ("liuxin_html5lib", vanilla),
        ("calibre-old", html5_parse),
    ):
        timings = []
        for i in memory_range(10):
            st = time.time()
            f(raw)
