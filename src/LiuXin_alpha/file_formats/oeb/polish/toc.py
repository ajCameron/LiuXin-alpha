...
def from_xpaths(container, xpaths):
    ...
    level_prev = {i + 1: None for i in memory_range(len(xpaths))}
    ...
    for name, level_item_map in iteritems(maps):
        ...
        for item in root.iterdescendants(etree.Element):
            ...
            level_prev[lvl] = toc
            for i in memory_range(lvl + 1, len(xpaths) + 1):
                level_prev[i] = None
...
def add_id(container, name, loc, totals=None):
    ...
    try:
        ...
        try:
            ...
            raise MalformedMarkup(
                _("The file %s has malformed markup. Try running the Fix HTML tool" " before editing.") % name
            )
...
def create_ncx(toc, to_href, btitle, lang, uid):
    ...
    etree.SubElement(head, NCX("meta"), name="dtb:uid", content=six_unicode(uid))
    ...
    def process_node(xml_parent, toc_parent):
        for child in toc_parent:
            ...
            if child_title:
                child_title = spat.sub(" ", child_title)
            etree.SubElement(label, NCX("text")).text = child_title
            if child.dest:
...
def toc_to_html(toc, container, toc_name, title, lang=None):
    def process_node(html_parent, local_toc, level=1, indent="  ", style_level=2):
        ...
        name, frag = local_toc.dest, local_toc.frag
        href = "#"
        if name:
        ...
        if len(local_toc) > 0:
            ...
            parent.text = "\n" + (indent * (level + 3))
            parent.tail = "\n\n" + (indent * (level + 1))
            for local_child in local_toc:
                process_node(parent, local_child, level + 3, style_level=style_level + 1)
            parent[-1].tail = "\n" + (indent * (level + 2))
