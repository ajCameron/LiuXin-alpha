...
class Unmanifested(BadLink):
    ...
    def __init__(self, name, unreferenced=None):
        ...
        if unreferenced is not None:
            self.INDIVIDUAL_FIX = (
                _("Remove %s from the book") % name if unreferenced else _("Add %s to the manifest") % name
            )
...
class MimetypeMismatch(BaseError):
    ...
    def __init__(self, container, name, opf_mt, ext_mt):
        ...
        BaseError.__init__(
            self,
            _("The file %s has a mimetype that does not match its extension") % name,
            container.opf_name,
        )
...
def check_links(container):
            ...
            for href, lnum, col in container.iterlinks(name):
                ...
                if tname is not None:
                    if container.exists(tname):
                        if tname in container.mime_map:
                            ...
                            if os.path.isdir(apath):
                                a(
                                    BadLink(
                                        _("The linked resource %s is a directory") % fl(href),
                                        name,
                                        lnum,
                                        col,
                                    )
                        ...
                        if cname is not None:
                            ...
                            a(
                                BadLink(
                                    _("The linked resource %s does not exist") % fl(href),
                                    name,
                                    lnum,
                                    col,
                                )
                    ...
                    if purl.scheme == "file":
                        ...
                        a(
                            LocalLink(
                                _("The link %s points to a file outside the book") % fl(href),
                                name,
                                lnum,
                                col,
                            )
    ...
    spine_styles = {
        tname for name in spine_docs for tname in links_map[name] if container.mime_map.get(tname, None) in OEB_STYLES
    }
    num = -1
    while len(spine_styles) > num:
        ...
        spine_styles |= {
            tname
            for name in spine_styles
            for tname in links_map[name]
            if container.mime_map.get(tname, None) in OEB_STYLES
        }
    seen = set(OEB_DOCS) | set(OEB_STYLES)
    spine_resources = {
        tname
        for name in spine_docs | spine_styles
        for tname in links_map[name]
