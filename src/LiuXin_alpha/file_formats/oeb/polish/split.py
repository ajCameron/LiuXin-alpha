...
from LiuXin_alpha.file_formats.oeb.base import barename, XPNSMAP, XPath, OPF, XHTML, OEB_DOCS
...
from LiuXin_alpha.file_formats.oeb.polish.toc import node_from_loc
...
def adjust_split_point(split_point: _typing.Any, log: _typing.Any) -> _typing.Any:
    ...
    while True:
        ...
        if (
            parent is None
            or barename(parent.tag) in {"body", "html"}
...
def do_split(split_point: _typing.Any, log: _typing.Any, before: bool = True) -> tuple[_typing.Any, ...]:
    ...
    if before:
        ...
        # contains any content after the original split point
    ...
    body, body2 = six_map(get_body, (root, root2))
    ...
    def nix_element(local_elem: _typing.Any, top: bool = True) -> None:
        ...
        if top:
            ...
            index = local_elem_parent.index(local_elem)
            local_elem_parent[index : index + 1] = list(local_elem.iterchildren())
        ...
        if hit_split_point:
            if keep_descendants:
                if elem in split_point_descendants:
                    ...
                    # We are out of split_point, so prevent further set lookups of split_point_descendants
                    keep_descendants = False
            nix_element(elem)
...
def split(container: _typing.Any, name: _typing.Any, loc_or_xpath: _typing.Any, before: bool = True, totals: _typing.Any = None) -> _typing.Any:
    ...
    if isinstance(loc_or_xpath, type("")):
        try:
            matches = root.xpath(loc_or_xpath)
        except Exception:
            raise AbortError(_("Invalid split XPath expression: %s") % loc_or_xpath)
        if not matches:
            raise AbortError(_("The expression %s did not match any nodes") % loc_or_xpath)
        split_point = matches[0]
    else:
        try:
            ...
            try:
                ...
                raise MalformedMarkup(
                    _("The file %s has malformed markup. Try running the Fix HTML tool" " before splitting") % name
                )
    ...
    for r, rname, anchors in [
        (root1, bottom_name, anchors_in_bottom),
        ...
        for a in r.xpath("//*[@href]"):
            ...
            if fname == name:
                ...
                if purl.fragment in anchors:
                    a.set(
                        "href",
                        "%s#%s" % (container.name_to_href(rname, name), purl.fragment),
                    )
...
def multisplit(container: _typing.Any, name: _typing.Any, xpath: _typing.Any, before: bool = True) -> _typing.Any:
    ...
    current = name
    all_names = [name]
    for i in memory_range(len(nodes)):
...
def merge_html(container: _typing.Any, names: _typing.Any, master: _typing.Any) -> None:
    ...
    for name in names:
        ...
        for first_child in children:
            if not isinstance(first_child, six_string_types):
                break

        if isinstance(first_child, six_string_types):
        ...
        # If this poit in the code is reached, then the attribute exists and is not empty - this false shows up as
        # errors in PyCharm
        ...
        for child in children:
            if isinstance(child, six_string_types):
