...
raw_null_namespace_regex = r"""(name\(\) = ['"])h:"""
try:
    # Python 2 - supports ur (don't think it should be needed here, but retaining it just in case)
    NULL_NAMESPACE_REGEX = re.compile(raw_null_namespace_regex.decode("raw_unicode_escape"))
except AttributeError:
    # Python 3 - raw is equivalent - using that
...
class CSSSelector(object):
    ...
    def __call__(self, node, log):
        ...
        try:
            ...
            log.exception("Failed to run CSS selector: %s" % self.css)
            return []
            ...
            if not hasattr(self, "ci_sel"):
                ...
                if self.ci_sel is not None:
                    try:
                        ...
                        log.exception("Failed to run case-insensitive CSS selector: %s" % self.css)
                        return []
                    if ans:
                        if not self.used_ci_sel:
                            log.warn("Interpreting class and id values case-insensitively in selector: %s" % self.css)
...
class Stylizer(object):
    ...
    def __init__(self, tree, path, oeb, opts, profile=None, extra_css="", user_css=""):
        ...
        for sheet_index, stylesheet in enumerate(stylesheets):
            ...
            for rule in stylesheet.cssRules:
                ...
                if rule.type == rule.MEDIA_RULE:
                    ...
                    media = {rule.media.item(i) for i in memory_range(rule.media.length)}
        ...
        raw_pseudo_pat = r":{1,2}(first-letter|first-line|link|hover|visited|active|focus|before|after)"
        try:
            # Python 2 - provides an emulation of the ur string prefix
            raw_pseudo_pat = raw_pseudo_pat.decode("raw_unicode_escape")
        except AttributeError:
            # Python 3 - this is not needed
            pass

        pseudo_pat = re.compile(raw_pseudo_pat, re.I)
        for _, _, cssdict, text, _ in rules:
            ...
            if fl is not None:
                fl = fl.group(1)
                if fl == "first-letter" and getattr(self.oeb, "plumber_output_format", "").lower() == "mobi":
                    # Fake first-letter
    ...
    def backgroundColor(self):
        ...
        if self._bgcolor is None:
            ...
            if val and validate_color(val):
                ...
                if val is not None:
                    try:
                        style = parseStyle("background: {}".format(val), validate=False)
