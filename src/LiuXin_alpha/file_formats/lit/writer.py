...
def invert_tag_map(tag_map):
    ...
    tattrs = [dict((v, k) for k, v in (local_map or {}).items()) for local_map in tattrs]
    for local_map in tattrs:
        if local_map:
            local_map.update(dattrs)
    tattrs[0] = dattrs
    return tags, tattrs
...
BLOCK_ITSF = "\x49\x54\x53\x46\x04\x00\x00\x00" "\x20\x00\x00\x00\x01\x00\x00\x00"

MSDES_CONTROL = "\x03\x00\x00\x00\x29\x17\x00\x00" "\x01\x00\x00\x00\xa5\xa5\x00\x00"
    ...
    def tree_to_binary(self, elem, nsrmap=NSRMAP, parents=None, inhead=False, preserve=False):
        ...
        for attr, value in attrib.items():
            ...
            if attr in ("href", "src"):
                ...
                prefix = six_unichar(3)
                if path in self.manifest.hrefs:
                    prefix = six_unichar(2)
        ...
        for next_chain in chain(elem, [None]):
            if self.stylizer:
                nstyle = None if next_chain is None else self.stylizer.style(next_chain)
            if child is not None:
                if (
                    not preserve
                    and (inhead or not nstyle or self.is_block(cstyle) or self.is_block(nstyle))
                    and child.tail
                    and child.tail.isspace()
                ):
                    child.tail = None
                self.tree_to_binary(child, nsrmap, parents, inhead, preserve)
            child, cstyle = next_chain, nstyle
        parents.pop()
...
class LitWriter(object):
    ...
    def _dump_stream(self, oeb, stream):
        ...
        self._stream = stream
        self._sections = [six_cStringIO() for i in memory_range(4)]
        self._directory = []
        self._meta = None
        self._litize_oeb()
        self._write_content()
    ...
    def _write_content(self):
        ...
        offset = self._tell()
        pieces = list(memory_range(offset, offset + (PIECE_SIZE * 5), PIECE_SIZE))
        ...
        self._write("IFCM", pack("<IIIQQ", 1, DCHUNK_SIZE, 0x100000, ULL_NEG1, number))
        ...
        self._write("IFCM", pack("<IIIQQ", 1, CCHUNK_SIZE, 0x20000, ULL_NEG1, 1))
        cchunk = six_cStringIO()
        last = 0
    ...
    def _build_data(self):
        ...
        for item in self._oeb.manifest.values():
            if item.media_type not in LIT_MIMES:
                self._logger.warn(
                    "File %r of unknown media-type %r " "excluded from output." % (item.href, item.media_type)
                )
                continue
            name = "/data/" + item.id
            data = item.data
    ...
    def _build_manifest(self):
        ...
        for state in states:
            ...
            for item in items:
                ...
                entry = [
                    six_unichar(len(item_id)),
                    six_unicode(item_id),
                    six_unichar(len(href)),
                    six_unicode(href),
                    six_unichar(len(media_type)),
                    six_unicode(media_type),
                ]
                for value in entry:
    ...
    def _build_dchunks(self):
        ...
        for entry in directory:
            ...
            usedlen = dchunk.tell() + len(local_next) + (len(quickref) * 2) + 52
