...
class TCRCompressor(object):
    ...
    The list takes the form 1 byte length and then a string. Each position in the list corresponds to a code found in
    the file. The coded text is a string of characters values. for instance the character Q represents the value 81
    which corresponds to the string in the code list at position 81.
    """
    ...
    def _combine_codes(self):
        ...
        for code in possible_codes:
            ...
            self.codes[ord(code[0])] = "%s%s" % (
                self.codes[ord(code[0])],
                self.codes[ord(code[1])],
            )
    ...
    def _new_codes(self):
        ...
        possible_new_codes = [x[0] for x in sorted(new_codes_count, key=lambda local_c: local_c[1])]
    ...
    def compress(self, txt):
        ...
        while possible_codes and self.unused_codes:
            while possible_codes and self.unused_codes:
                ...
                self.codes[unused_code] = "%s%s" % (
                    self.codes[ord(code1)],
                    self.codes[ord(code2)],
                )
