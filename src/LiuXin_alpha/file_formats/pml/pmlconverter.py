...
class PML_HTMLizer(object):
    ...
    def prepare_pml(self, pml):
        ...
        pml = re.sub(
            ...
            % (
                match.group("c"),
                self.strip_pml(match.group("text")),
                match.group("text"),
                match.group("c"),
            ),
            pml,
        ...
        pml = re.sub(
            ...
            lambda match: '\\FN="%s"%s\\FN' % (match.group("target"), match.group("text"))
            if match.group("text")
            else "",
            pml,
        )
        pml = re.sub(
            ...
            lambda match: '\\SB="%s"%s\\SB' % (match.group("target"), match.group("text"))
            if match.group("text")
            else "",
            pml,
        )
    ...
    def cleanup_html_remove_redundant(self, html):
        for key in self.STATES_TAGS.keys():
            ...
            if key in self.STATES_VALUE_REQ:
                html = re.sub(r"(?u)%s\s*%s" % (tag_open % ".*?", tag_close), "", html)
            else:
                html = re.sub(r"(?u)%s\s*%s" % (tag_open, tag_close), "", html)
            ...
            for c in self.DIV_STATES + self.SPAN_STATES:
                ...
                if self.state[c][0]:
                    if c in self.STATES_VALUE_REQ:
                        ...
                        text += self.STATES_TAGS[self.CODE_STATES[c]][0] % (
                            self.state[c][1],
                            self.state[c][1],
                        )
                    else:
    ...
    def parse_pml(self, pml, file_name=""):
        ...
        adv_indent_val = ""
        # Keep track of the number of empty lines between paragraphs. When we reach a set number we assume it's a
        # soft scene break.
        empty_count = 0
            ...
            while c != "":
                ...
                if c == "\\":
                    ...
                    if c in "qcriIuobBlk":
                        ...
                        if c in "XC":
                            level = line.read(1)
                        local_id = "pml_toc-%s" % len(self.toc)
                        ...
                        if not value or value == "":
                            ...
                            self.toc.append(
                                (
                                    level,
                                    (os.path.basename(self.file_name), local_id, value),
                                )
                            )
                            text = '%s<span id="%s"></span>' % (t, local_id)
                    elif c == "m":
                        empty = False
                        ...
                        empty = False
                        local_id = self.code_value(line)
                        text = '<span id="%s"></span>' % local_id
                    elif c == "p":
                        empty = False
            ...
            if level == "0":
                ...
                if t_l2 is None:
                    if t_l1 is None:
                        ...
                        t_l2 = t_l1
                t_l3 = t_l2.add_item(href, toc_id, text)
            # Level 4.
            ...
            # it as level 4.
                ...
                if t_l3 is None:
                    if t_l2 is None:
                        ...
                        t_l3 = t_l2
                t_l3.add_item(href, toc_id, text)
...
def footnote_sidebar_to_html(pre_id, id, pml):
    ...
    if id.strip():
        html = (
            '<br /><br style="page-break-after: always;" /><div id="%s-%s">%s<small>'
            '<a href="#r%s-%s">return</a></small></div>' % (pre_id, id, pml_to_html(pml), pre_id, id)
        )
    else:
