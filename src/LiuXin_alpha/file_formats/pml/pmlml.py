...
class PMLMLizer(object):
    ...
    def get_text(self):
        ...
        for item in self.oeb_book.spine:
            ...
            content = six_unicode(etree.tostring(item.data, encoding=six_unicode))
    ...
    def prepare_text(self, text):
        ...
        try:
            ...
            prepare_pat = prepare_pat.decode("raw_unicode_escape")
    ...
    def clean_text(self, text):
        ...
        clean_pat = r'(?msu)(?P<t>\\(x|X[0-4]))(?P<a>.*?)(?P<c>\\C[0-4]\s*=\s*"[^"]*")(?P<b>.*?)(?P=t)'
        try:
            # Python 2 - transform the string into unicode
            clean_pat = clean_pat.decode("raw_unicode_escape")
        except AttributeError:
        ...
        if self.opts.remove_paragraph_spacing:
            ...
            text = re.sub(
                "(?imu)^(?P<text>.+)$",
                lambda mo: mo.group("text")
                if re.search(r"\\[XxCmrctTp]", mo.group("text"))
                else "        %s" % mo.group("text"),
                text,
            )
        else:
            text = re.sub("\n{3,}", "\n\n", text)
