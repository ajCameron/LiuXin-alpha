...
class FB2MLizer(object):
    ...
    def fb2mlize_spine(self: _typing.Self) -> str:
        ...
        if self.opts.pretty_print:
            return '<?xml version="1.0" encoding="UTF-8"?>\n%s' % etree.tostring(
                etree.fromstring(output), encoding=six_unicode, pretty_print=True
            )
        else:
    ...
    def fb2_header(self: _typing.Self) -> str:
        ...
        metadata["keywords"] = ""
        tags = list(map(six_unicode, self.oeb_book.metadata.subject))
        if tags:
        ...
        if self.oeb_book.metadata.series:
            ...
            metadata["sequence"] = '<sequence name="%s" number="%s" />' % (
                prepare_string_for_xml("%s" % self.oeb_book.metadata.series[0]),
                index,
            )
    ...
    def get_cover(self: _typing.Self) -> str:
        ...
        if self.oeb_book.metadata.cover and six_unicode(self.oeb_book.metadata.cover[0]) in self.oeb_book.manifest.ids:
            cover_id = six_unicode(self.oeb_book.metadata.cover[0])
            cover_item = self.oeb_book.manifest.ids[cover_id]
            ...
            if page_name:
                ...
                try:
                    ...
                    info_str = "AttributeError while trying to find the first image in the page"
                    self.log.log_exception(info_str, e, "INFO", ("cover_item", cover_item))

                # Try the fallback
                if cover_href is None:

                    img = cover_item.data.find(".//img")
                    try:
                        if img is not None:
                            cover_href = cover_item.abshref(img.get("src"))
                    except AttributeError as e:
                        info_str = "AttributeError while trying to run the fallback to get cover"
                        self.log.log_exception(info_str, e, "INFO", ("img", img))
    ...
    def dump_text(
        ...
        if not isinstance(elem_tree.tag, six_string_types) or namespace(elem_tree.tag) != XHTML_NS:
            p = elem.getparent()
