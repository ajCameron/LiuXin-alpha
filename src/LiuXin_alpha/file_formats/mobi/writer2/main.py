...
class MobiWriter(object):
    ...
    def __call__(self, oeb, path_or_stream):
        ...
        if oeb.metadata.publication_type:
            x = six_unicode(oeb.metadata.publication_type[0]).split(":")
            if len(x) > 1:
                pt = x[1].lower()
    ...
    def generate_index(self):
        ...
        try:
            ...
            for i in memory_range(self.last_text_record_idx + 1):
                if i == 0:
                    continue
    ...
    def write_uncrossable_breaks(self):  # {{{
        ...
        for i in memory_range(1, self.last_text_record_idx + 1):
    ...
    def generate_record0(self):  # MOBI header {{{
        ...
        sir = 0xFFFFFFFF
        if self.primary_index_record_idx is not None and self.indexer.secondary_record_offset is not None:
        ...
        record0.write(
            pack(
                b">I",
                0xFFFFFFFF if self.primary_index_record_idx is None else self.primary_index_record_idx,
            )
        )
    ...
    def generate_joint_record0(self):  # {{{
        ...
        first_image_record = first_image_record if first_image_record else len(self.records)
