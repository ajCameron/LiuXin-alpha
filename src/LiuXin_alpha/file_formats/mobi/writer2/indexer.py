        ...
        if is_periodical:
            ...
            if not data:
                ...
                for l in itervalues(depth_map):
                    l.sort(key=lambda local_x: local_x.offset)
        ...
        if spanner is not None:
            ...
            if not completes and ((len(starts) == 1 and not ends) or (len(ends) == 1 and not starts)):
                ...
                for x in (starts, completes, ends):
                    nodes.extend(x)
                nodes.sort(key=lambda local_x: local_x.index)
...
class Indexer(object):  # {{{
    ...
    def create_book_index(self: _typing.Self) -> _typing.Any:  # {{{
        ...
        indices.sort(key=lambda local_x: local_x.offset)
    ...
    def create_periodical_index(self: _typing.Self) -> _typing.Any:  # {{{
        ...
        for sec in periodical_node:
            ...
            if normalized_articles:
                normalized_articles.sort(key=lambda local_x: local_x.offset)
                normalized_sections.append((section, normalized_articles))

        normalized_sections.sort(key=lambda local_x: local_x[0].offset)
        ...
        for i, x in list(enumerate(normalized_sections)):
            sec, normalized_articles = x
            normalized_articles = list(six_filter(lambda local_x: local_x.length > 0, normalized_articles))
            normalized_sections[i] = (sec, normalized_articles)

        normalized_sections = list(
            six_filter(
                lambda local_x: local_x[0].length > 0 and local_x[1],
                normalized_sections,
            )
        )
    ...
    def calculate_trailing_byte_sequences(self: _typing.Self) -> None:
        ...
        for i in memory_range(self.number_of_text_records):
