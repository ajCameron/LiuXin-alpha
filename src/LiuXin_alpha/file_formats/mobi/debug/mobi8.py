...
class MOBIFile(object):
    def __init__(self, mf):
        ...
        if mf.kf8_type == "joint":
            ...
            self.resource_ranges.insert(
                0,
                (h.first_resource_record, h.last_resource_record, h.first_image_index),
            )

        self.text_records = [
            TextRecord(i, r, h8.extra_data_flags, mf.decompress8)
            for i, r in enumerate(
                mf.records[first_text_record + offset : first_text_record + offset + h8.number_of_text_records]
            )
        ]
    ...
    def read_tbs(self):
        ...
        indexing_data = collect_indexing_data(entry_map, list(six_map(len, self.text_records)))
        self.indexing_data = [
            DOC
            + "\n"
            + textwrap.dedent(
                    ...
                    for e in entries:
                        desc.append(
                            ...
                            % (
                                ...
                                e.action,
                                e.parent,
                                e.index - (e.parent or 0),
                                e.start - i * RECORD_SIZE,
