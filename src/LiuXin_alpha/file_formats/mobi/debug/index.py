...
Elem = namedtuple("Chunk", "insert_pos toc_text file_number sequence_number start_pos length")
...
def read_variable_len_data(data, header):
    ...
    if offset > 0:
        ...
        for i in memory_range(header["count"]):
        ...
        if self.table is not None:
            for i, text in enumerate(iterkeys(self.table)):
                ...
                if set(iterkeys(tag_map)) != {1, 6}:
                    raise ValueError("SKEL Index has unknown tags: %s" % (set(iterkeys(tag_map)) - {1, 6}))
                self.records.append(
                    File(
                        i,  # file_number
                        text,  # name
        ...
        if self.table is not None:
            for i, text in enumerate(iterkeys(self.table)):
                ...
                if set(iterkeys(tag_map)) != {2, 3, 4, 6}:
                    raise ValueError("Chunk Index has unknown tags: %s" % (set(iterkeys(tag_map)) - {2, 3, 4, 6}))
        ...
        if self.table is not None:
            for i, text in enumerate(iterkeys(self.table)):
                ...
                if set(iterkeys(tag_map)) not in ({1, 6}, {1, 2, 3}):
                    raise ValueError("Guide Index has unknown tags: %s" % tag_map)
...
class NCXIndex(Index):
    def __init__(self, ncxidx, records, codec):
        ...
        if self.table is not None:
            ncxentry = namedtuple(
                "NCXEntry",
                "index start length depth parent " "first_child last_child title pos_fid kind",
            )
