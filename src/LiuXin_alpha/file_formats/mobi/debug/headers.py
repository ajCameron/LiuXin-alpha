...
class PalmDB(object):
    ...
    def __str__(self):
        ...
        ans.append("Creation date: %s (%s)" % (self.creation_date.isoformat(), self.creation_date_raw))
        ans.append("Modification date: %s (%s)" % (self.modification_date.isoformat(), self.modification_date_raw))
        ans.append("Backup date: %s (%s)" % (self.last_backup_date.isoformat(), self.last_backup_date_raw))
...
class EXTHHeader(object):
    def __init__(self, raw):
        ...
        self.length = struct.unpack(b">L", self.raw[4:8])[0]
        self.count = struct.unpack(b">L", self.raw[8:12])[0]

        pos = 12
        self.records = []
        for i in memory_range(self.count):
...
class MOBIHeader(object):  # {{{
    def __init__(self, record0, offset):
        ...
        }.get(struct.unpack(b">H", self.compression_raw)[0], repr(self.compression_raw))
        ...
        self.encryption_type_raw = struct.unpack(b">H", self.raw[12:14])[0]
        ...
        if self.has_extra_data_flags:
            ...
            (
                ...
                self.flis_number,
                self.flis_count,
            ) = struct.unpack(b">IIII", self.raw[200:216])
        ...
        # Default to first record after all text records
        self.first_resource_record = offset + 1 + self.number_of_text_records
        pointer = min(
            getattr(self, "first_non_book_record", NULL_INDEX),
    ...
    def __str__(self):
        ...
        if self.has_extra_data_flags:
            ...
            a(
                ("Extra data flags: %s (has multibyte: %s) " "(has indexing: %s) (has uncrossable breaks: %s)")
                % (
                    bin(self.extra_data_flags),
                    self.has_multibytes,
                    self.has_indexing_bytes,
                    self.has_uncrossable_breaks,
                )
            )
        ...
        ans += "\nNumber of bytes after full name: %d" % (len(self.raw) - (self.fullname_offset + self.fullname_length))
...
class MOBIFile(object):
    def __init__(self, stream):
        ...
        self.record_headers = []
        self.records = []
        for i in memory_range(self.palmdb.number_of_records):
            pos = 78 + i * 8
    ...
    def __init__(self, idx, record, extra_data_flags, decompress):
        ...
        for typ, val in iteritems(self.trailing_data):
            if isinstance(typ, int):
                print("Record %d has unknown trailing data of type: %d : %r" % (idx, typ, val))
