...
class Tag(object):  # {{{
    ...
    TAG_MAP = {
        ...
        69: (
            "image_index",
            "Offset from first image record to the"
...
class TBSIndexing(object):  # {{{
    def __init__(self: _typing.Self, text_records: _typing.Any, indices: _typing.Any, doc_type: _typing.Any) -> None:
        ...
        for r in text_records:
            ...
            for entry in indices:
                ...
                iend = istart + sz - 1
                has_start = end >= istart >= start
                has_end = end >= iend >= start
                rec = None
    ...
    def dump_record(self: _typing.Self, r: _typing.Any, dat: _typing.Any) -> tuple[_typing.Any, ...]:
        ...
        ans.append(
            "\tContains: %d index entries (%d ends, %d complete, %d starts)" % tuple(map(len, (s + e + c, e, c, s)))
        )
        ...
        for typ, entries in (("Ends", e), ("Complete", c), ("Starts", s)):
            if entries:
                ...
                for x in entries:
                    ans.append(
                        "\t\tIndex Entry: %s (Parent index: %s, Depth: %d, Offset: %d, Size: %d) [%s]"
        ...
        def read_section_transitions(byts: _typing.Any, psi: _typing.Any = None) -> _typing.Any:  # {{{
            ...
            while byts:
                ...
                if extra.get(0b1000, None) is not None:
                    if len(extra) > 1:
                        raise ValueError(
                            "Dont know how to interpret flags" " %r while reading section transitions" % extra
                        )
                    nsi = self.get_index(psi.index + 1)
                    ans.append(
                        "Last article in this record of section %d"
                        " (relative to next section index [%d]): "
                        "%d [%d absolute index]" % (psi.index, nsi.index, ai, ai + nsi.index)
                    )
                    psi = nsi
                    continue

                ans.append(
                    "First article in this record of section %d"
                ...
                if num is None:
                    ...
                    msg = ("Number of articles in this record of section %d: %d") % (
                        psi.index,
                        num,
                    )
                ans.append(msg)
                ...
                if offset is not None:
                    if offset == 0:
                        ...
                        ans.append(
                            ...
                            " of record: %d [%d absolute offset]" % (psi.index + 1, offset, offset + record_offset)
                        )
            return byts
...
class MOBIFile(object):  # {{{
    def __init__(self: _typing.Self, mf: _typing.Any) -> None:
        ...
        if pir != NULL_INDEX:
            ...
            self.cncx = CNCX(
                self.records[pir + 1 + numi : pir + 1 + numi + self.index_header.num_of_cncx_blocks],
                self.index_header.index_encoding,
            )
            self.index_record = IndexRecord(self.records[pir + 1 : pir + 1 + numi], self.index_header, self.cncx)
            self.indexing_record_nums = set(memory_range(pir, pir + 1 + numi + self.index_header.num_of_cncx_blocks))
        ...
        if sir != NULL_INDEX:
            ...
            self.secondary_index_record = IndexRecord(
                self.records[sir + 1 : sir + 1 + numi],
                self.secondary_index_header,
                self.cncx,
            )
            self.indexing_record_nums |= set(memory_range(sir + 1, sir + 1 + numi))

        ntr = self.mobi_header.number_of_text_records
        fii = self.mobi_header.first_image_index
        self.text_records = [
            TextRecord(r, self.records[r], self.mobi_header.extra_data_flags, mf.decompress6)
            for r in memory_range(1, min(len(self.records), ntr + 1))
        ]
        ...
        for i in memory_range(
            self.mobi_header.first_resource_record,
            min(self.mobi_header.last_resource_record, len(self.records)),
        ):
