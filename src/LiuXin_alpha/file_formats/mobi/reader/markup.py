...
def update_internal_links(mobi8_reader, log):
    ...
    for part in mr.parts:
        ...
        for j in memory_range(1, len(srcpieces), 2):
            tag = srcpieces[j]
            if tag.startswith(b"<"):
        ...
        for j in range(len(srcpieces)):
            ...
            if tag.startswith("<"):
                srcpieces[j] = within_tag_amznpagebreak_position_pattern.sub(
                    lambda local_m: ' style="page-break-after:%s"' % local_m.group(1),
                    tag,
                )
        part = "".join(srcpieces)
        parts[i] = part
...
def update_flow_links(mobi8_reader, resource_map, log):
    ...
    for flow in mr.flows:
        ...
        # links to raster image files from image tags image_pattern
            ...
            if tag.startswith("<im") or tag.startswith("<svg:image"):
                for m in img_index_pattern.finditer(tag):
                    ...
                    if href:
                        ...
                        log.warn("Referenced image %s was not recognized as a valid image in %s" % (num, tag))
                srcpieces[j] = tag
        flow = "".join(srcpieces)
        ...
        for j in range(1, len(srcpieces), 2):
            ...
            for m in font_index_pattern.finditer(tag):
                ...
                href = resource_map[num - 1]
                if href is None:
                    log.warn("Referenced font %s was not recognized as a valid font in %s" % (num, tag))
                else:
                    replacement = '"%s"' % ("../" + href)
