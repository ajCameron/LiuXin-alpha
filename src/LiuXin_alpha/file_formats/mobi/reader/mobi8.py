...
Part = namedtuple("Part", "num type filename start end aid")
...
class Mobi8Reader(object):
    ...
    def __call__(self: _typing.Self) -> _typing.Any:
        ...
        if self.mobi6_reader.kf8_type == "joint":
            ...
            self.resource_offsets = [
                (
                    get_first_resource_index(bh.first_image_index, bh.mobi6_records, 1),
                    offset - 2,
                ),
                (
                    get_first_resource_index(bh.kf8_first_image_index, bh.records, offset),
            ...
            self.resource_offsets = [
                (
                    get_first_resource_index(bh.first_image_index, bh.records, offset),
                    len(self.mobi6_reader.sections),
                )
            ]
    ...
    def build_parts(self: _typing.Self) -> None:
        ...
        for skelnum, skelname, divcnt, skelpos, skellen in self.files:
            ...
            inspos_warned = False
            for i in memory_range(divcnt):
                insertpos, idtext, filenum, seqnum, startpos, length = self.elems[divptr]
                if i == 0:
                    aidtext = idtext[12:-2]
                ...
                if tail.find(b">") < tail.find(b"<") or head.rfind(b">") < head.rfind(b"<"):
                    ...
                    if not inspos_warned:
                        self.log.warn(
                            "The div table for %s has incorrect insert " "positions. Calculating manually." % skelname
                        )
                        inspos_warned = True
                    bp, ep = locate_beg_end_of_tag(
                        skeleton,
                        aidtext if isinstance(aidtext, bytes) else aidtext.encode("utf-8"),
                    )
        ...
        for j in memory_range(1, len(self.flows)):
    ...
    def read_inline_toc(self: _typing.Self, href: _typing.Any, frag: _typing.Any) -> _typing.Any:
        ...
        def node_depth(local_elem: _typing.Any) -> _typing.Any:
            ...
            while local_parent is not None:
                local_parent = local_parent.getparent()
                local_ans += 1
            return local_ans
