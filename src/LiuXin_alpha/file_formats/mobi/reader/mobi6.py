...
class MobiReader(object):
    ...
    def __init__(
        ...
        self.book_header = bh = BookHeader(
            self.sections[0][0],
            self.ident,
            user_encoding,
            self.log,
        ...
        # Ancient PRC files from Baen can have random values for mobi_version, so be conservative
    ...
    def extract_content(self, output_dir, parse_cache):
        ...
        self.processed_html = re.sub(r"&(\S+?);", xml_entity_to_unicode, self.processed_html)
    ...
    def extract_text(self, offset=1):
        ...
        text_sections = [
            self.text_section(i)
            for i in memory_range(offset, min(self.book_header.records + offset, len(self.sections)))
        ]
    ...
    def extract_images(self, processed_records, output_dir):
        ...
        for i in range(start, self.num_sections):
            ...
            with open(path, "wb") as jpeg_save_file:
