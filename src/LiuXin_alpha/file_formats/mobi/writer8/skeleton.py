...
def tostring(raw: _typing.Any, **kwargs: _typing.Any) -> _typing.Any:
    ...
    kwargs["encoding"] = six_unicode
    kwargs["xml_declaration"] = False
    ans = etree.tostring(raw, **kwargs)
    if xml_declaration:
        ans = '<?xml version="1.0" encoding="%s"?>\n' % encoding + ans
    return re.sub(r"&#x([0-9A-Fa-f]+);", lambda m: mychr(int(m.group(1), 16)), ans).encode(encoding)
...
class Chunker(object):
    ...
    def merge_small_chunks(self: _typing.Self, chunks: _typing.Any) -> _typing.Any:
        ...
        for chunk in chunks[1:]:
            ...
            if (
                chunk.starts_tags
                or len(chunk) + len(prev) > CHUNK_SIZE  # Starts a tag in the skel
                or prev.ends_tags  # Too large  # Prev chunk ended a tag
            ):
                ans.append(chunk)
            else:
                prev.merge(chunk)
    ...
    def create_tables(self: _typing.Self) -> None:
        ...
        num = 0
        for loop_skel in self.skeletons:
            cp = 0
            for chunk in loop_skel.chunks:
                # This is not the same as the Chunk class - it's the named tuple above
                self.chunk_table.append(
                    Chunk(
                        chunk.insert_pos + loop_skel.start_pos,
                        chunk.selector,
                        loop_skel.file_number,
                        num,
                        cp,
                        len(chunk.raw),
                    )
    ...
    def set_internal_links(self: _typing.Self, text: _typing.Any, rebuilt_text: _typing.Any) -> _typing.Any:
        ...
        for match in re.finditer(rb'<[^>]+? aid=[\'"]([A-Z0-9]+)[\'"]', rebuilt_text):
            offset = match.start()
            pos_fid = None
