    ...
    for term in metadata:
        ...
        if term == "creator":
            if prefer_author_sort:
                creators = [authors_to_sort_string([six_unicode(c)]) for c in items]
            else:
                creators = [six_unicode(c) for c in items]
            items = creators
        elif term == "rights":
            try:
                rights = utf8_text(six_unicode(metadata.rights[0]))
            except:
                rights = b"Unknown"
