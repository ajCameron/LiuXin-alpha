    ...
    def extract_css_into_flows(self):
        ...
        def fix_import_rules(local_sheet):
            ...
            for rule in local_sheet.cssRules.rulesOfType(CSSRule.IMPORT_RULE):
                if rule.href:
                    rule_href = item.abshref(rule.href)
                    local_idx = sheets.get(rule_href, None)
                    if local_idx is not None:
                        local_idx = to_ref(local_idx)
                        rule.href = "kindle:flow:%s?mime=text/css" % local_idx
                        changed = True
            return changed
    ...
    def extract_svg_into_flows(self):
        ...
        for item in self.oeb.spine:
            ...
            for svg in XPath("//svg:svg")(root):
                raw = etree.tostring(svg, encoding=six_unicode, with_tail=False)
                idx = len(self.flows)
                self.flows.append(raw)
                p = svg.getparent()
                pos = p.index(svg)
                img = etree.Element(XHTML("img"), src="kindle:flow:%s?mime=image/svg+xml" % to_ref(idx))
                p.insert(pos, img)
                extract(svg)
    ...
    def create_indices(self):
        ...
        def get_next_start(local_entry):
            enders = [
                e["offset"]
                for e in entries
                if e["depth"] <= local_entry["depth"] and e["offset"] > local_entry["offset"]
            ]
            if enders:
                return min(enders)
