...
class Index(object):  # {{{
    ...
    def __call__(self):
        ...
        RECORD_LIMIT = 0x10000 - self.HEADER_LENGTH - 1048
                ...
                if values:
                    for val in values:
                        try:
                            ...
                            raise ValueError("Invalid values for %r: %r" % (tag, values))
            raw = buf.getvalue()
        ...
        index_records = []
        for index_block, idxt_block, record_count in six_zip(index_blocks, idxt_blocks, record_counts):
        ...
        for last_idx, num in six_zip(last_indices, record_counts):
            start = buf.tell()
...
class NCXIndex(Index):
    ...
    def __init__(self, toc_table):
        ...
        self.entries = list(six_map(to_entry, toc_table))
...
if __name__ == "__main__":
    ...
    os.chdir("/t")
    paras = ["<p>%d</p>" % i for i in memory_range(4000)]
