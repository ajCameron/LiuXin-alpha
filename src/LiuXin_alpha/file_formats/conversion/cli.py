...
def check_command_line_options(parser, args, log):
    ...
    usr_input = os.path.abspath(args[1])
    if (
        not usr_input.endswith(".recipe")
        and not os.access(usr_input, os.R_OK)
        and not ("-h" in args or "--help" in args)
    ):
        log.error("Cannot read from", usr_input)
        raise SystemExit(1)
    if usr_input.endswith(".recipe") and not os.access(usr_input, os.R_OK):
        usr_input = args[1]

    output = args[2]
    if output.startswith(".") and output[:2] not in {"..", "."} and "/" not in output and "\\" not in output:
        output = os.path.splitext(os.path.basename(usr_input))[0] + output
...
def add_input_output_options(parser, plumber):
    ...
    if input_options:
        ...
        io = OptionGroup(
            parser,
            title,
            _("Options to control the processing" " of the input %s file") % plumber.input_fmt,
        )
    ...
    if output_options:
        ...
        oo = OptionGroup(
            parser,
            title,
            _("Options to control the processing" " of the output %s") % plumber.output_fmt,
        )
...
def create_option_parser(args, log):
    ...
    plumber = Plumber(usr_input, output, log, reporter)
...
def main(args=sys.argv):
    ...
    recommendations = [
        (n.dest, getattr(opts, n.dest), OptionRecommendation.HIGH) for n in parser.options_iter() if n.dest
    ]
