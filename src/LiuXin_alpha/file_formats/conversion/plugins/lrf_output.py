...
class LRFOutput(OutputFormatPlugin):
    ...
    options = {
        OptionRecommendation(
            ...
            option_help=_("Enable autorotation of images that are wider than the screen width."),
        ),
        OptionRecommendation(
            name="wordspace",
            recommended_value=2.5,
            level=OptionRecommendation.LOW,
            option_help=_("Set the space between words in pts. Default is %default"),
        ),
        OptionRecommendation(
            name="header",
            recommended_value=False,
            option_help=_("Add a header to all the pages with title and author."),
        ),
        OptionRecommendation(
            name="header_format",
            recommended_value="%t by %a",
            option_help=_(
                "Set the format of the header. %a is replaced by the author " "and %t by the title. Default is %default"
            ),
        ),
        OptionRecommendation(
            name="header_separation",
            recommended_value=0,
            option_help=_("Add extra spacing below the header. Default is %default pt."),
        ),
        OptionRecommendation(
            name="minimum_indent",
            recommended_value=0,
            option_help=_(
                "Minimum paragraph indent (the indent of the first line " "of a paragraph) in pts. Default: %default"
            ),
        ),
        OptionRecommendation(
        ...
        OptionRecommendation(
            ...
            option_help=_("Multiply the size of text in rendered tables by this " "factor. Default is %default"),
        ),
        OptionRecommendation(
            name="serif_family",
            recommended_value=None,
            option_help=_("The serif family of fonts to embed"),
        ),
        OptionRecommendation(
            name="sans_family",
            recommended_value=None,
            option_help=_("The sans-serif family of fonts to embed"),
        ),
        OptionRecommendation(
            name="mono_family",
            recommended_value=None,
            option_help=_("The monospace family of fonts to embed"),
        ),
    }
    ...
    def convert_images(self, pages, opts, wide):
        ...
        for page in pages:
            image_stream = ImageStream(page)
            _page = book.create_page()
            _page.append(
                ImageBlock(
                    refstream=image_stream,
                    blockwidth=width,
                    blockheight=height,
                    xsize=width,
                    ysize=height,
