...
class HeuristicProcessor(object):
    def __init__(self, extra_opts=None, log=None):
        ...
        self.blankreg = re.compile(
            r"\s*(?P<openline><p(?!\sclass=\"(softbreak|whitespace)\")[^>]*>)\s*" r"(?P<closeline></p>)",
            re.IGNORECASE,
        )
        ...
        self.line_open = (
            r"<(?P<outer>p|div)[^>]*>\s*(<(?P<inner1>font|span|[ibu])[^>]*>)?\s*(<(?P<inner2>"
            r"font|span|[ibu])[^>]*>)?\s*(<(?P<inner3>font|span|[ibu])[^>]*>)?\s*"
        )
        ...
        self.scene_break_open = (
            '<p class="scenebreak" style="text-align:center; text-indent:0%; margin-top:1em; '
            'margin-bottom:1em; page-break-before:avoid">'
        )
    ...
    def chapter_head(self, match):
        ...
        if not title:
            ...
            self.log.debug("marked " + six_unicode(self.html_preprocess_sections) + " chapters. - " + six_unicode(chap))
            return "<h2>" + chap + "</h2>\n"
        else:
            ...
            self.log.debug(
                "marked "
                + six_unicode(self.html_preprocess_sections)
                + " chapters & titles. - "
                + six_unicode(chap)
                + ", "
                + six_unicode(title)
    ...
    def chapter_break(self, match):
        ...
        self.log.debug(
            "marked "
            + six_unicode(self.html_preprocess_sections)
            + " section markers based on punctuation. - "
            + six_unicode(chap)
        )
    ...
    def markup_italicis(self, html):
        ...
        for pat in italicize_style_pats:
            for match in re.finditer(pat, search_text):
                ital_string = six_unicode(match.group("words"))
                # self.log.debug("italicising "+unicode(match.group(0))+"    with <i>"+ital_string+"</i>")
                try:
                    html = re.sub(
                        re.escape(six_unicode(match.group(0))),
                        "<i>%s</i>" % ital_string,
                        html,
                    )
    ...
    def markup_chapters(self, html, wordcount, blanks_between_paragraphs):
        ...
        self.log.debug("minimum chapters required are: " + six_unicode(self.min_chapters))
        heading = re.compile("<h[1-3][^>]*>", re.IGNORECASE)
        self.html_preprocess_sections = len(heading.findall(html))
        self.log.debug("found " + six_unicode(self.html_preprocess_sections) + " pre-existing headings")

        # Build the Regular Expressions in pieces
        init_lookahead = "(?=<(p|div))"
        chapter_line_open = self.line_open
        title_line_open = (
            r"<(?P<outer2>p|div)[^>]*>\s*(<(?P<inner4>font|span|[ibu])[^>]*>)?\s*"
            r"(<(?P<inner5>font|span|[ibu])[^>]*>)?\s*(<(?P<inner6>font|span|[ibu])[^>]*>)?\s*"
        )
        ...
        chapter_types = [
            [
                r"[^'\"]?(Introduction|Synopsis|Acknowledgements|Epilogue|CHAPTER|Kapitel|Volume\b|Prologue|Book\b|"
                r"Part\b|Dedication|Preface)\s*([\d\w-]+\:?\'?\s*){0,5}",
                True,
                True,
                True,
                False,
            ...
            [
                r"<b[^>]*>\s*(<span[^>]*>)?\s*(?!([*#•=]+\s*)+)(\s*(?=[\d.\w#\-*\s]+<)([\d.\w#-*]+\s*){1,5}\s*)(?!\.)"
                r"(</span>)?\s*</b>",
                True,
                True,
                True,
        ...
        def recurse_patterns(local_html, analyze):
            ...
            for [
                ...
                if self.html_preprocess_sections >= self.min_chapters:
                    break
                full_chapter_line = (
                    chapter_line_open + chapter_header_open + chapter_type + chapter_header_close + chapter_line_close
                )
                ...
                if not analyze:
                    self.log.debug("Marked " + six_unicode(self.html_preprocess_sections) + " headings, " + log_message)
                ...
                if analyze:
                    hits = len(chapdetect.findall(local_html))
                    if hits:
                        chapdetect.sub(self.analyze_title_matches, local_html)
                        if float(self.chapters_with_title) / float(hits) > 0.5:
                            title_req = True
                            strict_title = False
                        self.log.debug(
                            six_unicode(type_name)
                            + " had "
                            + six_unicode(hits)
                            + " hits - "
                            + six_unicode(self.chapters_no_title)
                            + " chapters with no title, "
                            + six_unicode(self.chapters_with_title)
                            + " chapters with titles, "
                            + six_unicode(float(self.chapters_with_title) / float(hits))
                            + " percent. "
                        )
                        if type_name == "common":
                    ...
                    local_html = chapdetect.sub(self.chapter_head, local_html)
            return local_html
        ...
        self.log.debug(
            "Total wordcount is: "
            + six_unicode(wordcount)
            + ", Average words per section is: "
            + six_unicode(words_per_chptr)
            + ", Marked up "
            + six_unicode(self.html_preprocess_sections)
            + " chapters"
        )
    ...
    def punctuation_unwrap(self, length, content, format):
        ...
        lookahead = rf"(?<=.{{{six_unicode(length)}}}([a-zäëïöüàèìòùáćéíĺóŕńśúýâêîôûçąężıãõñæøþðßěľščťžňďřů,:)IAß]|(?<!&\w{{4}});))"
        #           (?<!&\w{4}); is a semicolon not part of an entity
        em_en_lookahead = "(?<=.{" + six_unicode(length) + "}[\u2013\u2014])"
        soft_hyphen = "\xad"
        line_ending = r"\s*(?P<style_close></(span|[iub])>)?\s*(</(p|div)>)?"
        blanklines = (
            r"\s*(?P<up2threeblanks><(p|span|div)[^>]*>\s*(<(p|span|div)[^>]*>\s*"
            r"</(span|p|div)>\s*)</(span|p|div)>\s*){0,3}\s*"
        )
    ...
    def fix_nbsp_indents(self, html):
        ...
        try:
            # Python 2
            txtindent_re = txtindent_re.decode("raw_unicode_escape")
        except AttributeError:
            # Python 3
            pass
        txtindent = re.compile(txtindent_re, re.IGNORECASE)
    ...
    def cleanup_markup(self, html):
        ...
        try:
            ...
            html_non_break_re_sub_pat = html_non_break_re_sub_pat.decode("raw_unicode_escape")
        except (SyntaxError, AttributeError):
            # Python 3 - nothing need be done
            pass
        html = re.sub(html_non_break_re_sub_pat, " ", html)
        # Get rid of various common microsoft specific tags which can cause issues later
        # Get rid of empty <o:p> tags to simplify other processing
        html_micro_re_sub_pat = r"\s*<o:p>\s*</o:p>"
        try:
            # Python 2- string originally had ur prefix - has been tested identical under py2
            html_micro_re_sub_pat = html_micro_re_sub_pat.decode("raw_unicode_escape")
        except (SyntaxError, AttributeError):
            # Python 3 - nothing need be done
            pass
        html = re.sub(html_micro_re_sub_pat, " ", html)
        ...
        html = re.sub(
            r"\s*<(font|[ibu]|em|strong)[^>]*>\s*(<(font|[ibu]|em|strong)[^>]*>\s*"
            r"</(font|[ibu]|em|strong)>\s*){0,2}\s*</(font|[ibu]|em|strong)>",
            " ",
            html,
        )
        html = re.sub(r"\s*<span[^>]*>\s*(<span[^>]>\s*</span>){0,2}\s*</span>\s*", " ", html)
    ...
    def analyze_blanks(self, html):
        ...
        if len(lines) > 1:
            self.log.debug(
                "There are "
                + six_unicode(len(blanklines))
                + " blank lines. "
                + six_unicode(float(len(blanklines)) / float(len(lines)))
                + " percent blank"
            )
    ...
    def detect_whitespace(self, html):
        ...
        blanks_around_scene_breaks = re.compile(
            ...
            r'(?P<content><p class="scenebreak"[^>]*>.*?</p>)'
            r"(?P<endparas>\s*(<(p|div)[^>]*>\s*</(p|div)>\s*){1,})?",
            re.IGNORECASE | re.DOTALL,
        )

        blanks_n_nopunct = re.compile(
            r"(?P<initparas>(<p[^>]*>\s*</p>\s*){1,}\s*)?<p[^>]*>\s*(<(span|[ibu]|em|strong|font)[^>]*>\s*)*"
            r".{1,100}?[^\W](</(span|[ibu]|em|strong|font)>\s*)*"
            r"</p>(?P<endparas>\s*(<p[^>]*>\s*</p>\s*){1,})?",
            re.IGNORECASE | re.DOTALL,
        )
        ...
        def merge_header_whitespace(match):
            ...
            if initblanks is not None:
                top_margin = "margin-top:" + six_unicode(len(self.single_blank.findall(initblanks))) + "em;"
            if endblanks is not None:
                bottom_margin = "margin-bottom:" + six_unicode(len(self.single_blank.findall(endblanks))) + "em;"
            ...
            if initblanks is None and endblanks is None:
                ...
                content = re.sub(
                    r"(?i)<h(?P<hnum>\d+)[^>]*>",
                    "\n\n<h" + r"\g<hnum>" + ' style="' + top_margin + bottom_margin + '">',
                    content,
                )
            return content
    ...
    def detect_scene_breaks(self, html):
        scene_break_regex = (
            ...
            + r"<))(?P<break>((?P<break_char>((?!\s)\W))\s*(?P=break_char)?)+)\s*"
            + self.line_close
        )
    ...
    def markup_user_break(self, replacement_break):
        ...
        hr_open = (
            ...
            'margin-bottom:1.5em; page-break-before:avoid">'
        )
                ...
                if replacement_break.find("width") != -1:
                    try:
                        ...
                        replacement_break = re.sub(
                            r"(?i)(width=\d+\%?|width:\s*\d+(\%|px|pt|em)?;?)",
                            "",
                            replacement_break,
                        )
                        divpercent = (100 - width) / 2
                        hr_open = re.sub("45", six_unicode(divpercent), hr_open)
    ...
    def abbyy_processor(self, html):
        ...
        def convert_styles(match):
            ...
            if image:
                ...
                for style, setting in styles:
                    ...
                    if style == "text-indent":
                        ...
                        if 9 < setting < 14:
                            ...
                            text_indent = style + ":" + six_unicode(setting) + "pt;"
                    if style == "padding":
                        ...
                        if debugabby:
                            self.log.debug("\n\n******\n")
                            self.log.debug("padding top is: " + six_unicode(setting[0]))
                            self.log.debug("padding right is:" + six_unicode(setting[1]))
                            self.log.debug("padding bottom is: " + six_unicode(setting[2]))
                            self.log.debug("padding left is: " + six_unicode(setting[3]))

                # print ("text-align is: "+unicode(text_align))
                # print ("\n***\nline is:\n     "+unicode(match.group(0))+'\n')
                if debugabby:
                    # print ("this line is a paragraph = " + unicode(is_paragraph) + ", previous line was " +
                    #        unicode(self.previous_was_paragraph))
                ...
                # print("is_paragraph is "+unicode(is_paragraph)+", previous_was_paragraph is "+
                #       unicode(self.previous_was_paragraph))
    ...
    def __call__(self, html):
        ...
        html_format = self.analyze_line_endings(html)

        # Check Line histogram to determine if the document uses hard line breaks, If 50% or
        # more of the lines break in the same region of the document then unwrapping is required
        docanalysis = DocAnalysis(html_format, html)
        hardbreaks = docanalysis.line_histogram(0.50)
        self.log.debug("Hard line breaks check returned " + six_unicode(hardbreaks))

        # Calculate Length
        unwrap_factor = getattr(self.extra_opts, "html_unwrap_factor", 0.4)
        length = docanalysis.line_length(unwrap_factor)
        self.log.debug("Median line length is " + six_unicode(length) + ", calculated with " + html_format + " format")

        # ------- Unwrap lines ------- #
        if getattr(self.extra_opts, "unwrap_lines", False):
            # only go through unwrapping code if the histogram shows unwrapping is required or if the user decreased
            # the default unwrap_factor
        ...
        if self.html_preprocess_sections < self.min_chapters and getattr(
            ...
            self.log.debug(
                ...
                " currently have " + six_unicode(self.html_preprocess_sections)
            )
            chapdetect3 = re.compile(
                r"<(?P<styles>(p|div)[^>]*)>\s*(?P<section>(<span[^>]*>)?\s*(?!([\W]+\s*)+)"
                r"(<[ibu][^>]*>){0,2}\s*(<span[^>]*>)?\s*(<[ibu][^>]*>){0,2}\s*"
                r"(<span[^>]*>)?\s*.?(?=[a-z#\-*\s]+<)([a-z#-*]+\s*){1,5}\s*\s*(</span>)?"
                r"(</[ibu]>){0,2}\s*(</span>)?\s*(</[ibu]>){0,2}\s*(</span>)?\s*</(p|div)>)",
                re.IGNORECASE,
            )
        ...
        if getattr(self.extra_opts, "renumber_headings", False):
            ...
            doubleheading = re.compile(
                r"(?P<firsthead><h(1|2)[^>]*>.+?</h(1|2)>\s*(<(?!h\d)[^>]*>\s*)*)<h(1|2)"
                r"(?P<secondhead>[^>]*>.+?)</h(1|2)>",
                re.IGNORECASE,
            )
            html = doubleheading.sub(r"\g<firsthead>" + "\n<h3" + r"\g<secondhead>" + "</h3>", html)
