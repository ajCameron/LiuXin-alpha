...
class DriverTreeMixinAPI(abc.ABC):
    ...
    @abc.abstractmethod
    def _get_linear_index_of_columns(self, start_row: dict[str, Any], display_column: str) -> list[str]:
        """
    ...
    @abc.abstractmethod
    def get_root_series(self, start_row: dict[str, Any]) -> dict[str, Any] :
        """
        Return the root row for a series from a given row in the tree.

        :param start_row:
        :return:
        """

    # Todo: This should - probably - be in marcos
    @abc.abstractmethod
    ...
    @abc.abstractmethod
    def direct_set_tree_ids(self, table: str) -> None:
        """
    ...
    @abc.abstractmethod
    def tree_aggregator(self, table: str, table_display_column: str, table_row_id: int) -> str:
        """
