...
class DatabaseAPI(abc.ABC):
    ...
    @abc.abstractmethod
    def get_interlink_values(self, target_row: "RowAPI", secondary_column: str) -> list[Any]:
    ...
    def dupe_interlinks(
        ...
        force_priority: Optional[Union[int, float]] = None,
    ) -> None:
    ...
    @abc.abstractmethod
    def update_interlink_priority(self, primary_row: "RowAPI", secondary_table: str, ordered_ids: list[int]) -> None:
    ...
    @abc.abstractmethod
    def get_intralinked_rows(self, primary_row: "RowAPI", secondary_row: "RowAPI") -> list["RowAPI"]:
        """Return rows intralinked to primary_row/secondary_row (direction depends on driver_wrapper policy)."""

    @abc.abstractmethod
    def unlinked_intralink(self, primary_row: "RowAPI", secondary_row: "RowAPI") -> None:
    ...
    @abc.abstractmethod
    def get_root_series(self, start_row: "RowAPI") -> list["RowAPI"]:
    ...
    @abc.abstractmethod
    def get_all_tree_rows(self, start_row: "RowAPI", back_iterate: bool = False) -> list["RowAPI"]:
    ...
    @abc.abstractmethod
    def search_tree(self, root_row: "RowAPI", for_ids: Iterable[int]) -> list["RowAPI"]:
        """Search a tree for particular row ids."""

    @abc.abstractmethod
    def nest_rows(self, parent_row: "RowAPI", child_rows: Iterable["RowAPI"]) -> None:
