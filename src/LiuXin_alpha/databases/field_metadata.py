...
class CalibreFieldMetadata(dict):
    ...
    def set_field_record_index_from_field_map(self, field_map):
        """
        Used during startup - takes a field map - a dictionary keyed with the label of a field and valued with the
        index (position) of that field - loads that data into this class.
        :param field_map:
        :return:
        """
        for (
    ...
    def set_field_record_index(self, label, index, prefer_custom=False):
        """
        Set the label for a given index - if there is a clash between a builtin field and a custom column label, by
        default prefer the builtin field over the custom column field.
        :param label:
        :param index:
        :param prefer_custom:
        :return:
...
def fm_from_dict(src):
    """
    Deserialize a dictionary into a complete field metadata object.
    :param src:
    :return:
    """
    ans = FieldMetadata()
