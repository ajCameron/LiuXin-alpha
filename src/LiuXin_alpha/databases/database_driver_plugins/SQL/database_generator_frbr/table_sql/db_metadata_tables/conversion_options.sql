...
CREATE TABLE IF NOT EXISTS `conversion_options` (
  ...
  `conversion_option_created_timestamp` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `conversion_option_created_timestamp_ep_k` INTEGER NOT NULL DEFAULT (CAST((julianday('now') - 2440587.5) * 86400000 AS INTEGER)),

  `conversion_option_modified_timestamp` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
