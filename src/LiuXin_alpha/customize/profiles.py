...
from LiuXin.customize import Plugin as BasePlugin
...
class IOProfilePluginBase(BasePlugin):
    """
...
# Input profiles {{{
class InputProfile(IOProfilePluginBase):
    """
    ...
    description = _(
        "This profile tries to provide sane defaults and is useful if you know nothing about the input document."
    )
    # Todo: Validation for short name, e.t.c
...
# input_profiles.sort(lambda x, y: cmp(x.name.lower(), y.name.lower()))
sorted(input_profiles, key=lambda x: x.name.lower())

# }}}


class OutputProfile(IOProfilePluginBase):
    """
...
sorted(output_profiles, key=lambda x: x.name.lower())
