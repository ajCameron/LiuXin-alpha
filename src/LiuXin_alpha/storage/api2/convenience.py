"""Safe convenience operations implemented from the raw store primitives."""
...
from LiuXin_alpha.storage.api2.models import Digest, FileInfo, Location, WriteMode
...
def try_stat(store: FileStore, location: Location) -> FileInfo | None:
    ...
    Example:
        >>> try_stat(store, Location("primary", "missing")) is None  # doctest: +SKIP
        True
    """
    try:
...
def exists(store: FileStore, location: Location) -> bool:
    ...
    Example:
        >>> exists(store, Location("primary", "objects/42"))  # doctest: +SKIP
        True
    """
...
def get(
    ...
    *,
    ...
    Example:
        >>> stream = get(store, Location("primary", "objects/42"))  # doctest: +SKIP
    """
...
def read_bytes(
    ...
    *,
    ...
    Example:
        >>> read_bytes(store, Location("primary", "objects/42"), length=4)  # doctest: +SKIP
        b'book'
    """
...
def put(
    ...
    *,
    ...
    Example:
        ...
        ...     store, Location("primary", "objects/42"), io.BytesIO(b"book"),
        ...     expected_size=4,
        ... )
    """
    if chunk_size < 1:
...
def write_bytes(
    ...
    *,
    ...
    Example:
        ...
        ...     store, Location("primary", "objects/42"), b"book",
        ... )
    """
    return put(
        store,
...
def compute_digest(
    ...
    *,
    ...
    Example:
        ...
        ...     store, Location("primary", "objects/42"), "sha256",
        ... )
    """
    if chunk_size < 1:
        raise ValueError("chunk_size must be at least one byte.")
    if store.capabilities.authoritative_digest and isinstance(store, DigestingStore):
