...
class Bookmark:  # {{{
    """
    A simple class fetching bookmark data kobo-specific
    """
    ...
    def get_bookmark_data(self):
        ...
        with closing(sqlite.connect(self.db_path)) as connection:
            ...
            kepub_chapter_query = "SELECT Title, volumeIndex " "FROM content " "WHERE ContentID LIKE ? "
            bookmark_query = (
            ...
            cursor.execute(
                "select datelastread, ___PercentRead from content " "where bookid is Null and " "contentid = ?",
                t,
            )
            for row in cursor:
