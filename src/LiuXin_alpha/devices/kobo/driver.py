...
__license__ = "GPL v3"
__copyright__ = (
    "2010-2012, Timothy Legge <timlegge@gmail.com>, Kovid Goyal <kovid@kovidgoyal.net> "
    "and David Forrester <davidfor@internode.on.net>"
)
...
from LiuXin.devices.kobo.books import KTCollectionsBookList
from LiuXin.devices.kobo.books import Book
from LiuXin.devices.kobo.books import ImageWrapper
from LiuXin.devices.mime import mime_type_ext
from LiuXin.devices.usbms.books import BookList
from LiuXin.devices.usbms.books import CollectionsBookList
from LiuXin.devices.usbms.driver import USBMS, debug_print

from LiuXin.utils.calibre import fsync
...
class KOBO(USBMS):
    ...
    EXTRA_CUSTOMIZATION_MESSAGE = [
        ...
        + ":::"
        + _(
            "Kobo now shows recommendations on the device.  In some case these have "
    ...
    def books(self, oncard=None, end_session=True):
        from LiuXin.metadata.meta import path_to_ext
            ...
            try:
                ...
                if idx is not None:
                    ...
                    if ImageID is not None:
                        imagename = self.normalize_path(
                            self._main_prefix + ".kobo/images/" + ImageID + " - NickelBookCover.parsed"
                        )
                        if not os.path.exists(imagename):
                            # Try the Touch version if the image does not exist
                            imagename = self.normalize_path(
                                self._main_prefix + ".kobo/images/" + ImageID + " - N3_LIBRARY_FULL.parsed"
                            )

                        # print "Image name Normalized: " + imagename
                        if not os.path.exists(imagename):
                            debug_print(
                                "Strange - The image name does not exist - title: ",
                                title,
                            )
                    ...
                    if lpath in playlist_map and playlist_map[lpath] not in bl[idx].device_collections:
        ...
        with closing(sqlite.connect(self.normalize_path(self._main_prefix + ".kobo/KoboReader.sqlite"))) as connection:

            # return bytestrings if the content cannot the decoded as unicode
            connection.text_factory = lambda x: six_unicode(x, "utf-8", "ignore")
            ...
            debug_print("DatabasePing Version: ", self.dbversion)

            opts = self.settings()
            if self.dbversion >= 33:
                query = (
                    "select Title, Attribution, DateCreated, ContentID, MimeType, ContentType, "
                    ...
                    expiry=" and ContentType = 6)"
                    if opts.extra_customization[self.OPT_SHOW_EXPIRED_BOOK_RECORDS]
                    else ")",
                    previews=" and Accessibility <> 6"
                    if opts.extra_customization[self.OPT_SHOW_PREVIEWS] == False
                    else "",
                    recomendations=" and IsDownloaded in ('true', 1)"
                    if opts.extra_customization[self.OPT_SHOW_RECOMMENDATIONS] == False
                    else "",
                )
                ...
                query = (
                    ...
                    "BookID is Null and not ((___ExpirationStatus=3 or ___ExpirationStatus is Null) %(expiry)s"
                ) % dict(
                    expiry=" and ContentType = 6)"
                    if opts.extra_customization[self.OPT_SHOW_EXPIRED_BOOK_RECORDS]
                    else ")"
    ...
    def delete_via_sql(self, ContentID, ContentType):
        ...
        with closing(sqlite.connect(self.normalize_path(self._main_prefix + ".kobo/KoboReader.sqlite"))) as connection:
            ...
            if ContentType == 6 and self.dbversion < 8:
                ...
                cursor.execute(
                    "delete from shortcover_page where shortcoverid in (select ContentID from content where BookID = ?)",
                    t,
                )
            ...
            # The next reader_server sync should remove the row
            cursor.execute("delete from content where BookID = ?", t)
            if ContentType == 6:
                try:
                    cursor.execute(
                        "update content set ReadStatus=0, FirstTimeReading = 'true', ___PercentRead=0, ___ExpirationStatus=3 "
                    ...
                    try:
                        cursor.execute(
                            "update content set ReadStatus=0, FirstTimeReading = 'true', ___PercentRead=0 "
                            "where BookID is Null and ContentID =?",
                            t,
                        )
                    except Exception as e:
    ...
    def delete_images(self, ImageID, book_path):
        if ImageID != None:
            ...
            file_endings = (
                " - iPhoneThumbnail.parsed",
                " - bbMediumGridList.parsed",
    ...
    def delete_books(self, paths, end_session=True):
        ...
        for i, path in enumerate(paths):
            ...
            ContentType = (
                self.get_content_type_from_extension(extension)
                if extension != ""
                else self.get_content_type_from_path(path)
            )
    ...
    def remove_books_from_metadata(self, paths, booklists):
        ...
        for i, path in enumerate(paths):
            self.report_progress(
                (i + 1) / float(len(paths)),
                _("Removing books from device metadata listing..."),
            )
            for bl in booklists:
                for book in bl:
    ...
    def modify_database_check(self, function):
        ...
        if self.dbversion > self.supported_dbversion:
            ...
            if not opts.extra_customization[self.OPT_SUPPORT_NEWER_FIRMWARE]:
                ...
                self.report_progress(1.0, _("Removing books from device..."))
                from LiuXin.devices.errors import UserFeedback

                raise UserFeedback(
                    _("Kobo database version unsupported - See details"),
                    _(
                        "Your Kobo is running an updated firmware/database version."
    ...
    def get_file(self, path, *args, **kwargs):
        ...
        if extension == ".kobo":
            ...
            raise UserFeedback(
                _("Not Implemented"),
                _(
                    '".kobo" files do not exist on the device as books '
    ...
    def book_from_path(cls, prefix, lpath, title, authors, mime, date, ContentType, ImageID):
        ...
        if mi is None:
            mi = MetaInformation(os.path.splitext(os.path.basename(lpath))[0], [_("Unknown")])
    ...
    def reset_readstatus(self, connection, oncard):
        ...
        try:
            ...
            debug_print("    DatabasePing Exception:  Unable to reset ReadStatus list")
            raise
        else:
            connection.commit()
    ...
    def set_readstatus(self, connection, ContentID, ReadStatus):
        ...
        try:
            cursor.execute(
                "update content set ReadStatus=?,FirstTimeReading='false',DateLastRead=? where BookID is Null and ContentID = ?",
                t,
            )
        except:
            debug_print("    DatabasePing Exception:  Unable update ReadStatus")
            raise
        else:
            connection.commit()
    ...
    def reset_favouritesindex(self, connection, oncard):
        ...
        if oncard == "carda":
            ...
            query = (
                "update content set FavouritesIndex=-1 where BookID is Null and ContentID not like 'file:///mnt/sd/%'"
            )
        ...
        try:
            ...
            debug_print("    DatabasePing Exception:  Unable to reset Shortlist list")
            if "no such column" not in str(e):
                raise
        else:
            connection.commit()
    ...
    def set_favouritesindex(self, connection, ContentID):
        ...
        try:
            cursor.execute(
                "update content set FavouritesIndex=1 where BookID is Null and ContentID = ?",
                t,
            )
        except Exception as e:
            debug_print("    DatabasePing Exception:  Unable set book as Shortlist")
            if "no such column" not in str(e):
                raise
        else:
            connection.commit()
                    ...
                    if category in supportedcategories:
                        ...
                        for book in books:
                            ...
                            ContentType = (
                                self.get_content_type_from_extension(extension)
                                if extension != ""
                                else self.get_content_type_from_path(book.path)
                            )
        ...
        if metadata.cover:
            ...
            if os.path.exists(cover):
                ...
                ContentType = (
                    self.get_content_type_from_extension(extension)
                    if extension != ""
                    else self.get_content_type_from_path(filepath)
                )
                ...
                with closing(
                    sqlite.connect(self.normalize_path(self._main_prefix + ".kobo/KoboReader.sqlite"))
                ) as connection:
                    ...
                    t = (ContentID,)
                    cursor.execute(
                        "select ImageId from Content where BookID is Null and ContentID = ?",
                        t,
                    )
                    result = cursor.fetchone()
                    if result is None:
                ...
                if ImageID != None:
                    ...
                    file_endings = {
                        " - iPhoneThumbnail.parsed": (103, 150),
    ...
    def get_annotations(self, path_map):
        from LiuXin.devices.kobo.bookmark import Bookmark
        ...
        for id in path_map:
            ...
            ContentType = (
                self.get_content_type_from_extension(extension)
                if extension != ""
                else self.get_content_type_from_path(path_map[id])
            )
    ...
    def generate_annotation_html(self, bookmark):
        import calendar
        from LiuXin.file_formats.BeautifulSoup import (
            BeautifulSoup,
            Tag,
            NavigableString,
        )
        ...
        if bookmark.last_read is not None:
            try:
                last_read = time.strftime(
                    "%Y-%m-%d %H:%M:%S",
                    time.localtime(calendar.timegm(time.strptime(bookmark.last_read, "%Y-%m-%dT%H:%M:%S"))),
                )
            except:
                try:
                    last_read = time.strftime(
                        "%Y-%m-%d %H:%M:%S",
                        time.localtime(calendar.timegm(time.strptime(bookmark.last_read, "%Y-%m-%dT%H:%M:%S.%f"))),
                    )
                except:
                    last_read = time.strftime(
                        "%Y-%m-%d %H:%M:%S",
                        time.localtime(calendar.timegm(time.strptime(bookmark.last_read, "%Y-%m-%dT%H:%M:%SZ"))),
                    )
        else:
        ...
        if bookmark.user_notes:
            ...
            for location in sorted(user_notes):
                if user_notes[location]["type"] == "Bookmark":
                    annotations.append(
                        _(
                            "<b>Chapter %(chapter)d:</b> %(chapter_title)s<br /><b>%(typ)s</b><br /><b>Chapter Progress:</b> %(chapter_progress)s%%<br />%(annotation)s<br /><hr />"
                        )
                        % dict(
                            ...
                            if user_notes[location]["annotation"] is not None
                            else "",
                        )
                    )
                    ...
                    annotations.append(
                        _(
                            "<b>Chapter %(chapter)d:</b> %(chapter_title)s<br /><b>%(typ)s</b><br /><b>Chapter Progress:</b> %(chapter_progress)s%%<br /><b>Highlight:</b> %(text)s<br /><hr />"
                        )
                        % dict(
                            chapter=user_notes[location]["chapter"],
                            dl=user_notes[location]["displayed_location"],
                    ...
                    annotations.append(
                        _(
                            "<b>Chapter %(chapter)d:</b> %(chapter_title)s<br /><b>%(typ)s</b><br /><b>Chapter Progress:</b> %(chapter_progress)s%%<br /><b>Highlight:</b> %(text)s<br /><b>Notes:</b> %(annotation)s<br /><hr />"
                        )
                        % dict(
                            chapter=user_notes[location]["chapter"],
                            dl=user_notes[location]["displayed_location"],
    ...
    def add_annotation_to_library(self, db, db_id, annotation):
        from LiuXin.file_formats.BeautifulSoup import Tag
        ...
        if bm.type == "kobo_bookmark":
            ...
            if mi.comments:
                ...
                mi.comments += six_unicode(user_notes_soup.prettify())
            else:
                mi.comments = six_unicode(user_notes_soup.prettify())
...
class KOBOTOUCH(KOBO):
    ...
    description = (
        "Communicate with the Kobo Touch, Glo, Mini and Aura HD ereaders. Based on the existing Kobo driver by %s."
        % (KOBO.author)
    )
    ...
    min_fwversion_images_tree = (
        2,
        9,
        0,
    )  # Cover images stored in tree under .kobo-images
    ...
    EXTRA_CUSTOMIZATION_MESSAGE = [
        _(
            "The Kobo from firmware V2.0.0 supports bookshelves."
            " These are created on the Kobo. " + "Specify a tags type column for automatic management."
        ),
        ...
        + ":::"
        + _(
            "Delete any empty bookshelves from the Kobo when syncing is finished. This is only for firmware V2.0.0 or later."
        ),
        ...
        + ":::"
        + _(
            "When uploading covers, do not change the aspect ratio when resizing for the device."
        ...
        + ":::"
        + _(
            "Kobo shows recommendations on the device.  In some cases these have "
            "files but in other cases they are just pointers to the web site to buy. "
            "Enable if you wish to see/delete them."
        ),
        ...
        + ":::"
        + _(
            "The book lists on the Kobo devices can display series information. "
        ...
        + ":::"
        + _(
            "This allows addition of user CSS rules and removal of some CSS. "
        ...
        + six_unicode(supported_dbversion),
        _("Title to test when debugging")
        + ":::"
        + _(
            "Part of title of a book that can be used when doing some tests for debugging. "
    ...
    PRODUCT_ID = (
        AURA_PRODUCT_ID + AURA_HD_PRODUCT_ID + AURA_H2O_PRODUCT_ID + GLO_PRODUCT_ID + MINI_PRODUCT_ID + TOUCH_PRODUCT_ID
    )
    ...
    COVER_FILE_ENDINGS = {
        ...
        " - AndroidBookLoadTablet_Aspect.parsed": [
            (355, 473),
            82,
            200,
            False,
    ...
    def books(self, oncard=None, end_session=True):
        ...
        from LiuXin.file_formats.metadata.meta import path_to_ext
        ...
        debug_print("Version of firmware:", self.fwversion, "Has kepubs:", self.has_kepubs)
        debug_print(
            "Firmware supports cover image tree:",
            self.fwversion >= self.min_fwversion_images_tree,
        )
        ...
        debug_print(
            "KoboTouch:books - prefs['manage_device_metadata']=",
        ...
        def update_booklist(
            ...
            if show_debug:
                debug_print(
                    "KoboTouch:update_booklist - title='%s'" % title,
            ...
            try:
                ...
                if idx is not None:  # and not (accessibility == 1 and isdownloaded == 'false'):
                    if show_debug:
                        ...
                        debug_print(
                            "KoboTouch:update_booklist - bl[idx].device_collections=",
                            bl[idx].device_collections,
                        )
                        debug_print("KoboTouch:update_booklist - playlist_map=", playlist_map)
                        debug_print("KoboTouch:update_booklist - bookshelves=", bookshelves)
                        debug_print(
                            "KoboTouch:update_booklist - kobo_collections=",
                            kobo_collections,
                        )
                        ...
                        debug_print(
                            "KoboTouch:update_booklist - application_id=",
                            bl[idx].application_id,
                        )
                    bl_cache[lpath] = None
                    ...
                    if show_debug:
                        debug_print(
                            "KoboTouch:update_booklist - updated bl[idx].device_collections=",
                            bl[idx].device_collections,
                        )
                        debug_print(
                            "KoboTouch:update_booklist - playlist_map=",
                            playlist_map,
                            "changed=",
                            changed,
                        )
                    ...
                    try:
                        if os.path.exists(self.normalize_path(os.path.join(prefix, lpath))):
                            ...
                            if show_debug:
                                debug_print(
                                    'KoboTouch:update_booklist - book file does not exist. ContentID="%s"' % ContentID
                                )
        ...
        def get_bookshelvesforbook(connection, ContentID):
            ...
            )  # This should never be nulll, but it is protection against an error cause by a sync to the Kobo reader_server
            values = (ContentID,)
        ...
        with closing(sqlite.connect(self.device_database_path())) as connection:
            ...
            result = cursor.fetchone()
            self.dbversion = result[0]
            debug_print("DatabasePing Version=%d" % self.dbversion)
            ...
            if self.supports_kobo_archive():
                where_clause = (
                    ...
                    expiry=""
                    if opts.extra_customization[self.OPT_SHOW_ARCHIVED_BOOK_RECORDS]
                    else "and IsDownloaded in ('true', 1)",
                    previews=" or (Accessibility in (6) and ___UserID <> '')"
                    if opts.extra_customization[self.OPT_SHOW_PREVIEWS]
                    else "",
                    recomendations=" or (Accessibility in (-1, 4, 6) and ___UserId = '')"
                    if opts.extra_customization[self.OPT_SHOW_RECOMMENDATIONS]
                    else "",
                )
            ...
            if self.has_externalid():
                card_condition = (
                    " AND (externalId IS NOT NULL AND externalId <> '' OR contentId LIKE 'file:///mnt/sd/%')"
                    if oncard == "carda"
                    else " AND (externalId IS NULL OR externalId = '') AND contentId NOT LIKE 'file:///mnt/sd/%'"
                )
            else:
                card_condition = (
                    " AND contentId LIKE 'file:///mnt/sd/%'"
                    if oncard == "carda"
                    else " AND contentId NOT LIKE'file:///mnt/sd/%'"
                )
            ...
            for i, row in enumerate(cursor):
                ...
                if (
                    not hasattr(row[3], "startswith")
                    or row[3].lower().startswith("file:///usr/local/kobo/help/")
                    or row[3].lower().startswith("/usr/local/kobo/help/")
                ):
                    # These are internal to the Kobo device and do not exist
                    continue
        ...
        if self.modifying_css():
            ...
            if os.path.exists(extra_css_path):
                ...
                try:
                    ...
                    debug_print(
                        "KoboTouch:get_extra_css: Using extra CSS in {0} ({1} rules)".format(
                            extra_css_path, len(extra_sheet.cssRules)
                        )
                    )
                    if len(extra_sheet.cssRules) == 0:
                        debug_print(
                            "KoboTouch:get_extra_css: Extra CSS file has no valid rules. CSS will not be modified."
                        )
                        extra_sheet = None
                except Exception as e:
    ...
    def upload_books(self, files, names, on_card=None, end_session=True, metadata=None):
        ...
        if self.modifying_epub():
            ...
            for file, n, mi in zip(files, names, metadata):
                debug_print(
                    "KoboTouch:upload_books: Processing book: {0} by {1}".format(mi.title, " and ".join(mi.authors))
                )
                debug_print("KoboTouch:upload_books: file=%s, name=%s" % (file, n))
                self.report_progress(
                    i / float(len(files)),
                    "Processing book: {0} by {1}".format(mi.title, " and ".join(mi.authors)),
                )
        ...
        if self.dbversion >= 53:
            ...
            try:
                with closing(
                    ...
                    cursor = connection.cursor()
                    cleanup_query = (
                        "DELETE FROM content WHERE ContentID = ? AND Accessibility = 1 AND IsDownloaded = 'false'"
                    )
    ...
    def _modify_epub(self, file, metadata, container=None):
        ...
        if not container:
            ...
            try:
                ...
                debug_print(
                    "KoboTouch:_modify_epub: exception from get_container {0} - {1}".format(
                        metadata.author_sort, metadata.title
                    )
                )
        ...
        from LiuXin.file_formats.oeb.base import OEB_STYLES
            ...
            if mt in OEB_STYLES:
                ...
                if (
                    len(
                        [
                            r
                            for r in self.extra_sheet
                            if r.type == r.STYLE_RULE and (r.style["widows"] or r.style["orphans"])
                        ]
                    )
                    > 0
                ):
                    widow_orphan_rules = [
                        r for r in newsheet if r.type == r.STYLE_RULE and (r.style["widows"] or r.style["orphans"])
                    ]
                ...
                debug_print(
                    "KoboTouch:_modify_epub:CSS rules {0} -> {1} ({2})".format(
                        oldrules, len(newsheet.cssRules), cssname
                    )
                )
    ...
    def delete_via_sql(self, ContentID, ContentType):
        ...
        if self.dbversion >= 53:
            ...
            debug_print(
                'KoboTouch:delete_via_sql: ContentID="%s"' % ContentID,
                'ContentType="%s"' % ContentType,
            )
            try:
                with closing(sqlite.connect(self.device_database_path())) as connection:
                ...
                debug_print("KoboTouch:delete_via_sql - DatabasePing Exception:  %s" % str(e))
    ...
    def contentid_from_path(self, path, ContentType):
        ...
        if show_debug:
            debug_print(
                "KoboTouch:contentid_from_path - path='%s'" % path,
                "ContentType='%s'" % ContentType,
            )
            debug_print(
                "KoboTouch:contentid_from_path - self._main_prefix='%s'" % self._main_prefix,
                "self._card_a_prefix='%s'" % self._card_a_prefix,
            )
        if ContentType == 6:
            extension = os.path.splitext(path)[1]
            if extension == ".kobo":
            ...
            debug_print(
                "KoboTouch:contentid_from_path ContentType other than 6 - ContentType='%d'" % ContentType,
                "path='%s'" % path,
            )
            ContentID = path
            ContentID = ContentID.replace(self._main_prefix, "file:///mnt/onboard/")
    ...
    def update_device_database_collections(self, booklists, collections_attributes, oncard):
        ...
        with closing(sqlite.connect(self.normalize_path(self._main_prefix + ".kobo/KoboReader.sqlite"))) as connection:
            ...
            if collections:
                ...
                for category, books in collections.items():
                    debug_print(
                        "KoboTouch:update_device_database_collections - category='%s' books=%d" % (category, len(books))
                    )
                    if create_bookshelves and not (
                        category in supportedcategories or category in readstatuslist or category in accessibilitylist
                    ):
                    ...
                    for book in books:
                        ...
                        if category_added and category not in book.device_collections:
                            if show_debug:
                                debug_print(
                                    "            adding category to book.device_collections",
            ...
            if (self.supports_bookshelves() or self.supports_series()) and (
                bookshelf_attribute or update_series_details
            ):
                ...
                for book in booklists:
                    if book.application_id is not None:
                        ...
                        if bookshelf_attribute:
                            if show_debug:
                                debug_print(
                                    "KoboTouch:update_device_database_collections - about to remove a book from shelves book.title=%s"
                                    % book.title
                                )
                ...
                debug_print(
                    "KoboTouch:update_device_database_collections - Number of series set=%d Number of books=%d"
                    % (self.series_set, books_in_library)
                )
    ...
    def upload_cover(self, path, filename, metadata, filepath):
        ...
        if (
            self._card_a_prefix
            and os.path.abspath(path).startswith(os.path.abspath(self._card_a_prefix))
            and not self.supports_covers_on_sdcard()
        ):
            return
    ...
    def images_path(self, path, imageId=None):
        if (
            self._card_a_prefix
            and os.path.abspath(path).startswith(os.path.abspath(self._card_a_prefix))
            and self.supports_covers_on_sdcard()
        ):
    ...
    def _upload_cover(
        ...
        from LiuXin.utils.magick.draw import save_cover_data_to, identify_data
        ...
        if metadata.cover:
            ...
            if show_debug:
                debug_print(
                    "KoboTouch:_upload_cover - path='%s'" % path,
                    "filename='%s'" % filename,
                )
                debug_print("        filepath='%s'" % filepath)
            cover = self.normalize_path(metadata.cover.replace("/", os.sep))
                ...
                try:
                    ...
                    with closing(sqlite.connect(self.device_database_path())) as connection:
                        ...
                        if result is None:
                            ImageID = self.imageid_from_contentid(ContentID)
                            debug_print(
                                "KoboTouch:_upload_cover - No rows exist in the database - generated ImageID='%s'"
                                % ImageID
                            )
                        else:
                            ImageID = result[0]
                    ...
                    if ImageID != None:
                        ...
                        if not os.path.exists(image_dir):
                            debug_print(
                                "KoboTouch:_upload_cover - Image directory does not exust. Creating path='%s'"
                                % (image_dir)
                            )
                        ...
                        for ending, cover_options in self.cover_file_endings().items():
                            ...
                            if show_debug:
                                debug_print(
                                    "KoboTouch:_upload_cover - resize=%s min_dbversion=%d max_dbversion=%d"
                                    % (resize, min_dbversion, max_dbversion)
                                )
                            if self.dbversion >= min_dbversion and self.dbversion <= max_dbversion:
                                if show_debug:
                                    debug_print(
                                        "KoboTouch:_upload_cover - creating cover for ending='%s'" % ending
                                    )  # , "resize'%s'"%resize)
                                fpath = path + ending
                                fpath = self.normalize_path(fpath.replace("/", os.sep))
                                ...
                                if keep_cover_aspect:
                                    if isFullsize:
                                        ...
                                        if cover_aspect > 1:
                                            resize = (
                                                resize[0],
                                                int(resize[0] / cover_aspect),
                                            )
                                        elif cover_aspect < 1:
                                            resize = (
                                                int(cover_aspect * resize[1]),
                                                resize[1],
                                            )
    ...
    def remove_book_from_device_bookshelves(self, connection, book):
        ...
        if show_debug:
            debug_print(
                'KoboTouch:remove_book_from_device_bookshelves - book.application_id="%s"' % book.application_id
            )
            debug_print('KoboTouch:remove_book_from_device_bookshelves - book.contentID="%s"' % book.contentID)
            debug_print(
                "KoboTouch:remove_book_from_device_bookshelves - book.device_collections=",
                book.device_collections,
            )
            debug_print(
                "KoboTouch:remove_book_from_device_bookshelves - book.current_shelves=",
                book.current_shelves,
            )
            debug_print(
                "KoboTouch:remove_book_from_device_bookshelves - remove_shelf_list=",
                remove_shelf_list,
            )
    ...
    def set_filesize_in_device_database(self, connection, contentID, fpath):
        ...
        test_query = "SELECT ___FileSize " "FROM content " "WHERE ContentID = ? " " AND ContentType = 6"
        test_values = (contentID,)

        updatequery = "UPDATE content " "SET ___FileSize = ? " "WHERE ContentId = ? " "AND ContentType = 6"
        ...
        if result is None:
            ...
            if show_debug:
                debug_print(
                    "        Found a record - will update - ___FileSize=",
                    result[0],
                    " file_size=",
                    file_size,
                )
    ...
    def check_for_bookshelf(self, connection, bookshelf_name):
        ...
        if self.dbversion < 64:
            ...
            addquery += (
                ' ("CreationDate", "InternalName","LastModified","Name","_IsDeleted","_IsVisible","_IsSynced", "Id")'
                " VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
            )
        ...
        if result is None:
            ...
            debug_print(
                'KoboTouch:check_for_bookshelf - Shelf "%s" is deleted - undeleting. result[2]="%s"'
                % (bookshelf_name, six_unicode(result[2]))
            )
    ...
    def set_series(self, connection, book):
        ...
        try:
            ...
            debug_print("    DatabasePing Exception:  Unable to set series info")
            raise
        else:
            connection.commit()
        cursor.close()
    ...
    def cover_file_endings(self):
        return (
            ...
            if self.isGlo() or self.isAura()
            else self.AURA_HD_COVER_FILE_ENDINGS
            if self.isAuraHD() or self.isAuraH2O()
    ...
    def copying_covers(self):
        opts = self.settings()
        return (
            opts.extra_customization[self.OPT_UPLOAD_COVERS]
            or opts.extra_customization[self.OPT_KEEP_COVER_ASPECT_RATIO]
        )
    ...
    def supports_covers_on_sdcard(self):
        return (
            self.dbversion >= self.min_dbversion_images_on_sdcard
            and self.fwversion >= self.min_fwversion_images_on_sdcard
        )
    ...
    def modify_database_check(self, function):
        ...
        if self.dbversion > self.supported_dbversion or self.fwversion > self.max_supported_fwversion:
            ...
            if not opts.extra_customization[self.OPT_SUPPORT_NEWER_FIRMWARE]:
                debug_print("The database has been upgraded past supported version")
                self.report_progress(1.0, _("Removing books from device..."))
                from LiuXin.utils.calibre.devices.errors import UserFeedback
    ...
    def is_debugging_title(cls, title):
        ...
        if opts.extra_customization:
            ...
            is_debugging = (
                len(debugging_title) > 0 and title.lower().find(debugging_title.lower()) >= 0 or len(title) == 0
            )
