...
from LiuXin.devices.usbms.books import Book as Book_
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        try:
                            self.datetime = parse_date(date, assume_utc=True).timetuple()
                        except:
                            try:
...
class KTCollectionsBookList(CollectionsBookList):
    def get_collections(self, collection_attributes):
        ...
        for book in self:
            ...
            if show_debug:  # or len(book.device_collections) > 0:
                debug_print(
                    "KTCollectionsBookList:get_collections - tsval=",
                    tsval,
                    "book.title=",
                    book.title,
                    "book.title_sort=",
                ...
                for category in val:
                    ...
                    if doing_dc:
                        ...
                        if attr == "tags" and len(category) > 1 and category[0] == "[" and category[-1] == "]":
                            continue
                        if attr == "series" or (
                            "series" in collection_attributes and book.get("series", None) == category
                        ):
                            is_series = True
                        ...
                        if is_series:
                            if doing_dc:
                                collections[cat_name][lpath] = (
                                    book,
                                    book.get("series_index", sys.maxint),
                                    tsval,
                                )
                            else:
                                collections[cat_name][lpath] = (
                                    book,
                                    book.get(attr + "_index", sys.maxint),
                                    tsval,
                                )
                        else:
    ...
    def is_debugging_title(self, title):
        ...
        is_debugging = (
            self.debugging_title is not None
            and len(self.debugging_title) > 0
            and title is not None
            and (title.lower().find(self.debugging_title.lower()) >= 0 or len(title) == 0)
        )
