...
from LiuXin.devices.errors import (
    OpenFailed,
    OpenFeedback,
    ControlError,
    TimeoutError,
...
from LiuXin.devices.interface import DevicePlugin, currently_connected_device
from LiuXin.devices.usbms.books import Book, CollectionsBookList
from LiuXin.devices.usbms.deviceconfig import DeviceConfig
from LiuXin.devices.usbms.driver import USBMS
from LiuXin.devices.utils import build_template_regexp

from LiuXin.file_formats import BOOK_EXTENSIONS

from LiuXin.interfaces.reader_server import server_config as content_server_config

from LiuXin.library import current_library_name

from LiuXin.metadata import title_sort
from LiuXin.metadata.book.json_codec import JsonCodec
from LiuXin.metadata.metadata import MetaData as Metadata

from LiuXin import prints
from LiuXin.utils.calibre.constants import numeric_version, cache_dir
from LiuXin.utils.config.config_base import tweaks
from LiuXin.utils.filenames import ascii_filename as sanitize, shorten_components_to
from LiuXin.utils.ipc import eintr_retry_call
from LiuXin.utils.localization import _
from LiuXin.utils.mdns import (
    publish as publish_zeroconf,
    unpublish as unpublish_zeroconf,
    get_all_ips,
)
from LiuXin.utils.ptempfiles import PersistentTemporaryFile
from LiuXin.utils.libraries.socket_inheritance import set_socket_inherit
...
class ConnectionListener(Thread):
    ...
    def run(self):
        ...
        while self.keep_running:
                ...
                while True:
                    ...
                    if len(ans[0]) > 0:
                        try:
                            ...
                            try:
                                content_server_port = str(content_server_config().parse().port)
                            except:
                                pass
                            message = str(
                                self.driver.ZEROCONF_CLIENT_STRING
                                + b" (on "
                                + str(socket.gethostname().partition(".")[0])
                                + b");"
                                + content_server_port
                                + b","
                                + str(self.driver.port)
                            )
            ...
            if self.driver.connection_queue.empty() and getattr(self.driver, "listen_socket", None) is not None:
...
class SMART_DEVICE_APP(DeviceConfig, DevicePlugin):
    ...
    EXTRA_CUSTOMIZATION_MESSAGE = [
        ...
        + "</p>",
        "",
        _("Comma separated list of metadata fields " "to turn into collections on the device.")
        + ":::<p>"
        + _(
            "Possibilities include: series, tags, authors, etc"
        ...
        % dict(
            ...
            abav=ALL_BY_AUTHOR,
            abs="abs",
            absv=ALL_BY_SOMETHING,
        ),
        "",
        _("Enable the no-activity timeout")
        + ":::<p>"
        + _(
            "If this box is checked, calibre will automatically disconnect if "
        ...
        + ":::<p>"
        + _(
            "Use this option to overwrite a book on the device if that book "
        ...
        + ":::<p>"
        + _(
            "Use this option to control the size and quality of the cover "
        ...
        + ":::<p>"
        + _(
            "Setting this option allows calibre to keep a copy of metadata "
            ...
            "you think that the cache might not be operating correctly."
        )
        + "</p>",
    ]
    ...
    def _debug(self, *args):
        ...
        with self.sync_lock:
            ...
            print(
                "SMART_DEV (%7.2f:%7.3f) %s" % (total_elapsed, elapsed, inspect.stack()[1][3]),
                end="",
            )
            for a in args:
                try:
    ...
    def _update_driveinfo_record(self, dinfo, prefix, location_code, name=None):
        from LiuXin.utils.calibre.utils.date import isoformat, now
        import uuid

        if not isinstance(dinfo, dict):
            dinfo = {}
        if dinfo.get("device_store_uuid", None) is None:
            dinfo["device_store_uuid"] = six_unicode(uuid.uuid4())
        ...
        dinfo["calibre_version"] = ".".join([six_unicode(i) for i in numeric_version])
    ...
    def _create_upload_path(self, mdata, fname, create_dirs=True):
        ...
        maxlen = self.MAX_PATH_LEN - (self.PATH_FUDGE_FACTOR + self.exts_path_lengths.get(ext, self.PATH_FUDGE_FACTOR))
        ...
        from LiuXin.utils.calibre.library.save_to_disk import get_components
        from LiuXin.utils.calibre.library.save_to_disk import config
    ...
    def _receive_from_client(self, print_debug_info=True):
        from LiuXin.utils.calibre.utils.config import from_json
    ...
    def _metadata_in_cache(self, uuid, ext_or_lpath, lastmod):
        from LiuXin.utils.calibre.utils.date import now, parse_date
    ...
    def _metadata_already_on_device(self, book):
        try:
            ...
            if v is not None:
                ...
                if v.get("uuid", None) == book.get("uuid", None) and v.get("last_modified", None) == book.get(
                    "last_modified", None
                ):
    ...
    def _read_metadata_cache(self):
        ...
        try:
            old_cache_file_name = os.path.join(
                cache_dir(),
                "device_drivers_" + self.__class__.__name__ + "_metadata_cache.pickle",
            )
        ...
        try:
            old_cache_file_name = os.path.join(
                cache_dir(),
                "device_drivers_" + self.__class__.__name__ + "_metadata_cache.json",
            )
            if os.path.exists(old_cache_file_name):
                os.remove(old_cache_file_name)
        except:
        ...
        cache_file_name = os.path.join(cache_dir(), "wireless_device_" + self.device_uuid + "_metadata_cache.json")
            ...
            if os.path.exists(cache_file_name):
                with open(cache_file_name, mode="rb") as fd:
                    while True:
                        ...
                        key = book.keys()[0]
                        metadata = self.json_codec.raw_to_book(book[key]["book"], SDBook, self.PREFIX)
    ...
    def _write_metadata_cache(self):
        self._debug()
        from LiuXin.utils.calibre.utils.date import now

        now_ = now()
        from LiuXin.utils.calibre.utils.config import to_json

        try:
            purged = 0
            count = 0
            prefix = os.path.join(cache_dir(), "wireless_device_" + self.device_uuid + "_metadata_cache")
            ...
            from LiuXin.utils.calibre.utils.filenames import atomic_rename
    ...
    def _read_file_metadata(self, temp_file_name):
        from LiuXin.utils.calibre.ebooks.metadata.meta import get_metadata
        from LiuXin.utils.calibre.customize.ui import quick_metadata
    ...
    def detect_managed_devices(self, devices_on_system, force_refresh=False):
        ...
        if self.is_connected:
            self.noop_counter += 1
            if (
                self.noop_counter > self.SEND_NOOP_EVERY_NTH_PROBE
                and (self.noop_counter % self.SEND_NOOP_EVERY_NTH_PROBE) != 1
            ):
                try:
                    ...
                    pass
            if (
                self.settings().extra_customization[self.OPT_AUTODISCONNECT]
                and self.noop_counter > self.DISCONNECT_AFTER_N_SECONDS
            ):
                self._close_device_socket()
    ...
    def open(self, connected_device, library_uuid):
        ...
        try:
            ...
            if not (
                client_can_stream_books
                and client_can_stream_metadata
                and client_can_receive_book_binary
                and client_can_delete_multiple
            ):
                self._debug("Software on device too old")
                self._close_device_socket()
                raise OpenFeedback(
                    _("The app on your device is too old and is no " "longer supported. Update it to a newer version.")
                )
            ...
            self.set_temp_mark_when_syncing_read = result.get("setTempMarkWhenReadInfoSynced", False)
            self._debug(
                "Will set temp mark when syncing read",
            ...
            try:
                if self.client_app_name == "CalibreCompanion" and self.app_version_number < self.CURRENT_CC_VERSION:
            ...
            self.max_book_packet_len = result.get("maxBookContentPacketLen", self.BASE_PACKET_LEN)
            ...
            if "coverWidth" in result:
                ...
                self.THUMBNAIL_WIDTH = result.get("coverWidth", (self.DEFAULT_THUMBNAIL_HEIGHT / 3) * 4)
    ...
    def config_widget(self):
        from LiuXin.utils.calibre.gui2.device_drivers.configwidget import ConfigWidget
    ...
    def set_driveinfo_name(self, location_code, name):
        ...
        self._call_client("SET_CALIBRE_DEVICE_NAME", {"location_code": "main", "name": name})
    ...
    def books(self, oncard=None, end_session=True):
        ...
        opcode, result = self._call_client(
            "GET_BOOK_COUNT",
            {
                ...
                "supportsSync": (bool(self.is_read_sync_col) or bool(self.is_read_date_sync_col)),
            ...
            for i in range(0, count):
                ...
                if opcode == "OK":
                    try:
                        ...
                        self._debug(
                            "exception retrieving metadata for book",
    ...
    def sync_booklists(self, booklists, end_session=True):
        colattrs = [x.strip() for x in self.settings().extra_customization[self.OPT_COLLECTIONS].split(",")]
        ...
        for book in booklists[0]:
            if book.get("_force_send_metadata_", None) or not self._metadata_already_on_device(book):
    ...
    def _show_message(self, message):
        self._call_client(
            "DISPLAY_MESSAGE",
            {"messageKind": self.MESSAGE_SHOW_TOAST, "message": message},
        )
    ...
    def _check_if_format_send_needed(self, db, id_, book):
        ...
        from LiuXin.utils.calibre.utils.date import parse_date, isoformat
            ...
            if fmt_metadata:
                ...
                if calibre_mtime > self.now:
                    if not self.have_sent_future_dated_book_message:
                        ...
                        self._show_message(
                            _(
                                "You have book formats in your library "
    ...
    def synchronize_with_db(self, db, id_, book, first_call):
        from LiuXin.utils.calibre.utils.date import parse_date, is_date_undefined, now

        if first_call:
            self.have_sent_future_dated_book_message = False
            self.now = now()

        if self.have_bad_sync_columns or not (self.is_read_sync_col or self.is_read_date_sync_col):
        ...
        if not self.have_checked_sync_columns:
            ...
            if self.is_read_sync_col:
                if self.is_read_sync_col not in fm:
                    ...
                    self._show_message(
                        _("The read sync column %s is " "not in calibre's library") % self.is_read_sync_col
                    )
                    self.have_bad_sync_columns = True
                elif fm[self.is_read_sync_col]["datatype"] != "bool":
                    self._debug("is_read_sync_col not bool type")
                    self._show_message(_("The read sync column %s is " "not a Yes/No column") % self.is_read_sync_col)
                    self.have_bad_sync_columns = True
            ...
            if self.is_read_date_sync_col:
                if self.is_read_date_sync_col not in fm:
                    ...
                    self._show_message(
                        _("The read date sync column %s is " "not in calibre's library") % self.is_read_date_sync_col
                    )
                    self.have_bad_sync_columns = True
                elif fm[self.is_read_date_sync_col]["datatype"] != "datetime":
                    self._debug("is_read_date_sync_col not date type")
                    self._show_message(
                        _("The read date sync column %s is " "not a Date column") % self.is_read_date_sync_col
                    )
                    self.have_bad_sync_columns = True
        ...
        if sync_type == 3:
            ...
            if has_is_read and self.is_read_sync_col:
                try:
                    calibre_val = db.new_api.field_for(self.is_read_sync_col, id_, default_value=None)
                    if is_read is not None:
                        ...
                        if is_read != calibre_val:
                            self._debug(
                                "special update calibre to is_read",
                                book.get("title", "huh?"),
                                "to",
                                is_read,
                                calibre_val,
                            )
                            changed_books = db.new_api.set_field(self.is_read_sync_col, {id_: is_read})
                            if self.set_temp_mark_when_syncing_read:
                        ...
                        self._debug(
                            "special update is_read to calibre value",
                            book.get("title", "huh?"),
                            "to",
                            calibre_val,
                        )
            ...
            if has_is_read_date and self.is_read_date_sync_col:
                try:
                    ...
                    calibre_val = db.new_api.field_for(self.is_read_date_sync_col, id_, default_value=None)
                    if is_read_date is not None:
                        if is_read_date != calibre_val:
                            self._debug(
                                "special update calibre to is_read_date",
                                book.get("title", "huh?"),
                                "to",
                                is_read_date,
                                calibre_val,
                            )
                            changed_books |= db.new_api.set_field(self.is_read_date_sync_col, {id_: is_read_date})
                            if self.set_temp_mark_when_syncing_read:
                                db.data.toggle_marked_ids({id_})
                    elif calibre_val is not None:
                        self._debug(
                            "special update is_read_date to calibre value",
                            book.get("title", "huh?"),
                            "to",
                            calibre_val,
                        )
                    ...
                    self._debug("exception special syncing is_read_date", self.is_read_sync_col)
                    traceback.print_exc()
        else:
            ...
            if has_is_read_date and self.is_read_date_sync_col:
                try:
                    ...
                    self._debug(
                        "Exception standard syncing is_read_date",
    ...
    def startup_on_demand(self):
        ...
        if not compression_quality_ok:
            ...
            message = (
                _("Bad compression quality setting. It must be a number " "between 50 and 99. Forced to be %d.")
        ...
        if self.settings().extra_customization[self.OPT_USE_PORT]:
            try:
                ...
                message = _("Invalid port in options: %s") % self.settings().extra_customization[self.OPT_PORT_NUMBER]
                self._debug(message)
            ...
            while i < 100:  # try 9090 then up to 99 random port numbers
                i += 1
                port = self._attach_to_port(self.listen_socket, 9090 if i == 1 else random.randint(8192, 32000))
                if port != 0:
                    break
            if port == 0:
        ...
        try:
            ip_addr = self.settings().extra_customization[self.OPT_FORCE_IP_ADDRESS]
            publish_zeroconf(
                "calibre smart device client",
                "_calibresmartdeviceapp._tcp",
                port,
                {},
