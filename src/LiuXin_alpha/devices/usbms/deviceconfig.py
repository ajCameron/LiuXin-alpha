...
from LiuXin.utils.config.config_base import Config, ConfigProxy
...
class DeviceConfig(object):
    ...
    def _default_save_template(cls):
        from LiuXin.library.save_to_disk import config
            ...
            if isinstance(cls.EXTRA_CUSTOMIZATION_MESSAGE, list):
                ...
                for i in range(0, len(cls.EXTRA_CUSTOMIZATION_MESSAGE)):
                    ...
                    if hasattr(config_widget.opt_extra_customization[i], "isChecked"):
                        ...
                        ec.append(six_unicode(config_widget.opt_extra_customization[i].currentText()).strip())
                    else:
                        ec.append(six_unicode(config_widget.opt_extra_customization[i].text()).strip())
            else:
                ec = six_unicode(config_widget.opt_extra_customization.text()).strip()
                if not ec:
                    ec = None
            proxy["extra_customization"] = ec
        st = six_unicode(config_widget.opt_save_template.text())
