...
from LiuXin.devices.errors import DeviceError
from LiuXin.devices.usbms.deviceconfig import DeviceConfig
from LiuXin.constants import iswindows, islinux, isosx, isfreebsd
from LiuXin.utils.localization import _
from LiuXin.utils.filenames import ascii_filename as sanitize

from past.builtins import basestring


if isosx:
    raise NotImplementedError("Plugin usbobserver needs to be installed.\n")
...
class USBDevice:
    ...
    def match_strings(self, vid, pid, bcd, man, prod):
        return self.match_numbers(vid, pid, bcd) and self.manufacturer == man and self.product == prod
...
class Device(DeviceConfig, DevicePlugin):
    ...
    def windows_match_device(self, pnp_id, attr):
        ...
        def test_vendor():
            vendors = [self.VENDOR_NAME] if isinstance(self.VENDOR_NAME, basestring) else self.VENDOR_NAME
            for v in vendors:
    ...
    def can_handle_windows(self, device_id, debug=False):
        from LiuXin.devices.scanner import win_pnp_drives
    ...
    def open_windows(self):
        from LiuXin.devices.scanner import win_pnp_drives, drivecmp
        ...
        for drive, pnp_id in win_pnp_drives().items():
            if self.windows_match_device(pnp_id, "WINDOWS_CARD_A_MEM") and not drives.get("carda", False):
                drives["carda"] = drive
                dup_prod_id |= check_for_dups(pnp_id)
            elif self.windows_match_device(pnp_id, "WINDOWS_CARD_B_MEM") and not drives.get("cardb", False):
                drives["cardb"] = drive
                dup_prod_id |= check_for_dups(pnp_id)
            elif self.windows_match_device(pnp_id, "WINDOWS_MAIN_MEM") and not drives.get("main", False):
                drives["main"] = drive
                dup_prod_id |= check_for_dups(pnp_id)
        ...
        if (
            dup_prod_id
            or self.WINDOWS_MAIN_MEM in (self.WINDOWS_CARD_A_MEM, self.WINDOWS_CARD_B_MEM)
            or self.WINDOWS_CARD_A_MEM == self.WINDOWS_CARD_B_MEM
        ):
    ...
    def osx_run_mount(cls):
        for i in range(3):
            try:
                return subprocess.Popen("mount", stdout=subprocess.PIPE).communicate()[0]
            except IOError:  # Probably an interrupted system call
                if i == 2:
                    raise
            time.sleep(2)
    ...
    def osx_get_usb_drives(cls):
        raise NotImplementedError("usbobserver needs to be installed. This cannot be fixed at this time.\n")
        # if usbobserver_err:
            ...
            if d.manufacturer and d.product:
                ...
                for path, vid, pid, bcd, man, prod, serial in drives:
                    if d.match_numbers(vid, pid, bcd):
                        matches.append(path)
        if not matches:
            raise DeviceError("Could not detect BSD names for %s. Try rebooting." % self.name)
    ...
    def node_mountpoint(self, node):
        from LiuXin.devices.udisks import node_mountpoint
    ...
    def open_linux(self):
        def mount(node, type):
            ...
            def do_mount(node):
                try:
                    from LiuXin.devices.udisks import mount
    ...
    def filter_read_only_mount_points(self):
        ...
        if self._main_prefix is None:
            raise DeviceError(
                _("The main memory of %s is read only. " "This usually happens because of file system errors.")
                % self.__class__.__name__
            )
    ...
    def open_freebsd(self):
        ...
        for path in paths:
            objif = dbus.Interface(
                bus.get_object("org.freedesktop.Hal", path),
                "org.freedesktop.Hal.Device",
            )
            # Extra paranoia...
            try:
                if (
                    d.idVendor == objif.GetProperty("usb.vendor_id")
                    and d.idProduct == objif.GetProperty("usb.product_id")
                    and d.manufacturer == objif.GetProperty("usb.vendor")
                    and d.product == objif.GetProperty("usb.product")
                    and d.serial == objif.GetProperty("usb.serial")
                ):
                            ...
                            for vpath in vpaths:
                                try:
                                    vdevif = dbus.Interface(
                                        bus.get_object("org.freedesktop.Hal", vpath),
                                        "org.freedesktop.Hal.Device",
                                    )
                                    ...
                                    vol = {
                                        "node": pdevif.GetProperty("block.device"),
                                        "dev": vdevif,
                                        "vol": volif,
    ...
    def eject_linux(self):
        from LiuXin.devices.udisks import eject, umount
    ...
    def eject(self):
        """
        Method to be called if the device is being ejected from within calibre.
        """
        if islinux:
            try:
                self.eject_linux()
    ...
    def _sanity_check(self, on_card, files):
        from LiuXin.devices.utils import sanity_check
    ...
    def create_upload_path(self, path, mdata, fname, create_dirs=True):
        from LiuXin.devices.utils import create_upload_path
