...
from LiuXin.devices.usbms.cli import CLI
from LiuXin.devices.usbms.device import Device
from LiuXin.devices.usbms.books import BookList, Book

from LiuXin.metadata.book.json_codec import JsonCodec

from LiuXin.utils.calibre import isbytestring, fsync
...
class USBMS(CLI, Device):
    ...
    def _update_driveinfo_record(self, dinfo, prefix, location_code, name=None):
        from LiuXin.utils.date import now, isoformat
        import uuid
    ...
    def _update_driveinfo_file(self, prefix, location_code, name=None):
        from LiuXin.utils.config.config_tools import from_json, to_json
        ...
        if os.path.exists(os.path.join(prefix, self.DRIVEINFO)):
            with open(os.path.join(prefix, self.DRIVEINFO), "rb") as f:
                try:
                    ...
                    driveinfo = None
                driveinfo = self._update_driveinfo_record(driveinfo, prefix, location_code, name)
    ...
    def get_device_information(self, end_session=True):
        ...
        if self._main_prefix is not None:
            try:
                ...
                raise IOError(
                    _(
                        "Failed to access files in the main memory of"
        ...
        try:
            ...
            raise IOError(
                _(
                    "Failed to access files on the SD card in your"
    ...
    def books(self, oncard=None, end_session=True):
        ...
        debug_print(
            "USBMS: Fetching list of books from device. Device=",
        ...
        ebook_dirs = (
            self.get_carda_ebook_dir()
            if oncard == "carda"
            else self.EBOOK_DIR_CARD_B
            if oncard == "cardb"
            else self.get_main_ebook_dir()
        )
            ...
            if path_to_ext(filename) in all_formats:
                try:
                    ...
                    if idx is not None:
                        ...
                        if bl.add_book(self.book_from_path(prefix, lpath), replace_metadata=False):
                            changed = True
        ...
        for ebook_dir in ebook_dirs:
            ...
            if self.SCAN_FROM_ROOT:
                ...
                ebook_dir = self.normalize_path(os.path.join(prefix, *(ebook_dir.split("/"))) if ebook_dir else prefix)
            ...
            if self.SUPPORTS_SUB_DIRS or self.SUPPORTS_SUB_DIRS_FOR_SCAN:
                ...
                for i, filename in enumerate(paths):
                    self.report_progress(
                        (i + 1) / float(len(paths)),
                        _("Getting list of books on device..."),
                    )
        ...
        debug_print(
            "USBMS: count found in cache: %d, count of files in metadata: %d, need_sync: %s"
    ...
    def sync_booklists(self, booklists, end_session=True):
        ...
        def write_prefix(prefix, listid):
            if prefix is not None and len(booklists) > listid and isinstance(booklists[listid], self.booklist_class):
    ...
    def metadata_from_formats(cls, fmts):
        ...
        from LiuXin.customize.ui import quick_metadata
    ...
    def book_from_path(cls, prefix, lpath):
        ...
        if mi is None:
            mi = Metadata(os.path.splitext(os.path.basename(lpath))[0], [_("Unknown")])
