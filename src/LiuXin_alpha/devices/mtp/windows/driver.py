...
from LiuXin.constants import isxp

from LiuXin.devices.errors import OpenFailed, DeviceError, BlacklistedDevice
from LiuXin.devices.mtp.base import MTPDeviceBase, debug

from LiuXin.utils.calibre import as_unicode, force_unicode
from LiuXin import prints
from LiuXin.utils.calibre.constants import __appname__, numeric_version
...
class MTP_DEVICE(MTPDeviceBase):
    ...
    def detect_managed_devices(self, devices_on_system, force_refresh=False):
        ...
        if (
            force_refresh
            or devices_on_system != self.previous_devices_on_system
            or time.time() - self.last_refresh_devices_time > 10
        ):
    ...
    # Todo: Should be over in the plugins code - where it can be built
    def filesystem_cache(self):
        if self._filesystem_cache is None:
            debug("Loading filesystem metadata...")
            st = time.time()
            from LiuXin.utils.calibre.devices.mtp.filesystem_cache import (
                FilesystemCache,
            )
            ...
            for storage_id, capacity in zip([self._main_id, self._carda_id, self._cardb_id], ts):
                ...
                self._currently_getting_sid = six_unicode(storage_id)
    ...
    def open(self, connected_device, library_uuid):
        ...
        try:
            ...
            try:
                ...
                raise OpenFailed("Failed to open %s with error: %s" % (connected_device, as_unicode(e)))
    ...
    def get_mtp_file(self, f, stream=None, callback=None):
        ...
        try:
            ...
            raise DeviceError("Failed to fetch the file %s with error: %s" % (f.full_path, as_unicode(e)))
        stream.seek(0)
        if set_name:
            stream.name = f.name
        return stream
