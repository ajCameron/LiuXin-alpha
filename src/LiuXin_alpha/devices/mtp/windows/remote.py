...
def build(mod="wpd"):
    ...
    try:
        ...
        subprocess.check_call(("scp win64:build/calibre/src/calibre/plugins/%s.pyd /tmp" % mod).split())
        subprocess.check_call(("scp /tmp/%s.pyd getafix:calibre-src/src/calibre/devices/mtp/windows" % mod).split())
        p = subprocess.Popen(
...
def main():
    ...
    from LiuXin.devices.scanner import win_scanner
