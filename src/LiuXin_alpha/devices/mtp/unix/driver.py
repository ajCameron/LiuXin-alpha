...
from LiuXin.constants import islinux, isosx

from LiuXin.devices.errors import OpenFailed, DeviceError, BlacklistedDevice
from LiuXin.devices.mtp.base import MTPDeviceBase, synchronous, debug
...
from LiuXin.utils.ptempfiles import SpooledTemporaryFile

from LiuXin.utils.lx_libraries.liuxin_six import six_unicode

MTPDevice = namedtuple("MTPDevice", "busnum devnum vendor_id product_id bcd serial manufacturer product")
...
class MTP_DEVICE(MTPDeviceBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if islinux:
            from LiuXin.devices.mtp.unix.sysfs import MTPDetect
    ...
    def is_device_mtp(self, d, debug=None):
        ...
        if self._is_device_mtp is None:
            return False
        return self._is_device_mtp(d, debug=debug) and self.libmtp.is_mtp_device(d.busnum, d.devnum)
    ...
    def detect_managed_devices(self, devices_on_system, force_refresh=False):
        ...
        if self.currently_connected_dev is not None:
            return self.currently_connected_dev if self.currently_connected_dev in devs else None
        ...
        for d in devs:
            ...
            if ans is None:
                ans = (
                    d.vendor_id,
                    d.product_id,
                ) in self.known_devices or self.is_device_mtp(d)
                cache[d] = ans
            if ans:
                return d
    ...
    def debug_managed_device_detection(self, devices_on_system, output):
        ...
        devs = [
            d
            for d in devices_on_system
            if ((d.vendor_id, d.product_id) in self.known_devices or self.is_device_mtp(d, debug=p))
            and d.vendor_id != APPLE
        ]
        if not devs:
    ...
    def format_errorstack(self, errs):
        return "\n".join(["%d:%s" % (code, msg.decode("utf-8", "replace")) for code, msg in errs])
    ...
    def open(self, connected_device, library_uuid):
        ...
        try:
            ...
            raise OpenFailed("Failed to open %s: Error: %s" % (connected_device, as_unicode(e)))
        ...
        if snum in self.prefs.get("blacklist", []):
            self.blacklisted_devices.add(connected_device)
            self.dev = None
            raise BlacklistedDevice("The %s device has been blacklisted by the user" % (connected_device,))
    ...
    def filesystem_cache(self):
        if self._filesystem_cache is None:
            ...
            from LiuXin.devices.mtp.filesystem_cache import FilesystemCache

            with self.lock:
                storage, all_items, all_errs = [], [], []
                for sid, capacity in zip([self._main_id, self._carda_id, self._cardb_id], self.total_space()):
                    if sid is None:
                        continue
                    name = _("Unknown")
                    ...
                    self._currently_getting_sid = six_unicode(sid)
                ...
                if not all_items and all_errs:
                    raise DeviceError(
                        ...
                        % (self.current_friendly_name, self.format_errorstack(all_errs))
                    )
                if all_errs:
                    prints(
                        "There were some errors while getting the "
                        " filesystem from %s: %s" % (self.current_friendly_name, self.format_errorstack(all_errs))
                    )
                self._filesystem_cache = FilesystemCache(storage, all_items)
            debug(
                "Filesystem metadata loaded in %g seconds (%d objects)"
    ...
    def total_space(self, end_session=True):
        ...
        for s in self.dev.storage_info:
            i = {self._main_id: 0, self._carda_id: 1, self._cardb_id: 2}.get(s["id"], None)
            if i is not None:
                ans[i] = s["capacity"]
        return tuple(ans)
    ...
    def put_file(self, parent, name, stream, size, callback=None, replace=True):
        e = parent.folder_named(name)
        if e is not None:
            raise ValueError("Cannot upload file, %s already has a folder named: %s" % (parent.full_path, e.name))
        e = parent.file_named(name)
        if e is not None:
            if not replace:
                raise ValueError("Cannot upload file %s, it already exists" % (e.full_path,))
        ...
        if ans is None:
            raise DeviceError(
                "Failed to upload file named: %s to %s: %s" % (name, parent.full_path, self.format_errorstack(errs))
            )
        return parent.add_child(ans)
    ...
    def get_mtp_file(self, f, stream=None, callback=None):
        ...
        if not ok:
            raise DeviceError("Failed to get file: %s with errors: %s" % (f.full_path, self.format_errorstack(errs)))
        stream.seek(0)
        if set_name:
            stream.name = f.name
        return stream
    ...
    def delete_file_or_folder(self, obj):
        ...
        if not obj.can_delete:
            raise ValueError("Cannot delete %s as deletion not allowed" % (obj.full_path,))
        if obj.is_system:
            raise ValueError("Cannot delete %s as it is a system object" % (obj.full_path,))
        if obj.files or obj.folders:
            raise ValueError("Cannot delete %s as it is not empty" % (obj.full_path,))
        parent = obj.parent
        ok, errs = self.dev.delete_object(obj.object_id)
        if not ok:
            raise DeviceError("Failed to delete %s with error: %s" % (obj.full_path, self.format_errorstack(errs)))
        parent.remove_child(obj)
        return parent


def develop():
    from LiuXin.devices.scanner import DeviceScanner
