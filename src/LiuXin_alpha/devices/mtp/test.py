...
from LiuXin.utils.icu import lower
...
class TestDeviceInteraction(unittest.TestCase):
    ...
    def test_folder_operations(self):
        ...
        self.assertIs(
            subfolder,
            self.dev.create_folder(folder, "SUB-FOLDER"),
        ...
        root_file = [f for f in self.dev.filesystem_cache.entries[0].files if not f.is_folder]
        if root_file:
    ...
    def test_memory_leaks(self):
        ...
        used_by_one = self.measure_memory_usage(
            1, self.dev.detect_managed_devices, self.scanner.devices, force_refresh=True
        )

        used_by_many = self.measure_memory_usage(
            100,
            self.dev.detect_managed_devices,
        ...
        used_once = self.measure_memory_usage(1, send_file, self.storage, name, raw, size)
        used_many = self.measure_memory_usage(20, send_file, self.storage, name, raw, size)
        ...
        # Test get_filesystem
        used_by_one = self.measure_memory_usage(
            1, self.dev.dev.get_filesystem, self.storage.object_id, lambda x, l: True
        )

        used_by_many = self.measure_memory_usage(
            5, self.dev.dev.get_filesystem, self.storage.object_id, lambda x, l: True
        )
