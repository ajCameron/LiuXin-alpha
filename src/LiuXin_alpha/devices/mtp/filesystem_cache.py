...
from LiuXin.utils.calibre import human_readable, force_unicode
from LiuXin import prints
from LiuXin.utils.date import local_tz, as_utc
from LiuXin.utils.icu import sort_key, lower
...
class FileOrFolder(object):
    def __init__(self, entry, fs_cache):
        ...
        if self.storage_id not in self.all_storage_ids:
            raise ValueError(
                "Storage id %s not valid for %s, valid values: %s" % (self.storage_id, entry, self.all_storage_ids)
            )
        ...
        self.is_ebook = not self.is_folder and self.name.rpartition(".")[-1].lower() in bexts

    def __repr__(self):
        name = "Folder" if self.is_folder else "File"
        try:
            path = six_unicode(self.full_path)
        except:
            path = ""
        datum = "size=%s" % (self.size)
        if self.is_folder:
            datum = "children=%s" % (len(self.files) + len(self.folders))
        return "%s(id=%s, storage_id=%s, %s, path=%s, modified=%s)" % (
            name,
            self.object_id,
            self.storage_id,
            datum,
            path,
            self.last_mod_string,
        )
    ...
    def dump(self, prefix="", out=sys.stdout):
        ...
        data = (
            "%s children" % (sum(map(len, (self.files, self.folders)))) if self.is_folder else human_readable(self.size)
        )
