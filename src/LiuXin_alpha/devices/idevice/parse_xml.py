...
class XmlPropertyListParser(object):
    ...
    def endDocument(self):
        ...
        self._assert(len(self.__stack) is 0, "multiple objects at top level.")
    ...
    def _parse_key(self, name, content):
        if not self.__in_dict:
            print(
                "XmlPropertyListParser() WARNING: ignoring <key>%s</key> (<key> elements must be contained in <dict> element)"
                % content
            )
