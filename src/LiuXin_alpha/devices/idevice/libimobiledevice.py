...
from LiuXin.constants import islinux, isosx, iswindows

from LiuXin.devices.idevice.parse_xml import XmlPropertyListParser
from LiuXin.devices.usbms.driver import debug_print
...
class IDEVICE_T(Structure):
    ...
    _fields_ = [("udid", c_char_p), ("conn_type", c_int), ("conn_data", c_void_p)]
...
class LOCKDOWND_SERVICE_DESCRIPTOR(Structure):
    ...
    _fields_ = [("port", c_uint), ("ssl_enabled", c_ubyte)]
...
class libiMobileDevice:
    ...
    def copy_from_idevice(self, src, dst):
        ...
        if handle is not None:
            ...
            if file_size > BUFFER_SIZE:
                ...
                while bytes_remaining:
                    if bytes_remaining > BUFFER_SIZE:
                        ...
                        self._log(
                            "copying file {0} to {1}, final {2:,} bytes".format(repr(src), dst.name, bytes_remaining)
                        )
            ...
            file_stats = self._afc_get_file_info(src)
            self._log(
                "copied file {0} ({1:,} bytes) to file '{2}' ({3:,} bytes)".format(
                    repr(src), file_size, dst.name, os.path.getsize(dst.name)
                )
            )
        ...
        if app_name:
            try:
                ...
                if not app_name in self.installed_apps:
                    ...
                    self._house_arrest_send_command(
    ...
    def rename(self, from_name, to_name):
        ...
        error = self.lib.afc_rename_path(byref(self.afc), str(from_name), str(to_name))
        if error:
    ...
    def _afc_client_new(self):
        ...
        error = self.lib.afc_client_new(byref(self.device), self.lockdown, byref(afc_client_t)) & 0xFFFF
    ...
    def _afc_client_new_from_house_arrest_client(self):
        ...
        self.afc = None
        afc_client_t = POINTER(AFC_CLIENT_T)()
        error = self.lib.afc_client_new_from_house_arrest_client(byref(self.house_arrest), byref(afc_client_t)) & 0xFFFF
    ...
    def _afc_file_close(self, handle):
        ...
        error = self.lib.afc_file_close(byref(self.afc), handle) & 0xFFFF
        if error:
    ...
    def _afc_file_open(self, filename, mode="r"):
        ...
        if "r" in mode:
            error = (
                self.lib.afc_file_open(byref(self.afc), str(filename), self.AFC_FOPEN_RDONLY, byref(handle)) & 0xFFFF
            )
        elif "w" in mode:
            error = (
                self.lib.afc_file_open(byref(self.afc), str(filename), self.AFC_FOPEN_WRONLY, byref(handle)) & 0xFFFF
            )
        ...
        if "b" in mode:
            ...
            while bytes_remaining > 0:
                ...
                if error:
                    ...
                    self._log_error(
                        " ERROR: reading {0:,} bytes, 0 bytes read, handle:{1}".format(bytes_remaining, handle.value)
                    )
                    bytes_remaining = 0
                else:
            ...
            while bytes_remaining > 0:
                bytes_read = c_uint(0)
                error = self.lib.afc_file_read(
                    byref(self.afc),
                    handle,
                    byref(data, size - bytes_remaining),
                    bytes_remaining,
                    byref(bytes_read),
                )
                if error:
    ...
    def _afc_file_write(self, handle, content, mode="w"):
        ...
        while bytes_remaining > 0:
            ...
            if error:
                ...
                self._log_error(
                    " ERROR: writing {0:,} bytes, 0 bytes written, handle:{1}".format(bytes_remaining, handle.value)
                )
                return False
            else:
    ...
    def _afc_get_device_info(self):
        ...
        if self.afc is not None:
            ...
            error = self.lib.afc_get_device_info(byref(self.afc), byref(info_raw)) & 0xFFFF
            if not error:
                num_items = 0
                item_list = []
    ...
    def _afc_get_file_info(self, path, silent=False):
        ...
        error = self.lib.afc_get_file_info(byref(self.afc), str(path), byref(infolist)) & 0xFFFF
        file_stats = {}
        if error:
    ...
    def _afc_make_directory(self, path):
        ...
        error = self.lib.afc_make_directory(byref(self.afc), str(path)) & 0xFFFF
        if error:
            self._log_error(" ERROR: {0} path: {1}".format(self._afc_error(error), repr(path)))
    ...
    def _afc_read_directory(self, directory="", get_stats=True):
        ...
        dirs_p = c_char_p
        dirs = POINTER(dirs_p)()
        error = self.lib.afc_read_directory(byref(self.afc), str(directory), byref(dirs)) & 0xFFFF
        if error:
    ...
    def _house_arrest_client_new(self):
        ...
        error = (
            self.lib.house_arrest_client_new(byref(self.device), self.lockdown, byref(house_arrest_client_t)) & 0xFFFF
        )
        if error:
            error = error - 0x10000
            error_description = self.LIB_ERROR_TEMPLATE.format(
    ...
    def _house_arrest_get_result(self):
        ...
        plist = c_char_p()
        (self.lib.house_arrest_get_result(byref(self.house_arrest), byref(plist)) & 0xFFFF)
    ...
    def _house_arrest_send_command(self, command=None, appid=None):
        ...
        error = self.lib.house_arrest_send_command(byref(self.house_arrest), _command, _appid) & 0xFFFF
        if error:
            error = error - 0x10000
            error_description = self.LIB_ERROR_TEMPLATE.format(
                cls=self.__class__.__name__,
    ...
    def _idevice_get_device_list(self):
        ...
        self.lib.idevice_get_device_list.argtypes = [
            POINTER(POINTER(POINTER(c_char * self.UDID_SIZE))),
            POINTER(c_long),
        ]
    ...
    def _idevice_new(self):
        ...
        error = self.lib.idevice_new(byref(idevice_t), c_void_p()) & 0xFFFF
    ...
    def _instproxy_browse(self, applist=[]):
        ...
        apps = c_void_p()
        error = self.lib.instproxy_browse(byref(self.instproxy), self.client_options, byref(apps)) & 0xFFFF
        ...
        if error:
            ...
            for app in app_list:
                ...
                if not applist:
                    ...
                    if app_name in applist:
                        installed_apps[app["CFBundleName"]] = {
                            "app_id": app["CFBundleIdentifier"],
                            "app_version": app["CFBundleVersion"],
                        }
            ...
            if self.verbose:
                for app in sorted(installed_apps, key=lambda s: s.lower()):
                    attrs = {
                        "app_name": app,
                        "app_id": installed_apps[app]["app_id"],
                        "app_version": installed_apps[app]["app_version"],
                    }
    ...
    def _instproxy_client_new(self):
        ...
        error = self.lib.instproxy_client_new(byref(self.device), self.lockdown, byref(instproxy_client_t)) & 0xFFFF
        if error:
            error = error - 0x10000
            error_description = self.LIB_ERROR_TEMPLATE.format(
                cls=self.__class__.__name__,
    ...
    def _instproxy_client_options_add(self, app_type, domain):
        ...
        self.lib.instproxy_client_options_add(self.client_options, app_type, domain, None)
    ...
    def _lockdown_client_new_with_handshake(self):
        ...
        error = (
            self.lib.lockdownd_client_new_with_handshake(byref(self.device), byref(lockdownd_client_t), SERVICE_NAME)
            & 0xFFFF
        )
        if error:
            error = error - 0x10000
    ...
    def _lockdown_get_device_name(self):
        ...
        device_name = None
        error = self.lib.lockdownd_get_device_name(byref(self.control), byref(device_name_p)) & 0xFFFF
        if error:
            error = error - 0x10000
            error_description = self.LIB_ERROR_TEMPLATE.format(
                cls=self.__class__.__name__,
    ...
    def _lockdown_get_value(self, requested_items=[]):
        ...
        error = self.lib.lockdownd_get_value(byref(self.control), None, None, byref(preferences)) & 0xFFFF
        if error:
            error = error - 0x10000
            error_description = self.LIB_ERROR_TEMPLATE.format(
                cls=self.__class__.__name__,
    ...
    def _lockdown_start_service(self, service_name):
        ...
        error = self.lib.lockdownd_start_service(byref(self.control), SERVICE_NAME, byref(self.lockdown)) & 0xFFFF
        if error:
            error = error - 0x10000
            error_description = self.LIB_ERROR_TEMPLATE.format(
                cls=self.__class__.__name__,
