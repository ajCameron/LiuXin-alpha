...
from LiuXin.metadata import authors_to_string, title_sort, authors_to_sort_string

from LiuXin.utils.localization import _
from LiuXin.utils.calibre import guess_type, isbytestring, fsync
...
class XMLCache(object):
    ...
    def get_or_create_playlist(self, bl_idx, title):
        ...
        ans = root.makeelement(
            "{%s}playlist" % self.namespaces[bl_idx],
            nsmap=root.nsmap,
            attrib={
                "uuid": uuid(),
                "title": title,
            ...
            if record is not None:
                for thumbnail in record.xpath('descendant::*[local-name()="thumbnail"]'):
                    for img in thumbnail.xpath(
                        'descendant::*[local-name()="jpeg"]|' 'descendant::*[local-name()="png"]'
                    ):
                        if img.text:
                            try:
        ...
        for i, booklist in booklists.items():
            ...
            for book in booklist:
                ...
                if plugboard is not None:
                    ...
                    newmi = book
                (gtz_count, ltz_count, use_tz_var) = self.update_text_record(
                    record, newmi, path, i, gtz_count, ltz_count, use_tz_var
                )
