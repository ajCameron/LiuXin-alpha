...
from LiuXin.devices.prs505 import (
    MEDIA_XML,
    MEDIA_EXT,
    CACHE_XML,
    CACHE_EXT,
...
from LiuXin.utils.calibre_utils.calibre_prints import prints
from LiuXin.utils.calibre_utils.calibre_init_functions import fsync
...
class PRS505(USBMS):
    ...
    description = _("Communicate with Sony eBook readers older than the PRST1.")
    author = "Kovid Goyal"
    ...
    VENDOR_NAME = "SONY"
    WINDOWS_MAIN_MEM = re.compile(r"(PRS-(505|500|300))|" r"(PRS-((700[#/])|((6|9|3)(0|5)0&)))")
    WINDOWS_CARD_A_MEM = re.compile(r"(PRS-(505|500)[#/]\S+:MS)|" r"(PRS-((700[/#]\S+:)|((6|9)(0|5)0[#_]))MS)")
    WINDOWS_CARD_B_MEM = re.compile(r"(PRS-(505|500)[#/]\S+:SD)|" r"(PRS-((700[/#]\S+:)|((6|9)(0|5)0[#_]))SD)")
    ...
    EXTRA_CUSTOMIZATION_MESSAGE = [
        _("Comma separated list of metadata fields " "to turn into collections on the device. Possibilities include: ")
        + "series, tags, authors"
        + _(
            ". Two special collections are available: %(abt)s:%(abtv)s and %(aba)s:%(abav)s. Add  "
            "these values to the list to enable them. The collections will be "
            'given the name provided after the ":" character.'
        )
        % dict(abt="abt", abtv=ALL_BY_TITLE, aba="aba", abav=ALL_BY_AUTHOR),
    ...
    def initialize_XML_cache(self):
        from LiuXin.devices.prs505.sony_cache import XMLCache
