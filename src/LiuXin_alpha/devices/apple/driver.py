...
# Todo: Check these methods work
from LiuXin.constants import __appname__, isosx, iswindows
from LiuXin.constants import __calibre_version__ as __version__
from LiuXin.paths import LiuXin_calibre_caches as _cache_dir, config_dir

from LiuXin.devices.errors import OpenFeedback, UserFeedback
from LiuXin.devices.usbms.deviceconfig import DeviceConfig
...
from LiuXin.utils.calibre import fit_image, confirm_config_name, strftime as _strftime
...
def strftime(fmt="%Y/%m/%d %H:%M:%S", dt=None):
    from LiuXin.utils.date import now
...
def logger():
    global _log
    if _log is None:
        from LiuXin.utils.calibre.utils.logging import ThreadSafeLog
    ...
    def custom_dialog(self, parent):
        ...
        class Dialog(QDialog):
            ...
            def do_it(self, return_code):
                from LiuXin.utils.calibre.utils.logging import default_log
                from LiuXin.utils.calibre.utils.config import dynamic
                ...
                if return_code == self.Accepted:
                    ...
                    from LiuXin.utils.calibre.customize.ui import disable_plugin
...
class DriverBase(DeviceConfig, DevicePlugin):
    ...
    EXTRA_CUSTOMIZATION_MESSAGE = [
        ...
        + ":::"
        + _("Enable to use the series name as the iTunes Genre, " "iBooks Category"),
        _("Cache covers from iTunes/iBooks") + ":::" + _("Enable to cache and display covers from iTunes/iBooks"),
        _('"Copy files to iTunes Media folder %s" is enabled in iTunes Preferences|Advanced') % "\u2026"
        + ":::"
        + _(
...
class ITUNES(DriverBase):
    ...
    OPEN_FEEDBACK_MESSAGE = _("Apple iDevice detected, launching iTunes, please wait ...")
    ...
    ArtworkFormat = ["Unknown", "JPEG", "PNG", "BMP"]
    PlaylistKind = ["Unknown", "Library", "User", "CD", "Device", "Radio Tuner"]
    PlaylistSpecialKind = [
        "Unknown",
        "Purchased Music",
        "Party Shuffle",
    ...
    def add_books_to_metadata(self, locations, metadata, booklists):
        ...
        if self.update_list:
            ...
            for (j, p_book) in enumerate(self.update_list):
                if False and self.verbose:
                    if isosx:
                        logger().info(
                            "  looking for '%s' by %s uuid:%s" % (p_book["title"], p_book["author"], p_book["uuid"])
                        )
                    elif iswindows:
                        logger().info(
                            " looking for '%s' by %s (%s)" % (p_book["title"], p_book["author"], p_book["uuid"])
                        )
                ...
                for i, bl_book in enumerate(booklists[0]):
                    if bl_book.uuid == p_book["uuid"]:
                        ...
                        if False:
                            if isosx:
                                logger().info(
                                    "  removing old %s %s from booklists[0]"
                                    % (p_book["title"], str(p_book["lib_book"])[-9:])
                                )
                            elif iswindows:
                                logger().info(" removing old '%s' from booklists[0]" % (p_book["title"]))
                        ...
                        if matching_uuids > 1:
                            for cb in self.cached_books:
                                if self.cached_books[cb]["uuid"] == p_book["uuid"]:
                                    if (
                                        self.cached_books[cb]["title"] == p_book["title"]
                                        and self.cached_books[cb]["author"] == p_book["author"]
                                    ):
                                        if self.verbose:
                                            self._dump_cached_book(
                                                self.cached_books[cb],
                                                header="removing from self.cached_books:",
                                                indent=2,
                                            )
        ...
        for new_book in locations[0]:
            if self.verbose:
                logger().info("  adding '%s' by '%s' to booklists[0]" % (new_book.title, new_book.author))
    ...
    def books(self, oncard=None, end_session=True):
        ...
        """
        from LiuXin.utils.calibre.utils.date import parse_date
            ...
            if "iPod" in self.sources:
                ...
                if isosx:
                    ...
                    for (i, book) in enumerate(device_books):
                        ...
                        this_book.path = self.path_template % (
                            book.name(),
                            book.artist(),
                            format,
                        )
                        ...
                        this_book.library_id = (
                            library_books[this_book.path] if this_book.path in library_books else None
                        )
    ...
    def get_file(self, path, outfile, end_session=True):
        ...
        try:
            ...
            raise UserFeedback(
                "Unable to extract books from iDevices",
                details=None,
    ...
    def open(self, connected_device, library_uuid):
        ...
        if self.verbose:
            ...
            logger().info(
                "%s.open(MFG: %s, VENDOR_ID: %s, MODEL: %s, BCD: %s, PRODUCT_ID: %s)"
                % (self.__class__.__name__, mfg, vendor_id, model, bcd, product_id)
            )
    ...
    def remove_books_from_metadata(self, paths, booklists):
        ...
        for path in paths:
            ...
            for i, bl_book in enumerate(booklists[0]):
                if False and self.verbose:
                    logger().info("  evaluating '%s' by '%s' uuid:%s" % (bl_book.title, bl_book.author, bl_book.uuid))
                ...
                if bl_book.uuid and bl_book.uuid == self.cached_books[path]["uuid"]:
                    ...
                    bl_book.title == self.cached_books[path]["title"]
                    and bl_book.author == self.cached_books[path]["author"]
                ):
                    if True and self.verbose:
                ...
                if found:
                    ...
                    for cb in self.cached_books:
                        ...
                        if (
                            self.cached_books[cb]["uuid"] == self.cached_books[path]["uuid"]
                            and self.cached_books[cb]["author"] == self.cached_books[path]["author"]
                            and self.cached_books[cb]["title"] == self.cached_books[path]["title"]
                        ):
        ...
        if isosx:
            for (i, fpath) in enumerate(files):
                ...
                if self.verbose:
                    logger().info("%s.upload_books()" % self.__class__.__name__)
                    logger().info(
                        " adding '%s' by '%s' uuid:%s to self.cached_books"
                        % (
                            metadata[i].title,
                            authors_to_string(metadata[i].authors),
                            metadata[i].uuid,
            ...
            try:
                ...
                for (i, fpath) in enumerate(files):
                    ...
                    if self.manual_sync_mode and not db_added:
                        ...
                        self.problem_msg = (
                            "Title and/or author metadata mismatch with uploaded books.\n"
    ...
    def _add_device_book(self, fpath, metadata):
        ...
        if isosx:
            ...
            if "iPod" in self.sources:
                ...
                while attempts:
                    try:
                        ...
                        if self.verbose:
                            logger().warning(
                                "  failed to add book, waiting %.1f seconds to try again (attempt #%d)"
                                % (delay, (3 - attempts))
                            )
                        time.sleep(delay)
                else:
                    ...
                    raise UserFeedback(
                        "Unable to add '%s' in direct connect mode" % metadata.title,
                        details=None,
            ...
            if "iPod" in self.sources:
                ...
                db_added = None
                for pl in device.Playlists:
                    if pl.Kind == self.PlaylistKind.index("User") and pl.SpecialKind == self.PlaylistSpecialKind.index(
                        "Books"
                    ):
                        break
                else:
        ...
        if self.manual_sync_mode:
            ...
            if not self.settings().extra_customization[self.USE_ITUNES_STORAGE]:
                local_copy = os.path.join(
                    self.iTunes_local_storage,
                    str(metadata.uuid) + os.path.splitext(fpath)[1],
                )
            ...
            if not lb_added:
                raise UserFeedback(
                    "iTunes Media folder inaccessible",
        ...
        if metadata.cover:
            ...
            if format == "epub":
                ...
                try:
                    ...
                    if scaled:
                        if self.verbose:
                            logger().info("   cover scaled from %sx%s to %sx%s" % (width, height, nwidth, nheight))
                ...
                if isosx:
                        ...
                        while attempts:
                            try:
                                ...
                                if self.verbose:
                                    ...
                                    logger().info(
                                        "  waiting %.1f seconds for artwork to become writable (attempt #%d)"
                                        % (delay, (10 - attempts))
                                    )
                        else:
                            if self.verbose:
                    ...
                    if lb_added:
                        try:
                            ...
                            if self.verbose:
                                logger().info(
                                    "  iTunes automation interface reported an error"
    ...
    def _discover_manual_sync_mode(self, wait=0):
        ...
        if isosx:
            ...
            if dev_books is not None and len(dev_books):
                ...
                if False:
                    logger().info(
                        "  determining manual mode by modifying '%s' by %s" % (first_book.name(), first_book.artist())
                    )
                try:
    ...
    def _dump_booklist(self, booklist, header=None, indent=0):
        ...
        for book in booklist:
            if isosx:
                logger().info(
                    "%s%-40.40s %-30.30s %-40.40s %-10.10s"
                    % (
                        " " * indent,
                        book.title,
                        book.author,
                        book.uuid,
                        str(book.library_id)[-9:],
                    )
                )
            elif iswindows:
                logger().info("%s%-40.40s %-30.30s" % (" " * indent, book.title, book.author))
        logger().info()
    ...
    def _dump_cached_book(self, cached_book, header=None, indent=0):
        ...
        if isosx:
            if header:
                ...
                logger().info("%s%s" % (" " * indent, "-" * len(msg)))
                logger().info(
                    "%s%-40.40s %-30.30s %-10.10s %-10.10s %s"
                    % (" " * indent, "title", "author", "lib_book", "dev_book", "uuid")
                )
            logger().info(
                "%s%-40.40s %-30.30s %-10.10s %-10.10s %s"
                % (
    ...
    def _dump_cached_books(self, header=None, indent=0):
        ...
        if isosx:
            for cb in self.cached_books.keys():
                logger().info(
                    "%s%-40.40s %-30.30s %-40.40s %-10.10s %-10.10s"
                    % (
                        " " * indent,
                        self.cached_books[cb]["title"],
                        self.cached_books[cb]["author"],
    ...
    def _dump_epub_metadata(self, fpath):
        """"""
        from LiuXin.utils.calibre.ebooks.BeautifulSoup import BeautifulSoup
        from LiuXin.utils.calibre_utils.calibre_zipfile import ZipFile
        ...
        if opf:
            opf_raw = six_cStringIO(zf.read(opf))
    ...
    def _dump_update_list(self, header=None, indent=0):
        ...
        if isosx:
            ...
            for ub in self.update_list:
                logger().info("%s%-40.40s %-30.30s" % (" " * indent, ub["title"], ub["author"]))
    ...
    def _find_device_book(self, search, attempts=9):
        ...
        if iswindows:
            ...
            if self.verbose:
                ...
                logger().info("  searching for '%s' by '%s' (%s)" % (search["title"], search["author"], search["uuid"]))
            ...
            while attempts:
                ...
                if search["format"] == "pdf":
                    ...
                    hits = dev_books.Search("%s - %s" % (title, author), self.SearchField.index("All"))
                    if hits:
                        hit = hits[0]
                        logger().info("   found '%s' by %s (%s)" % (hit.Name, hit.Artist, hit.Composer))
                        return hit
        ...
        if iswindows:
            ...
            while attempts:
                ...
                if search["format"] == "pdf":
                    ...
                    if self.verbose:
                        logger().info("   searching by name: %s - %s" % (title, author))
                    hits = lib_books.Search("%s - %s" % (title, author), self.SearchField.index("All"))
                    if hits:
                        hit = hits[0]
                        logger().info("   found '%s' by %s (%s)" % (hit.Name, hit.Artist, hit.Composer))
                        return hit
        ...
        if isosx:
            ...
            try:
                tmp_thumb = os.path.join(
                    tempfile.gettempdir(),
                    "thumb.%s" % self.ArtworkFormat[book.Artwork.Item(1).Format],
                )
            ...
            if "iPod" in self.sources:
                try:
                    ...
                    if device.Playlists is not None:
                        dev_books = None
                        for pl in device.Playlists:
                            if pl.Kind == self.PlaylistKind.index(
                                "User"
                            ) and pl.SpecialKind == self.PlaylistSpecialKind.index("Books"):
                                if self.verbose:
                                    logger().info("  Books playlist: '%s'" % (pl.Name))
                                dev_books = pl.Tracks
                                break
                        ...
                        if book.kind() in self.Audiobooks:
                            ...
                            if str(book.description()).startswith(self.description_prefix):
                                try:
                                    ...
                                    if self.verbose:
                                        logger().error(
                                            "   iTunes returned an error returning .location() with %s" % book.name()
                                        )

                            library_books[path] = book
                            if self.verbose:
                                logger().info(
                                    "   %-30.30s %-30.30s %-40.40s [%s]"
                                    % (
                                        book.name(),
                                        book.artist(),
                                        book.album(),
        ...
        if "iPod" in self.sources:
            ...
            if isosx:
                if "Purchased" in self.iTunes.sources[connected_device].playlists.name():
                    return [
                        pb.database_ID()
                        for pb in self.iTunes.sources[connected_device].playlists["Purchased"].file_tracks()
                    ]
                else:
                    return []
            elif iswindows:
    ...
    def _get_sources(self):
        ...
        if kinds.count("iPod") > 1:
            if self.verbose:
                logger().error(
                    "  %d connected iPod devices detected, calibre supports a single connected iDevice"
                    % kinds.count("iPod")
                )
    ...
    def _launch_iTunes(self):
        ...
        if isosx:
            ...
            if not "iTunes" in running_apps.processes.name():
                ...
                try:
                    ...
                    self.iTunes = None
                    raise UserFeedback(
                        " %s._launch_iTunes(): unable to find installed iTunes" % self.__class__.__name__,
                        details=None,
                        level=UserFeedback.WARN,
                    )
            ...
            try:
                ...
                try:
                    ...
                    if self.verbose:
                        logger().info(
                            "   unable to communicate with iTunes via %s %s using any binding" % (as_name, as_version)
                        )
                    return
            ...
            if self.verbose:
                ...
                logger().info(
                    "  [OSX %s, %s %s (%s), %s driver version %d.%d.%d]"
                    % (
                        platform.mac_ver()[0],
                        self.iTunes.name(),
                        self.iTunes.version(),
                ...
                logger().info(
                    "  communicating with iTunes via %s %s using %s binding" % (as_name, as_version, as_binding)
                )
        ...
        if iswindows:
            ...
            *** Current implementation doesn't handle UNC paths correctly,
                ...
                Once you have the //reader_server/share, convert with os.path.normpath('//reader_server/share')
                os.path.splitdrive doesn't work as advertised, so use os.path.splitunc
                os.path.splitunc("//reader_server/share") returns ('//reader_server/share','')
                os.path.splitunc("C:/Documents") returns ('c:','/documents')
                os.path.normpath("//reader_server/share") returns "\\\\reader_server\\share"
            """
            # Instantiate iTunes
            try:
                self.iTunes = win32com.client.Dispatch("iTunes.Application")
            ...
            if self.verbose:
                logger().info("  %s %s" % (__appname__, __version__))
                logger().info(
                    "  [Windows %s - %s (%s), driver version %d.%d.%d]"
                    % (
                        self.iTunes.Windows[0].name,
                        self.iTunes.Version,
                        self.initial_status,
                        self.version[0],
                        self.version[1],
    ...
    def _purge_orphans(self, library_books, cached_books):
        ...
        for book in library_books:
            if isosx:
                if book not in cached_books and str(library_books[book].description()).startswith(
                ...
                if book not in cached_books and library_books[book].Description.startswith(self.description_prefix):
                    if PURGE_ORPHANS:
                        if self.verbose:
                            logger().info("  '%s' not found on iDevice, removing from iTunes" % book)
                        btr = {
        ...
        if self.manual_sync_mode:
            ...
            for book in self.cached_books:
                if self.cached_books[book]["uuid"] == metadata.uuid or (
                    self.cached_books[book]["title"] == metadata.title
                    and self.cached_books[book]["author"] == metadata.author
                ):
    ...
    def _remove_from_device(self, cached_book):
        ...
        if isosx:
            ...
            if hit:
                ...
                if self.verbose:
                    logger().warning(
                        "   unable to remove '%s' by '%s' (%s) from device"
                        % (
                            cached_book["title"],
                            cached_book["author"],
                            cached_book["uuid"],
        ...
        if isosx:
            ...
            try:
                ...
                if fp.startswith(prefs["library_path"]):
                    ...
                    not self.settings().extra_customization[self.USE_ITUNES_STORAGE]
                    and fp.startswith(self.iTunes_local_storage)
                ...
                if self.verbose:
                    logger().info(
                        "   '%s' by %s not found in iTunes storage" % (cached_book["title"], cached_book["author"])
                    )
    ...
    def _update_epub_metadata(self, fpath, metadata):
        ...
        from LiuXin.metadata.file_sources.epub import set_metadata
        ...
        with open(fpath, "r+b") as zfo:
            if False:
                try:
                    ...
                    raise UserFeedback(
                        "'%s' is not a valid EPUB" % metadata.title,
                        None,
                        level=UserFeedback.WARN,
                    )
                ...
                if md_els:
                    ...
                    if ts is not None:
                        ...
                        metadata.timestamp = datetime.datetime(
                            old_ts.year,
                            old_ts.month,
                            old_ts.day,
                            old_ts.hour,
        ...
        if isosx:
            ...
            if wait:
                ...
                if self.verbose:
                    ...
                    sys.stdout.flush()
                while len(self._get_device_books()) != (
    ...
    def _update_iTunes_metadata(self, metadata, db_added, lb_added, this_book):
        ...
        if isosx:
            ...
            if metadata_x.series and self.settings().extra_customization[self.USE_SERIES_AS_CATEGORY]:
                ...
                if self.verbose:
                    logger().info(
                        "   %susing Tag as Genre" % "no Series name available, "
            ...
            if db_added:
                ...
                while attempts:
                    try:
                        ...
                        if self.verbose:
                            logger().info(
                                "  waiting %.1f seconds for iDevice metadata to become writable (attempt #%d)"
            ...
            if metadata_x.rating:
                ...
                try:
                    ...
                    if self.verbose:
                        logger().info(
                            "  iTunes automation interface reported an error" " setting AlbumRating on iDevice"
                        )
            ...
            if metadata_x.series and self.settings().extra_customization[self.USE_SERIES_AS_CATEGORY]:
                ...
                if lb_added:
                    # If no title_sort plugboard tweak, create sort_name from series/index
                    if metadata.title_sort == metadata_x.title_sort:
                        lb_added.SortName = "%s %s" % (
                            self.title_sorter(metadata_x.series),
                            series_index,
                        )
                    ...
                    try:
                        ...
                        if self.verbose:
                            logger().info(
                                "  iTunes automation interface reported an error" " setting TrackNumber in iTunes"
                            )
                    try:
                        ...
                        if self.verbose:
                            logger().info(
                                "  iTunes automation interface reported an error" " setting EpisodeNumber in iTunes"
                            )
                ...
                if db_added:
                    ...
                    try:
                        ...
                        if self.verbose:
                            logger().info(
                                "  iTunes automation interface reported an error" " setting TrackNumber on iDevice"
                            )
                    try:
                        ...
                        if self.verbose:
                            logger().info(
                                "  iTunes automation interface reported an error" " setting EpisodeNumber on iDevice"
                            )
        ...
        if self.plugboard_func:
            ...
            if pb is not None and self.verbose:
                ...
                logger().info(
                    ...
                    % (
                        book.title,
                        ">>> '%s'" % newmi.title if book.title != newmi.title else "",
                    )
                )
                logger().info(
                    "  title_sort: %s %s"
                    % (
                        book.title_sort,
                        ">>> %s" % newmi.title_sort if book.title_sort != newmi.title_sort else "",
                    )
                )
                logger().info(
                    "     authors: %s %s"
                    % (
                        book.authors,
                        ">>> %s" % newmi.authors if book.authors != newmi.authors else "",
                    )
                )
                logger().info(
                    " author_sort: %s %s"
                    % (
                        book.author_sort,
                        ">>> %s" % newmi.author_sort if book.author_sort != newmi.author_sort else "",
                    )
                )
                logger().info(
                    "    language: %s %s"
                    % (
                        book.language,
                        ">>> %s" % newmi.language if book.language != newmi.language else "",
                    )
                )
                logger().info(
                    "   publisher: %s %s"
                    % (
                        book.publisher,
                        ">>> %s" % newmi.publisher if book.publisher != newmi.publisher else "",
                    )
                )
                logger().info(
                    "        tags: %s %s"
                    % (
                        book.tags,
                        ">>> %s" % newmi.tags if book.tags != newmi.tags else "",
                    )
                )
            else:
                if self.verbose:
...
class ITUNES_ASYNC(ITUNES):
    ...
    def open(self, connected_device, library_uuid):
        ...
        if self.verbose:
            logger().info("%s.open(connected_device: %s)" % (self.__class__.__name__, repr(connected_device)))
