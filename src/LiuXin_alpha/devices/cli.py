...
from LiuXin.customize.ui import device_plugins

from LiuXin.devices.errors import ArgumentError, DeviceError, DeviceLocked
from LiuXin.devices.scanner import DeviceScanner

from LiuXin.utils.calibre import __version__, __appname__, human_readable, fsync
from LiuXin.utils.config.config_tools import device_prefs
        ...
        if not ll and len(lsoutput) > 0:
            ...
            for r in range(len(trytable)):
                for c in range(len(trytable[r])):
                    ...
                    print(trytablecol[r][c], "".ljust(padding), file=output)
                print(file=output)
        print(file=output)
...
def main():
    from LiuXin.utils.terminal import geometry
    ...
    if dev is None:
        print("Unable to find a connected ebook reader.", sys.stderr)
        shutdown_plugins()
        return 1
    ...
    try:
        if command == "df":
            ...
            parser = OptionParser(
                usage="usage: %prog mkdir [options] path\nCreate a directory on the device\n\npath must begin with / or card:/"
            )
            if len(args) != 1:
                parser.print_help()
                sys.exit(1)
            ...
            parser = OptionParser(
                usage="usage: %prog ls [options] path\nList files on the device\n\npath must begin with / or card:/"
            )
            parser.add_option(
                "-l",
                help="In addition to the name of each file, print the file type, permissions, and  timestamp  (the  modification time, in the local timezone). Times are local.",  # noqa
                dest="ll",
                action="store_true",
                default=False,
            )
            parser.add_option(
                "-R",
                help="Recursively list subdirectories encountered. /dev and /proc are omitted",
                dest="recurse",
                action="store_true",
                default=False,
            )
            ...
            parser = OptionParser(
                ...
                + "Unfortunately, I cant figure out how to update file times on the device, so if path already exists, touch does nothing"  # noqa
            )
            options, args = parser.parse_args(args)
            if len(args) != 1:
                parser.print_help()
