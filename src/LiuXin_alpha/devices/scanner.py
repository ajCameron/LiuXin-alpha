...
"""
Device scanner that fetches list of devices on system in a platform dependent manner.
"""
...
from LiuXin.constants import iswindows, isosx, islinux, isfreebsd, isnetbsd
...
_USBDevice = namedtuple("USBDevice", "vendor_id product_id bcd manufacturer product serial")
...
class LibUSBScanner(object):
    def __call__(self):
        if not hasattr(self, "libusb"):
            ...
            if self.libusb is None:
                raise ValueError("DeviceScanner needs libusb to work. Error: %s" % self.libusb_err)
    ...
    def check_for_mem_leak(self):
        import gc
        from LiuXin.utils.calibre.utils.mem import memory
        ...
        try:
            ...
            for path in paths:
                ...
                while parentdriver != "umass":
                    try:
                        obj = bus.get_object("org.freedesktop.Hal", objif.GetProperty("info.parent"))
                        objif = dbus.Interface(obj, "org.freedesktop.Hal.Device")
                        try:
...
def test_for_mem_leak():
    from LiuXin.utils.mem import memory, gc_histogram, diff_hists
    import gc
    ...
    for reps in (1, 10, 100, 1000):
        ...
        prints("Memory used in %d repetitions of scan(): %.5f KB" % (reps, 1024 * usedmem))
    ...
    for reps in (1, 10, 100, 1000):
        ...
        for i in xrange(3):
            gc.collect()
        usedmem = memory(startmem)
        prints("Memory used in %d repetitions of pnp_scan(): %.5f KB" % (reps, 1024 * usedmem))
        prints("Differences in python object counts:")
        diff_hists(h1, gc_histogram())
        prints()
