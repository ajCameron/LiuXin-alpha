...
class DevicePlugin(Plugin):
    ...
    def is_usb_connected_windows(self, devices_on_system, debug=False, only_presence=False):
        ...
        for vendor_id, product_id, bcd in id_iterator():
            ...
            for device_id in devices_on_system:
                if (
                    (vid in device_id or vidd in device_id)
                    and (pid in device_id or pidd in device_id)
                    and self.test_bcd_windows(device_id, bcd)
                ):
                    if debug:
                        self.print_usb_device_info(device_id)
                    if only_presence or self.can_handle_windows(device_id, debug=debug):
                        try:
                            bcd = int(device_id.rpartition("rev_")[-1].replace(":", "a"), 16)
                        except:
                            bcd = None
    ...
    def is_usb_connected(self, devices_on_system, debug=False, only_presence=False):
        ...
        """
        if iswindows:
            return self.is_usb_connected_windows(devices_on_system, debug=debug, only_presence=only_presence)
...
class BookList(list):
    ...
    def add_book(self, book, replace_metadata):
        """
        Add the book to the booklist. Intent is to maintain any device-internal metadata. Return True if booklists must
        be sync'ed.
        :param book:
