...
class APNXBuilder(object):
    ...
    def generate_apnx(self, pages, apnx_meta):
        ...
        if apnx_meta["format"] == "MOBI_8":
            content_header = (
                '{"contentGuid":"%(guid)s","asin":"%(asin)s","cdeType":"%(cdetype)s","format":"%(format)s","fileRevisionId":"1","acr":"%(acr)s"}'
                % apnx_meta
            )  # noqa
        else:
            ...
            content_header = (
                '{"contentGuid":"%(guid)s","asin":"%(asin)s","cdeType":"%(cdetype)s","fileRevisionId":"1"}' % apnx_meta
            )
