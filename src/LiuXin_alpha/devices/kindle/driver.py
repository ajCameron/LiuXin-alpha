...
from LiuXin.devices.kindle.bookmark import Bookmark
from LiuXin.devices.usbms.driver import USBMS

from LiuXin.utils.calibre import strftime, fsync
...
class KINDLE(USBMS):
    ...
    def get_annotations(self, path_map):
        ...
        if mc_path:
            ...
            bookmarked_books["clippings"] = self.UserAnnotation(
                type="kindle_clippings", value=dict(path=mc_path, timestamp=timestamp)
            )
    ...
    def generate_annotation_html(self, bookmark):
        from LiuXin.utils.calibre.ebooks.BeautifulSoup import (
            BeautifulSoup,
            Tag,
            NavigableString,
        )
...
class KINDLE2(KINDLE):
    ...
    def upload_kindle_thumbnail(self, metadata, filepath):
        ...
        thumbfile = os.path.join(
            thumb_dir,
            "thumbnail_{uuid}_{cdetype}_portrait.jpg".format(uuid=mh.exth.uuid, cdetype=mh.exth.cdetype),
        )
    ...
    def upload_apnx(self, path, filename, metadata, filepath):
        from LiuXin.devices.kindle.apnx import APNXBuilder

        opts = self.settings()
        if not opts.extra_customization[self.OPT_APNX]:
            return

        if os.path.splitext(filepath.lower())[1] not in (
            ".azw",
            ".mobi",
            ".prc",
            ".azw3",
        ...
        if opts.extra_customization[self.OPT_APNX_OVERWRITE] or not os.path.exists(apnx_path):
            try:
                ...
                if cust_col_name:
                    try:
                        temp = six_unicode(metadata.get(cust_col_name)).lower()
