...
def debug(ioreg_to_tmp=False, buf=None, plugins=None, disabled_plugins=None):
    ...
    from LiuXin.customize.ui import device_plugins, disabled_device_plugins

    from LiuXin.devices.scanner import DeviceScanner, win_pnp_drives

    from LiuXin import prints
    from LiuXin.utils.debug import print_basic_debug_info
    ...
    devplugins = list(sorted(devplugins, cmp=lambda x, y: cmp(x.__class__.__name__, y.__class__.__name__)))
    if plugins is None:
        for d in devplugins:
            try:
                d.startup()
    ...
    try:
        ...
        if iswindows:
            ...
            out("Drives detected:")
            for drive in sorted(drives.keys(), key=operator.attrgetter("order")):
                prints("\t(%d)" % drive.order, drive, "~", drives[drive])

        ioreg = None
        if isosx:
            from LiuXin.utils.calibre.devices.usbms.device import Device
        ...
        if disabled_plugins:
            out(
                "\nDisabled plugins:",
                textwrap.fill(" ".join([x.__class__.__name__ for x in disabled_plugins])),
            )
            out(" ")
        else:
            out("\nNo disabled plugins")
