...
from LiuXin.constants import load_library
...
class Bambook:
    ...
    def SendFile(self, fileName, guid=None):
        ...
        if self.handle:
            taskID = job.NewJob()
            if guid:
                if BambookReplacePrivBook(self.handle, fileName, guid, bambookTransferCallback, taskID):
...
if __name__ == "__main__":
    ...
    if bb.VerifySNB("/tmp/f8268e6c1f4e78c.snb"):
        passed()
    else:
        failed()
