...
class UDisks(object):
    ...
    def device(self, device_node_path):
        ...
        return dbus.Interface(
            self.bus.get_object("org.freedesktop.UDisks", devpath),
...
class UDisks2(object):
    ...
    def __init__(self):
        ...
        self.bus = dbus.SystemBus()
        try:
            self.bus.get_object("org.freedesktop.UDisks2", "/org/freedesktop/UDisks2")
        except dbus.exceptions.DBusException as e:
            if getattr(e, "_dbus_error_name", None) == "org.freedesktop.DBus.Error.ServiceUnknown":
                raise NoUDisks2()
            raise
    ...
    def device(self, device_node_path):
        ...
        try:
            device = bd.Get(self.BLOCK, "Device", dbus_interface="org.freedesktop.DBus.Properties")
        ...
        devs = self.bus.get_object("org.freedesktop.UDisks2", "/org/freedesktop/UDisks2/block_devices")
    ...
    def unmount(self, device_node_path):
        d = self.device(device_node_path)
        d.Unmount(
            {"force": True, "auth.no_user_interaction": True},
            dbus_interface=self.FILESYSTEM,
        )

    def drive_for_device(self, device):
        drive = device.Get(self.BLOCK, "Drive", dbus_interface="org.freedesktop.DBus.Properties")
        return self.bus.get_object("org.freedesktop.UDisks2", drive)

    def eject(self, device_node_path):
        drive = self.drive_for_device(self.device(device_node_path))
        drive.Eject({"auth.no_user_interaction": True}, dbus_interface=self.DRIVE)
