"""AgentMatcher implementation scaffold."""
...
class AgentMatcher:
    ...
    def candidates(self, candidate: MetadataCandidate, *, limit: int = 20) -> Sequence[MatchResult]:
        raise NotImplementedError("Move agent candidate matching policy here from databases")

    def best(self, candidate: MetadataCandidate) -> MatchResult:
        results = self.candidates(candidate, limit=1)
        if results:
            return results[0]
        return MatchResult(entity_id=None, confidence=0.0, reason="no agent candidates")
