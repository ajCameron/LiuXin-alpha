"""
API contract for a rich ``item metadata`` container.

An item sits at the point where bibliographic identity meets storage reality.
This API therefore bundles three kinds of information:

- the core ``items`` row (copy/exemplar identity)
- bibliographic context above it (manifestation/expression/work, agents, etc.)
- storage context beside/below it (digital assets, replicas, stores, folders)
"""
...
class ItemMetadataContainerAPI(abc.ABC):
    ...
    @property
    def works(self) -> list[Any]:
        return self.get_related("works")

    @works.setter
    def works(self, values: Iterable[Any]) -> None:
    ...
    @stores.setter
    def stores(self, values: Iterable[Any]) -> None:
    ...
    @annotations.setter
    def annotations(self, values: Iterable[Any]) -> None:
