...
These containers model agent-to-target credit records.
They are editable metadata value objects, not agent identity objects and not read-side views.

These provide entries to model an agents
 - credit for a work
 - credit for an item
 - credit for an expression
e.t.c.
"""
...
class WorkAgentCredit(AgentCreditBase):
    """
    Credit for an agent interacting with a work.
    """
    work_id: WorkID
    role: WorkAgentRole
...
class ExpressionAgentCredit(AgentCreditBase):
    """
    Credit for a Agent working on an expression.
    """
...
class ManifestationAgentCredit(AgentCreditBase):
    """
    Credit for an agent working on a Manifestation.
    """
...
class RoleCreditsContainer(Generic[CreditT, RoleT], abc.ABC):
    ...
    def __init__(
            ...
            *,
            role: RoleT,
            target_id: int,
            credits: list[CreditT] | None = None) -> None:
        self.role = role
...
class WorkRoleCreditsContainer(RoleCreditsContainer[WorkAgentCredit, WorkAgentRole]):
    """
    Contains all the credits for a work role.
    """
