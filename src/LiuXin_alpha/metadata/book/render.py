...
def mi_to_html(
    ...
    for field in (field for field, display in field_list if display):
        ...
        if metadata["datatype"] == "comments" or field == "comments":
                ...
                if metadata["display"].get("contains_html", False):
                    ...
                    if not metadata["is_multiple"]:
                        val = '<a href="%s" title="%s">%s</a>' % (
                            ...
                            _("Click to see books with {0}: {1}").format(metadata["name"], a(val)),
                            p(val),
                        )
                    else:
                        all_vals = [v.strip() for v in val.split(metadata["is_multiple"]["list_to_ui"]) if v.strip()]
                        links = [
                            '<a href="%s" title="%s">%s</a>'
                            % (
                                search_href(field, x),
                                _("Click to see books with {0}: {1}").format(metadata["name"], a(x)),
                                p(x),
                            )
                            for x in all_vals
                        ]
            ...
            if mi.path:
                ...
                url = prepare_string_for_xml(path if isdevice else six_unicode(mi.id), True)
            ...
            fmts = ['<a title="{path}/{fname}.{ext}" href="format:{id}:{fmt}">{fmt}</a>'.format(**x) for x in data]
            ...
            for aut in mi.authors:
                ...
                aut = p(aut)
                if link:
                    authors.append('<a calibre-data="authors" title="%s" href="%s">%s</a>' % (lt, link, aut))
                else:
                    authors.append(aut)
            ...
            if metadata["datatype"] == "series":
                ...
                val = _(
                    '%(sidx)s of <a href="%(href)s" title="%(tt)s">' '<span class="%(cls)s">%(series)s</span></a>'
                ) % dict(
                ...
                val = '<a href="%s" title="%s">%s</a>' % (
                    search_href(st, val),
                    _("Click to see books with {0}: {1}").format(metadata["name"], val),
                    val,
                )
    ...
    ans = [
        '<tr id="%s" class="%s">%s</tr>' % (fieldl.replace("#", "_"), classname(fieldl), html) for fieldl, html in ans
    ]
    # print '\n'.join(ans)
