...
# Worker for the web metadata interface - supports the downloading of metadata from the internet
...
from LiuXin.utils.date import as_utc
from LiuXin.file_formats.metadata.sources.identify import identify, msprefs
from LiuXin.file_formats.metadata.book.base import Metadata
from LiuXin.customize.ui import metadata_plugins
from LiuXin.file_formats.metadata.sources.covers import download_cover, run_download
from LiuXin.file_formats.metadata.sources.base import dump_caches, load_caches
from LiuXin.utils.logger import GUILog
from LiuXin.file_formats.metadata.opf2 import metadata_to_opf, OPF
...
def single_identify(title, authors, identifiers):
    ...
    results = identify(log, Event(), title=title, authors=authors, identifiers=identifiers)
    return (
        [metadata_to_opf(r) for r in results],
        [r.has_cached_cover_url for r in results],
        dump_caches(),
        log.dump(),
    )
