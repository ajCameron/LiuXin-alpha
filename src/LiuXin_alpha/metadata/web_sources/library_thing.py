...
from LiuXin import browser, random_user_agent, prints

from LiuXin.metadata.metadata import MetaInformation

from LiuXin.utils.config.config_tools import OptionParser
from LiuXin.utils.calibre_chardet import strip_encoding_declarations
...
def check_for_cover(isbn, timeout=5.0):
    br = get_browser()
...
"""
    ...
    parser.add_option("-u", "--username", default=None, help="Username for LibraryThing.com")
    parser.add_option("-p", "--password", default=None, help="Password for LibraryThing.com")
    return parser


def get_social_metadata(title, authors, publisher, isbn, username=None, password=None):
    ...
    if isbn:
        ...
        if h1 and not mi.title:
            mi.title = html.tostring(h1[0], method="text", encoding=six_unicode)
        h2 = root.xpath('//div[@class="headsummary"]/h2/a')
        if h2 and not mi.authors:
            mi.authors = [html.tostring(x, method="text", encoding=six_unicode) for x in h2]
        h3 = root.xpath('//div[@class="headsummary"]/h3/a')
        if h3:
            match = None
            for h in h3:
                series = html.tostring(h, method="text", encoding=six_unicode)
        ...
        #            in tags]
        span = root.xpath('//table[@class="wsltable"]/tr[@class="wslcontent"]/td[4]//span')
        if span:
            raw = html.tostring(span[0], method="text", encoding=six_unicode)
...
def main(args=sys.argv):
    ...
    if len(args) != 2:
        ...
        return 1
    isbn = args[1]
    from LiuXin.customize.ui import metadata_sources, cover_sources
