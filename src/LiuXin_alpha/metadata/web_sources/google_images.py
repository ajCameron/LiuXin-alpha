...
class GoogleImages(Source):
    ...
    def get_image_urls(self, title, author, log, abort, timeout):
        ...
        try:
            return fork_job(
                "calibre.ebooks.metadata.sources.google_images",
                "search",
                args=(title, author, self.prefs["size"], timeout),
                no_output=True,
                abort=abort,
                timeout=timeout,
            )["result"]
...
def find_image_urls(br, ans):
    ...
    for w in br.page.mainFrame().documentElement().findAll("div#ires a.rg_l[href]"):
        try:
            imgurl = urlparse.parse_qs(urlparse.urlparse(six_unicode(w.attribute("href"))).query)["imgurl"][0]
        except:
            # import traceback
            # traceback.print_exc()
            continue
...
def search(title, author, size, timeout, debug=False):
    import time
    from LiuXin.utils.web.jsbrowser.browser import Browser, LoadWatcher, Timeout
