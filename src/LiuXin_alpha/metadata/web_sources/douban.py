...
def to_metadata(browser, log, entry_, timeout):  # {{{
    ...
    # Todo: Merge the two copies of cardet after making sure that they function the same
    from LiuXin.file_formats.chardet import xml_to_unicode
    from LiuXin.utils.date import parse_date, utcnow
    ...
    try:
        ...
        feed = etree.fromstring(xml_to_unicode(clean_ascii_chars(raw), strip_encoding_pats=True)[0])
        extra = entry(feed)[0]
    except Exception as e:
        err_str = "Failed to get additional details for"
        log.log_exception(err_str, e, "DEBUG", ("mi.title", mi.title))
        return mi
...
class Douban(Source):
    ...
    def identify(
        ...
        try:
            ...
            err_str = "Failed to parse identify results"
        ...
        # There is no point running these queries in threads as douban throttles requests returning 403 Forbidden errors
