...
from LiuXin.utils.calibre import sanitize_file_name2
from LiuXin import prints
from LiuXin.metadata import check_isbn
from LiuXin.metadata.web_sources.base import create_log, get_cached_cover_urls
...
def series_test(series, series_index):
    ...
    def test(mi):
        ...
        if mi.series:
            prints(
                "Series test failed. Expected: '%s [%d]' found '%s[%d]'" % (series, series_index, ms, mi.series_index)
            )
        else:
    ...
    for kwargs, test_funcs in tests:
        ...
        if "cover" in plugin.capabilities:
            ...
            if not results and fail_missing_meta:
                ...
                cover = os.path.join(
                    tdir,
                    plugin.name.replace(" ", "") + "-%s-cover.jpg" % sanitize_file_name2(mi.title.replace(" ", "_")),
                )
