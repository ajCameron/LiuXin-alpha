...
# Todo: Merge with the main logging system
from LiuXin.metadata.web_sources.base import create_log
from LiuXin.metadata.web_sources.prefs import msprefs
from LiuXin.metadata.web_sources.xisbn import xisbn

from LiuXin.utils.date import utc_tz, as_utc
from LiuXin.utils.html2text import html2text
from LiuXin.utils.icu import lower
from LiuXin.utils.date import UNDEFINED_DATE
...
class ISBNMerge(object):
    ...
    def finalize(self):
        ...
        results = [
            r
            for r in results
            if r.identify_plugin not in isbn_sources or not r.identify_plugin.prefer_results_with_isbn
        ]
        if results:
...
def identify(log, abort, title=None, authors=None, identifiers={}, timeout=30):
    ...
    log(
        "Merging results from different sources and finding earliest ",
...
def urls_from_identifiers(identifiers):  # {{{
    from LiuXin.customize.ui import all_metadata_plugins
    ...
    if isbn:
        ans.append((isbn, "isbn", isbn, "http://www.worldcat.org/isbn/" + isbn))
    ...
    if oclc:
        ans.append(("OCLC", "oclc", oclc, "http://www.worldcat.org/oclc/" + oclc))
    issn = check_issn(identifiers.get("issn", None))
    if issn:
        ans.append((issn, "issn", issn, "http://www.worldcat.org/issn/" + issn))
    for x in ("uri", "url"):
