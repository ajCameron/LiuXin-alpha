# Generates test_db_7 - which is the regular data set but without most of it's titles
...
class TestDB7Builder(TestDatabaseBuilder):
    """
    Constructs test_db_7 - which is the regular test database - but without most of the titles (means it's faster to
    ...
    def detail_databases(scratch_db):
        ...
        approved_title_set = {0, 1, 5, 6, 7, 18}
...
def build_test_db(
    ...
    """
    test_db_7 is intended for applications where the whole database had to be read into the cache - as such size is at a
    premium in order to speed up the tests.
    The test database has only got five titles (six if the
    ...
    :return:
    """
    test_db_builder = TestDB7Builder(
